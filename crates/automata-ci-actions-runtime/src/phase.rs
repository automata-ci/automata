...
pub struct ActionsCompletedStepApplicator {
    limits: PhaseApplicationLimits,
}

impl ActionsCompletedStepApplicator {
    /// Creates an applicator constrained by `limits`.
    #[must_use]
    pub const fn new(limits: PhaseApplicationLimits) -> Self {
        Self { limits }
    }

    /// Returns the limits applied to each derived job-state snapshot.
    #[must_use]
...
impl Default for ActionsCompletedStepApplicator {
    fn default() -> Self {
        Self::new(PhaseApplicationLimits::default())
    }
}

impl CompletedStepApplicator for ActionsCompletedStepApplicator {
    fn apply_completed_step(
        &self,
        current: &JobCommandState,
        scope: &StepScope,
