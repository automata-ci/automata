use automata_ci_actions_runtime::{
    ARTIFACT_LIST_SCHEMA_VERSION, ActionsCommandFileDecoder, ActionsCompletedStepApplicator,
    ...
    CompletedStepApplicator, CompletedStepCommands, EnvironmentCommandFile, JobCommandState,
...
fn decode(source: &[u8]) -> automata_ci_actions_runtime::ArtifactDeclarationCommandFile {
    let ParsedCommandFile::Artifacts(file) = ActionsCommandFileDecoder::default()
        .decode(
...
fn malformed_explicit_oci_equals_and_unsupported_schemes_fail_without_leaking_data() {
    let decoder = ActionsCommandFileDecoder::default();
    for source in [
...
fn declaration_file_reports_a_lower_configured_ceiling() {
    ...
    let decoder = ActionsCommandFileDecoder::new(limits);
