use automata_ci_actions_runtime::{
    ...
    CompletedStepCommands, EnvironmentMutationBlockReason, JobCommandState, ParsedCommandFile,
...
fn value<'a>(
    values: &'a [automata_ci_actions_runtime::NameValueCommand],
    name: &str,
) -> Option<&'a str> {
    values
        .iter()
        .find(|value| value.name() == name)
        .map(automata_ci_actions_runtime::NameValueCommand::value)
}
...
fn completed_step_boundary_delays_environment_and_path_but_scopes_outputs() {
    ...
    let applied = ActionsCompletedStepApplicator::default()
...
fn action_state_is_visible_only_to_the_exact_paired_post_action() {
    ...
    let next = ActionsCompletedStepApplicator::default()
        .apply_completed_step(
            &JobCommandState::new(CommandFilePlatform::Unix),
            &scope,
            &files,
...
fn action_post_preserves_main_outputs_and_merges_exact_updates() {
    ...
    let initial = JobCommandState::new(CommandFilePlatform::Unix);
    let main = ActionsCompletedStepApplicator::default()
        .apply_completed_step(
            &initial,
            &StepScope::new(step_id.clone(), StepPhase::ActionMain(invocation.clone())),
    ...
    let empty_post = ActionsCompletedStepApplicator::default()
        .apply_completed_step(
            &main,
    ...
    let updated_post = ActionsCompletedStepApplicator::default()
        .apply_completed_step(
            &main,
...
fn environment_key_semantics_follow_target_platform() {
    ...
    let applicator = ActionsCompletedStepApplicator::default();
