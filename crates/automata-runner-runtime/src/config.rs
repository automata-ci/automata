...
use automata_core::RunnerCapabilities;
use automata_protocol::ProtocolLimits;
use automata_runner_journal::clamp_registration_slots;
use thiserror::Error;

/// Maximum exponential-backoff ramp length for one immutable prepared request.
pub const MAX_EXCHANGE_ATTEMPTS: u16 = 64;
/// Largest individual retry delay accepted by the runtime.
pub const MAX_RETRY_DELAY: Duration = Duration::from_mins(5);
/// Largest server-directed idle delay accepted by the runtime.
pub const MAX_IDLE_DELAY: Duration = Duration::from_mins(5);
/// Largest cancellation grace period accepted by the runtime.
pub const MAX_CANCELLATION_GRACE: Duration = Duration::from_mins(10);
