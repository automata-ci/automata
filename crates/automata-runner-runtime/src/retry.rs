...
use crate::{
    RetryPolicy, RunnerRuntimeControlClient, RunnerRuntimeError, RuntimeControlErrorKind,
    RuntimeControlReply, RuntimeControlRetry, RuntimeSleeper,
};
    ...
    loop {
        ...
        match client.exchange(prepared, cancellation.clone()).await {
            ...
            Err(error) if error.retry() == RuntimeControlRetry::SamePreparedRequest => {
                ...
                sleeper
                    .sleep(
                        policy.jittered_delay_after(ramp_step, entropy),
                        cancellation.clone(),
                    )
                    .await;
                if cancellation.is_cancelled() {
