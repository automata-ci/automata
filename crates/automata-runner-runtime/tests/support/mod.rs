...
use automata_core::{
    ...
    IsolationLevel, JobConclusion, JobId, JobIr, JobIrEnvelope, JobLifecycle, JobResult, JobSource,
    Lease, LeaseId, LogAck, LogChannel, LogFrame, OperationId, RunId, RunnerCapabilities, RunnerId,
    RunnerPlatform, RunnerRequirements, SandboxCapabilities, SemanticStep, Sha256Digest, ShellSpec,
...
use automata_protocol_protobuf::{encode_job_ir, encode_runtime_authorities};
...
/// Holds the first non-empty cumulative log publication until a test releases
...
fn sandbox_environment_for(attestation: EnvironmentProfile) -> SandboxEnvironment {
    SandboxEnvironment::new(
        ...
        ImmutableImage::new(format!(
            "ghcr.io/goneuralai/automata-runner@sha256:{}",
            "a5".repeat(32)
        ))
...
pub fn capabilities(runner_id: RunnerId, slots: u16) -> RunnerCapabilities {
    RunnerCapabilities::new(
        runner_id,
        RunnerPlatform::new(automata_core::OperatingSystem::Linux, Architecture::X86_64),
    )
...
pub fn downgrade_journal_v4_to_v3(root: &StateRoot) {
    let path = root.as_path().join("runner-journal.json");
    let current = fs::read_to_string(&path).expect("current v4 journal");
    let legacy = current
        .replacen("\"schema_version\":4", "\"schema_version\":3", 1)
        .replacen("\"selected_protocol\":4", "\"selected_protocol\":3", 1)
        .replacen(
            ",\"lease_poll_requires_fresh_session\":false,\"lease_poll_checkpoints\":[]",
            "",
            1,
        );
    assert_ne!(legacy, current, "v3 fixture transformation must apply");
    assert!(!legacy.contains("lease_poll_checkpoints"));
    fs::write(path, legacy).expect("write canonical v3 journal fixture");
}
...
pub struct ObservedRequest {
    ...
    pub address: usize,
    pub is_hello: bool,
    pub resume: Option<automata_protocol::SessionResume>,
}
...
pub struct ScriptedControlClient {
    ...
    limits: automata_protocol::ProtocolLimits,
...
impl RecoveringPollClient {
    pub fn new(
        ...
        Self {
            ...
            limits: automata_protocol::ProtocolLimits::default(),
...
pub struct ResumeFallbackClient {
    stale_session_id: automata_core::RunnerSessionId,
    fresh_session_id: automata_core::RunnerSessionId,
...
impl ResumeFallbackClient {
    ...
    fn rejection(
        hello: &automata_protocol::RunnerHello,
        ...
        Box::pin(async move {
            let response = match request.message() {
                ...
                RunnerToServer::LeaseResponse(response)
                    if response.disposition() == &automata_protocol::LeaseDisposition::Accepted =>
                {
...
impl AcceptanceClient {
    pub fn new(session_id: automata_core::RunnerSessionId, shutdown: CancellationToken) -> Self {
        Self {
            session_id,
            shutdown,
            limits: automata_protocol::ProtocolLimits::default(),
...
pub struct RejectionClient {
    ...
    shutdown: CancellationToken,
    limits: automata_protocol::ProtocolLimits,
    rejection: Mutex<Option<automata_protocol::LeaseRejectionReason>>,
}
...
impl RejectionClient {
    ...
    pub fn rejection(&self) -> Option<automata_protocol::LeaseRejectionReason> {
        self.rejection
            .lock()
            .unwrap_or_else(PoisonError::into_inner)
            .clone()
    ...
    fn exchange<'a>(
        ...
        Box::pin(async move {
            let response = match request.message() {
                ...
                RunnerToServer::LeaseResponse(response) => {
                    let automata_protocol::LeaseDisposition::Rejected(reason) =
...
impl FailureIsolationClient {
    pub fn new(session_id: automata_core::RunnerSessionId) -> Self {
        Self {
            session_id,
            limits: automata_protocol::ProtocolLimits::default(),
        ...
        Box::pin(async move {
            let response = match request.message() {
                ...
                RunnerToServer::LogBatch(batch) => {
                    ...
                    ServerToRunner::LogAck(LogAckMessage::new(
                        reply_header(batch.header()),
                        automata_core::LogAck::new(last.stream_id(), Some(last.sequence())),
                    ))
                }
                RunnerToServer::JobResult(result) => {
                    self.terminal_results
...
impl DisconnectedHeartbeatClient {
    pub fn new(session_id: automata_core::RunnerSessionId, clock: Arc<ManualClock>) -> Self {
        Self {
            session_id,
            clock,
            limits: automata_protocol::ProtocolLimits::default(),
...
struct SlowLogState {
    log_batches: Vec<LogBatchObservation>,
    heartbeats: Vec<(JobLifecycle, u64)>,
...
impl SlowLogClient {
    ...
    pub fn heartbeats(&self) -> Vec<(JobLifecycle, u64)> {
        self.state
            .lock()
            .unwrap_or_else(PoisonError::into_inner)
            .heartbeats
        ...
        Box::pin(async move {
            let response = match request.message() {
                ...
                RunnerToServer::Heartbeat(heartbeat) => {
                    ...
                    state.heartbeats.push((heartbeat.lifecycle(), now));
                    ServerToRunner::LeaseRenewal(LeaseRenewal::new(
                        reply_header(heartbeat.header()),
                        self.lease.attempt_id(),
                        self.lease.guard(),
...
pub struct AcceptedFixture {
    pub session_id: automata_core::RunnerSessionId,
...
fn seed_recorded_offer_expiring_at(
    ...
    let session_id = automata_core::RunnerSessionId::new();
...
fn record_test_offer(
    ...
    let limits = automata_protocol::ProtocolLimits::default();
...
pub fn assert_session(journal: &dyn RunnerJournal, expected: automata_core::RunnerSessionId) {
    assert_eq!(
        journal
            .snapshot()
