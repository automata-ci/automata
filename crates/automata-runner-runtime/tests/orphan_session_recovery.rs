...
use automata_runner_journal::{
    LogProductionRecord, ProviderFailureKind, ProviderFailureOutcome, ProviderName,
...
impl OrphanControlClient {
    ...
    fn reject(&self, hello: &automata_protocol::RunnerHello) -> ServerToRunner {
        ...
        Box::pin(async move {
            let response = match request.message() {
                RunnerToServer::Hello(hello) => {
                    ...
                    if call == 0 {
                        ...
                        ServerToRunner::Hello(ServerHello::new(
                            ...
                            ServerTiming::new(
                                automata_core::UnixMillis::new(10_000),
                                1_000,
                                30_000,
                            ),
                        ))
                ...
                RunnerToServer::LeaseRequest(request) => {
                    ...
                    ServerToRunner::NoWork(automata_protocol::NoWork::new(
                        automata_protocol::MessageHeader::reply(
    ...
    let produced = spool
        ...
        .commit_with(|content| {
            journal.record_log_produced(
                ...
                fixture.slot,
                guard,
                LogProductionRecord::new(stream, LogSequence::new(0), true, content.clone())?,
            )
        });
...
fn runtime(
    ...
    spool: Arc<automata_runner_spool::FileSpool>,
    ...
    RunnerSessionSupervisor::new(
        support::config_with_slots_and_retry(
            runner_id,
            2,
            automata_runner_runtime::RetryPolicy::default(),
        ),
...
#[tokio::test]
async fn migrated_v3_active_slot_requires_orphan_authority_before_fresh_v4_session() {
    let scratch = support::Scratch::new("orphan-v3-active-recovery");
    let runner_id = RunnerId::new();
    let old_session;
    {
        let (journal, spool) = support::durable_ports(&scratch, runner_id);
    ...
    support::downgrade_journal_v4_to_v3(&scratch.journal_root());
    let (journal, spool) = support::durable_ports(&scratch, runner_id);
    let migrated = journal.snapshot().expect("migrated active snapshot");
    assert!(
        migrated
            .session()
    ...
    assert_eq!(
        migrated.slots().len(),
        1,
        "active v3 slot remains recoverable"
    );
    ...
    runtime(runner_id, client.clone(), journal.clone(), spool, executor)
        .run(shutdown)
        .await
        .expect("authorized v3 orphan cleanup then fresh v4 session");

    assert_eq!(client.hello_calls(), 2);
    let recovered = journal.snapshot().expect("fresh recovered snapshot");
    assert!(recovered.slots().is_empty());
    let session = recovered.session().expect("fresh session");
    assert_eq!(session.session_id(), fresh_session);
