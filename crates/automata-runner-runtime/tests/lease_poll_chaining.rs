...
impl NoWorkClient {
    ...
    fn hello(&self, hello: &automata_protocol::RunnerHello) -> ServerToRunner {
...
fn reply(message: ServerToRunner) -> Result<RuntimeControlReply, RuntimeControlError> {
    RuntimeControlReply::from_message(message, &automata_protocol::ProtocolLimits::default())
...
#[tokio::test]
async fn migrated_v3_empty_session_opens_exactly_one_fresh_v4_session() {
    let scratch = support::Scratch::new("lease-poll-v3-empty-runtime");
    let runner_id = RunnerId::new();
    let legacy_session = RunnerSessionId::new();
    ...
    let migrated = Arc::new(
        FileJournal::open(scratch.journal_root(), runner_id).expect("migrate empty v3 journal"),
    );
    assert!(
        migrated
            .snapshot()
    ...
    runtime(runner_id, 1, client.clone(), migrated.clone(), spool)
        .run(shutdown)
        .await
        .expect("non-resumable empty v3 session opens fresh v4");

    assert_eq!(client.hello_calls(), 2);
    let snapshot = migrated.snapshot().expect("fresh snapshot");
    let session = snapshot.session().expect("fresh session");
