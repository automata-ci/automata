...
use automata_core::{AttemptId, JobConclusion, RunnerId};
use automata_runner_journal::{FileJournal, RunnerJournal};
use automata_runner_runtime::{
    RetryPolicy, RunnerRuntimePorts, RunnerSessionSupervisor, SystemRuntimeIds,
};
use automata_runner_spool::{DurableContentStore, FileSpool, FileSpoolOptions, SpoolLimits};
...
async fn capacity_reclamation_terminalizes_missing_eos_without_stopping_sibling() {
    ...
    // Even without envelopes, framing, protection, the empty stream, or job
    // content, retaining every immutable cumulative version needs more than
    // this spool permits. Completing the job therefore exercises midstream
    // reclamation rather than relying only on terminal slot cleanup.
    let minimum_cumulative_history = FRAME_PAYLOAD_BYTES
        * u64::try_from(DATA_FRAMES).expect("frame count")
        * u64::try_from(DATA_FRAMES + 1).expect("frame count plus one")
        / 2;
    assert!(minimum_cumulative_history > MAX_TOTAL_BYTES);
