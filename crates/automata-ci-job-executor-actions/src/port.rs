...
use automata_ci_actions_runtime::JobCommandState;
...
pub enum ActionsExecutionPhase {
...
#[derive(Clone, Copy, Debug)]
pub struct ActionsExecutionIdentity<'a> {
    job: &'a JobIrEnvelope,
...
impl<'a> ActionsExecutionIdentity<'a> {
...
pub struct GithubContextRequest<'a> {
    identity: ActionsExecutionIdentity<'a>,
    event_path: &'a TargetPath,
    event: &'a GithubValue,
    commands: &'a JobCommandState,
    steps: &'a [ActionsStepSnapshot],
...
impl<'a> GithubContextRequest<'a> {
    ...
    #[must_use]
    pub const fn steps(self) -> &'a [ActionsStepSnapshot] {
        self.steps
    }
    ...
    #[must_use]
    pub const fn phase(self) -> ActionsExecutionPhase {
        self.phase
    }
}
...
pub struct ActionsStepSnapshot {
    id: String,
    outcome: JobConclusion,
    conclusion: JobConclusion,
}

impl ActionsStepSnapshot {
...
pub trait ActionsToolchain: fmt::Debug + Send + Sync {
