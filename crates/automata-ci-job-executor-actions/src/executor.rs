...
use automata_ci_actions_runtime::{
    ActionInvocationId, ActionsCommandFileDecoder, ActionsCompletedStepApplicator,
    ArtifactDeclaration, ArtifactDeclarationCommandFile, ArtifactSubject,
    ...
    JobCommandState, MAX_ARTIFACT_DECLARATION_FILE_BYTES, MAX_ARTIFACT_SUBJECTS, ParsedCommandFile,
    PathCommandFile, PhaseApplicationError, PhaseApplicationNotice, StateCommandFile,
    StepId as RuntimeStepId, StepPhase, StepScope, StepSummaryCommandFile, WorkflowCommandLimits,
...
use crate::{
    ...
    ActionsExecutionIdentity, ActionsExecutionPhase, ActionsJobExecutorConfig, ActionsStepSnapshot,
    ActionsToolchain, CheckedOutLocalActionPreparer, ExecutionClock, ExecutionOperationIds,
    GithubContextPort, GithubContextRequest, JobContentPort, LocalActionPreparationRequest,
...
/// Object-safe dependencies composed into an Actions-compatible job executor.
pub struct ActionsJobExecutorPorts {
...
impl fmt::Debug for ActionsJobExecutorPorts {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("ActionsJobExecutorPorts")
...
/// Generic Actions-compatible job executor over one pluggable sandbox provider.
pub struct ActionsJobExecutor {
    config: ActionsJobExecutorConfig,
    ports: ActionsJobExecutorPorts,
    local_actions: CheckedOutLocalActionPreparer,
    expressions: GithubExpressionEvaluator,
    command_files: ActionsCommandFileDecoder,
    completed_steps: ActionsCompletedStepApplicator,
...
impl ActionsJobExecutor {
    /// Creates an executor pinned to the reviewed Actions compatibility engines.
    #[must_use]
    pub fn new(config: ActionsJobExecutorConfig, ports: ActionsJobExecutorPorts) -> Self {
        Self {
            config,
            ports,
            ...
            command_files: ActionsCommandFileDecoder::default(),
            completed_steps: ActionsCompletedStepApplicator::default(),
    ...
    pub fn with_secret_custody(
        ...
        Self {
            ...
            ports: ActionsJobExecutorPorts::new(
    ...
    async fn execute_job(
        ...
        for (index, step) in executable_steps.iter().enumerate() {
            ...
            let phase = match step.kind() {
                SemanticStep::Run { .. } => ActionsExecutionPhase::Run,
                SemanticStep::Action { .. } => ActionsExecutionPhase::ActionMain,
            };
                ...
                PreparedActionExecution::Composite(_) => {
                    ...
                    let context = cancellation_dominant(
                        self.context(
                            ...
                            *status,
                            Some(step.id().as_str()),
                            ActionsExecutionPhase::ActionPre,
                        ),
                        cancellation,
                    )?;
            ...
            let result = async {
                ...
                let base = cancellation_dominant(
                    self.context(
                        ...
                        status,
                        Some(top_step.id().as_str()),
                        ActionsExecutionPhase::ActionMain,
                    ),
                    cancellation,
                )?;
                for secret in base.secret_masks() {
    ...
    fn publish_action_outputs(
        ...
        let next = cancellation_dominant(
            self.completed_steps
                ...
                .map(automata_ci_actions_runtime::PhaseApplication::into_next_state)
            ...
            let Some(context) = reconcile_post_operation(
                self.context(
                    ...
                    *status,
                    ...
                    ActionsExecutionPhase::ActionPost,
                ),
                cancellation,
                status,
                conclusion,
...
impl fmt::Debug for ActionsJobExecutor {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("ActionsJobExecutor")
...
impl JobExecutor for ActionsJobExecutor {
...
impl ExecutionAttachments {
    fn record_phase(
        ...
        step_id: &str,
        summary: &str,
        annotations: &[automata_ci_actions_runtime::Annotation],
        ...
        for annotation in annotations {
            ...
            retained.annotations.push(StepAnnotation::new(
                match annotation.level() {
                    automata_ci_actions_runtime::AnnotationLevel::Error => {
                        StepAnnotationLevel::Error
                    }
                    automata_ci_actions_runtime::AnnotationLevel::Warning => {
                        StepAnnotationLevel::Warning
                    }
                    automata_ci_actions_runtime::AnnotationLevel::Notice => {
        ...
        for notice in notices {
            let message = match notice {
                ...
                PhaseApplicationNotice::BlockedReservedEnvironment(namespace) => match namespace {
                    automata_ci_actions_runtime::ReservedEnvironmentNamespace::Github => {
                        "a runner-owned GITHUB default from a command file was ignored"
                    }
                    automata_ci_actions_runtime::ReservedEnvironmentNamespace::Runner => {
...
impl MutableStepResult {
    ...
    fn snapshot(&self) -> ActionsStepSnapshot {
        ActionsStepSnapshot::new(self.step_id.as_str(), self.outcome, self.conclusion)
    }
...
fn validate_action_step_admission(
    job: &JobIrEnvelope,
    workspace: &TargetPath,
    toolchain: &dyn ActionsToolchain,
) -> Result<(), AdmissionRejection> {
...
fn valid_toolchain(toolchain: &dyn ActionsToolchain) -> bool {
...
mod tests {
    ...
    fn composite_action_outputs_round_trip_scalar_and_multiline_values() {
        ...
        let ParsedCommandFile::Output(outputs) = ActionsCommandFileDecoder::default()
