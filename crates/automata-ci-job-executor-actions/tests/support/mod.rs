...
use automata_ci_job_executor_actions::{
    ...
    ActionsJobExecutor, ActionsJobExecutorConfig, ActionsJobExecutorPorts,
    ContextEnvironmentVariable, DeterministicOperationIds, ExecutionClock, GithubContextPort,
    GithubContextRequest, GithubContextSnapshot, ImmutableSandboxEnvironmentCatalog,
...
pub struct Fixture {
    pub executor: ActionsJobExecutor,
...
impl Fixture {
    ...
    fn with_platform_components_and_timeout_and_node_and_runtime_service_proxy(
        ...
        let toolchain = match platform {
            TargetPlatform::Posix => {
                let toolchain = StaticActionsToolchain::new(
                    target("/usr/bin/bash"),
                    target("/usr/bin/sh"),
            ...
            TargetPlatform::Windows => {
                let toolchain = StaticActionsToolchain::windows(
        ...
        let ports = ActionsJobExecutorPorts::new(
            provider.clone(),
            catalog,
        ...
        let config = ActionsJobExecutorConfig::new(
        ...
        .expect("valid config");
        Self {
            executor: ActionsJobExecutor::new(config, ports),
            provider,
            endpoint_state,
        ...
        self.actions
            ...
            .ok_or_else(|| {
                ActionPreparationError::new(
                    automata_ci_job_executor_actions::ActionPreparationErrorKind::Resolution,
                )
            })
    }
}
...
impl GithubContextPort for CountingActionMainContexts {
    fn snapshot(
        ...
        if request.phase() == ActionsExecutionPhase::ActionMain {
...
impl GithubContextPort for CancellingMainContexts {
    fn snapshot(
        ...
        if matches!(
            request.phase(),
            ActionsExecutionPhase::Run | ActionsExecutionPhase::ActionMain
        ) {
            self.cancellation
...
impl GithubContextPort for CancellingFinalContexts {
    fn snapshot(
        ...
        let final_context = request.phase() == ActionsExecutionPhase::Job
...
impl GithubContextPort for CancellingPostContexts {
    fn snapshot(
        ...
        if request.phase() != ActionsExecutionPhase::ActionPost {
...
fn phase_context_values(phase: ActionsExecutionPhase) -> [(String, GithubValue); 3] {
    let name = match phase {
        ActionsExecutionPhase::Job => "job",
        ActionsExecutionPhase::Run => "run",
        ActionsExecutionPhase::ActionPre => "action_pre",
        ActionsExecutionPhase::ActionMain => "action_main",
        ActionsExecutionPhase::ActionPost => "action_post",
    };
    let post = phase == ActionsExecutionPhase::ActionPost;
    [
