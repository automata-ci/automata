...
impl OrphanClaim {
    ...
    /// Returns the runner identity bound into the authority decision.
    #[must_use]
    pub const fn runner_id(self) -> RunnerId {
        self.runner_id
    }

    /// Returns the old session whose lease is being reconciled.
    #[must_use]
    pub const fn session_id(self) -> RunnerSessionId {
        self.session_id
    }

    /// Returns the stable slot containing the old-session lease.
    #[must_use]
    pub const fn slot(self) -> RunnerSlotOrdinal {
        self.slot
    }

    /// Returns the exact attempt fence that authority must invalidate.
    #[must_use]
...
impl OrphanAbandonmentPermissions {
    /// Creates the explicit per-delivery abandonment permission set.
    #[must_use]
    ...
    /// Reports whether terminal-result abandonment is authorized.
    #[must_use]
    pub const fn terminal_result(self) -> bool {
        self.terminal_result
    }

    /// Reports whether log-stream abandonment is authorized.
    #[must_use]
    pub const fn log_delivery(self) -> bool {
        self.log_delivery
    }

    /// Reports whether rejected-offer-response abandonment is authorized.
    #[must_use]
...
impl OrphanAuthorityGrant {
    ...
    /// Returns the exact fence verified by the authority adapter.
    #[must_use]
    pub const fn claim(self) -> OrphanClaim {
        self.claim
    }

    /// Returns the idempotency identity of the authority decision.
    #[must_use]
    pub const fn authority_operation_id(self) -> OperationId {
        self.authority_operation_id
    }

    /// Returns non-secret evidence identifying the verified proof.
    #[must_use]
    pub const fn evidence_digest(self) -> Sha256Digest {
        self.evidence_digest
    }

    /// Returns the delivery-specific abandonment permissions.
    #[must_use]
...
pub enum OrphanAuthorityError {
    /// Proof bytes were empty, oversized, malformed, or failed verification.
    #[error("orphan authority proof is invalid")]
    InvalidProof,
    /// The authenticated control plane did not authorize this exact claim.
    #[error("control plane denied orphan reconciliation")]
    Denied,
    /// Authority could not be established because the verifier was unavailable.
...
pub enum OrphanDelivery {
    /// The terminal-result outbox item.
    TerminalResult,
    /// The remaining durable log stream.
    LogStream,
    /// The rejected-offer response awaiting acknowledgement.
    LeaseRejection,
}
...
pub enum OrphanAbandonmentReason {
    /// The control plane invalidated the old runner session.
    SessionInvalidated,
    /// The control plane permanently rejected the delivery.
...
impl OrphanRecord {
    ...
    /// Returns the old session bound into the durable authority decision.
    #[must_use]
    pub const fn session_id(self) -> RunnerSessionId {
        self.session_id
    }

    /// Returns the exact old attempt fence.
    #[must_use]
    pub const fn guard(self) -> LeaseGuard {
        self.guard
    }

    /// Returns the authority decision's idempotency identity.
    #[must_use]
    pub const fn authority_operation_id(self) -> OperationId {
        self.authority_operation_id
    }

    /// Returns the non-secret digest of the verified evidence.
    #[must_use]
    pub const fn evidence_digest(self) -> Sha256Digest {
        self.evidence_digest
    }

    /// Returns the delivery-specific permissions granted by the authority.
    #[must_use]
    pub const fn permissions(self) -> OrphanAbandonmentPermissions {
        self.permissions
    }

    /// Returns the durable abandonment reason for `delivery`, if recorded.
    #[must_use]
