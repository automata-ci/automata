...
impl AdmissionIdGenerator for Sha256AdmissionIdGenerator {
    ...
    fn logical_invocation_id(&self, run_id: RunId) -> LogicalWorkflowInvocationId {
        LogicalWorkflowInvocationId::from_uuid(derived_uuid(
            b"automata.admission.logical-invocation.v1\0",
            &[run_id.as_uuid().as_bytes()],
        ))
        .expect("UUIDv8 derivation is necessarily non-nil")
    }

    fn logical_job_id(&self, run_id: RunId, job_key: &WorkflowJobKey) -> LogicalWorkflowJobId {
        LogicalWorkflowJobId::from_uuid(derived_uuid(
            b"automata.admission.logical-job.v1\0",
