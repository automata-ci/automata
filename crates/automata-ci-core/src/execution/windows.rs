...
pub const WINDOWS_HYPERV_BROKER_GRANT_SCHEMA_V3: u16 = 3;
...
const SIGNING_DOMAIN: &[u8] = b"automata.windows-hyperv-broker-grant.v3\0";
const DIGEST_DOMAIN: &[u8] = b"automata.windows-hyperv-broker-grant-digest.v3\0";
const SANDBOX_SPEC_DOMAIN: &[u8] = b"automata.windows-hyperv-sandbox-spec.v3\0";
...
impl WindowsHyperVBrokerGrantClaims {
    ...
    pub fn validate(&self) -> Result<(), WindowsHyperVBrokerGrantError> {
        ...
        if self.sealed_action_policy_sha256 != windows_action_archive_policy_sha256() {
            return Err(WindowsHyperVBrokerGrantError::SealedActionPolicyMismatch);
        }
    ...
    /// Returns the exact sealed-action namespace policy admitted for this host.
    #[must_use]
    pub const fn sealed_action_policy_sha256(&self) -> Sha256Digest {
        self.sealed_action_policy_sha256
    }

    /// Returns the exact admitted Windows action graph, when the job has one.
    #[must_use]
    ...
    pub fn authorizes_sandbox_spec(
        ...
        profile_contract_sha256 == self.profile_contract_sha256
            && operation_id == self.post_accept_operation_id
            && generation == self.fencing_token.get()
            && allocation == self.job_resource_allocation
            && pids_limit == self.sandbox_pids_limit
            && network_disabled
            && sealed_action_policy_sha256 == self.sealed_action_policy_sha256
            && windows_action_graph_sha256 == self.windows_action_graph_sha256
    ...
    fn append_signing_fields(&self, bytes: &mut Vec<u8>) {
        ...
        field(bytes, self.sealed_action_policy_sha256.as_bytes());
        optional_digest_field(bytes, self.windows_action_graph_sha256);
    ...
    fn compute_sandbox_spec_sha256(&self) -> Sha256Digest {
        ...
        field(&mut bytes, self.sealed_action_policy_sha256.as_bytes());
        optional_digest_field(&mut bytes, self.windows_action_graph_sha256);
        field(
            &mut bytes,
            self.post_accept_operation_id.as_uuid().as_bytes(),
        );
...
impl WindowsHyperVBrokerGrant {
    ...
    pub fn new(
        ...
        Self::from_parts(
            WINDOWS_HYPERV_BROKER_GRANT_SCHEMA_V3,
            key_id,
            claims,
            signature,
        )
    ...
    pub fn from_parts(
        ...
        if schema != WINDOWS_HYPERV_BROKER_GRANT_SCHEMA_V3 {
    ...
    pub fn signing_bytes_for(
        ...
        field(
            &mut bytes,
            &WINDOWS_HYPERV_BROKER_GRANT_SCHEMA_V3.to_be_bytes(),
        );
...
fn optional_digest_field(bytes: &mut Vec<u8>, digest: Option<Sha256Digest>) {
    match digest {
        Some(digest) => {
            field(bytes, &[1]);
            field(bytes, digest.as_bytes());
        }
        None => field(bytes, &[0]),
    }
}
...
pub enum WindowsHyperVBrokerGrantError {
    ...
    /// The signed sealed-action namespace policy differs from this build.
    #[error("Windows Hyper-V broker sealed-action namespace policy mismatch")]
    SealedActionPolicyMismatch,
...
mod tests {
    ...
    fn claims(windows_action_graph_sha256: Option<Sha256Digest>) -> WindowsHyperVBrokerGrantClaims {
    ...
    #[test]
    fn v3_signing_and_sandbox_binding_commit_to_exact_optional_action_graph() {
        let key_id = Sha256Digest::from_bytes([9; 32]);
        let absent = claims(None);
        let exact_digest = Sha256Digest::from_bytes([0x41; 32]);
        let mut present = absent.clone();
        present.windows_action_graph_sha256 = Some(exact_digest);
        present.sandbox_spec_sha256 = present.compute_sandbox_spec_sha256();
        let mut substituted = present.clone();
        substituted.windows_action_graph_sha256 = Some(Sha256Digest::from_bytes([0x42; 32]));
        substituted.sandbox_spec_sha256 = substituted.compute_sandbox_spec_sha256();

        assert_ne!(absent.sandbox_spec_sha256(), present.sandbox_spec_sha256());
        assert_ne!(
            present.sandbox_spec_sha256(),
            substituted.sandbox_spec_sha256()
        );
        assert_ne!(
            WindowsHyperVBrokerGrant::signing_bytes_for(key_id, &absent),
            WindowsHyperVBrokerGrant::signing_bytes_for(key_id, &present)
        );
        assert_ne!(
            WindowsHyperVBrokerGrant::signing_bytes_for(key_id, &present),
            WindowsHyperVBrokerGrant::signing_bytes_for(key_id, &substituted)
        );
        assert!(present.authorizes_sandbox_spec(
            present.profile_contract_sha256(),
            present.post_accept_operation_id(),
            present.fencing_token().get(),
            present.job_resource_allocation(),
            present.sandbox_pids_limit(),
            true,
            present.sealed_action_policy_sha256(),
            Some(exact_digest),
        ));
        assert!(!present.authorizes_sandbox_spec(
            present.profile_contract_sha256(),
