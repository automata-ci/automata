...
pub enum EnvironmentProfileError {
    /// No profile identity was provided.
    #[error("environment profile identity cannot be empty")]
    Empty,
    /// The encoded profile identity exceeds the bounded wire limit.
    #[error("environment profile identity exceeds {maximum} bytes")]
    TooLong {
        /// Maximum accepted UTF-8 byte length.
        maximum: usize,
    },
    /// The identity contains characters outside canonical ASCII.
    #[error("environment profile identity must contain only ASCII characters")]
    NonAscii,
    /// The identity omits its provider namespace separator.
    #[error("environment profile identity must be namespaced as `<namespace>/<name>`")]
    MissingNamespace,
    /// The provider namespace does not use canonical lower-case components.
    #[error("environment profile namespace must be canonical provider text")]
    InvalidNamespace,
    /// The profile name does not use canonical lower-case components.
