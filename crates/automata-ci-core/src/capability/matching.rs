...
pub enum RequirementMismatch {
    /// The runner advertisement uses an unsupported schema.
    CapabilitiesSchemaVersion {
        /// Schema understood by this scheduler.
        supported: u16,
        /// Schema carried by the advertisement.
        received: u16,
    },
    /// The job requirements use an unsupported schema.
    RequirementsSchemaVersion {
        /// Requirements schema understood by this scheduler.
        supported: u16,
        /// Schema carried by the requirements value.
        received: u16,
    },
    /// One required all-of runner label is absent.
    MissingLabel(RunnerLabel),
    /// The runner belongs to none of the eligible administrative groups.
    NoEligibleGroup {
        /// Groups of which at least one was required.
        eligible: BTreeSet<RunnerGroup>,
    },
    /// The runner exposes a different operating-system family.
    OperatingSystem {
        /// Operating-system family required by the job.
        required: OperatingSystem,
        /// Operating-system family exposed by the runner.
        available: OperatingSystem,
    },
    /// The runner exposes a different CPU architecture.
    Architecture {
        /// CPU architecture required by the job.
        required: Architecture,
        /// CPU architecture exposed by the runner.
        available: Architecture,
    },
    /// The advertisement contains no usable concurrent-job slot.
    NoJobSlots,
    /// One quantitative per-job capacity is below the requested minimum.
    InsufficientResource {
        /// Resource dimension that is insufficient.
        resource: ResourceKind,
        /// Minimum requested quantity, in the resource's documented unit.
        required: u64,
        /// Advertised quantity, in the same unit.
        available: u64,
    },
    /// The strongest available sandbox boundary is weaker than required.
    Isolation {
        /// Weakest acceptable isolation boundary.
        required: IsolationLevel,
        /// Strongest boundary advertised by the runner.
        available: IsolationLevel,
    },
    /// One required sandbox feature is absent.
    MissingSandboxFeature(SandboxFeature),
    /// One required container-runtime feature is absent.
    MissingContainerFeature(ContainerFeature),
    /// One required workflow-runtime feature is absent.
    MissingRunnerFeature(RunnerFeature),
    /// The runner does not supply the exact required profile identity and digest.
    EnvironmentProfile {
        /// Exact content-attested environment required by the job.
        required: EnvironmentProfile,
        /// Exact content-attested environments advertised by the runner.
...
pub enum ResourceKind {
    /// CPU quantity measured in thousandths of one logical CPU.
    CpuMillis,
    /// Memory quantity measured in bytes.
    MemoryBytes,
    /// Ephemeral-storage quantity measured in bytes.
    EphemeralDiskBytes,
    /// GPU quantity measured in whole devices.
    GpuCount,
}
...
impl RequirementMismatches {
    ...
    /// Borrows the complete mismatch list in deterministic evaluation order.
    #[must_use]
    ...
    /// Returns whether every requirement was satisfied.
    #[must_use]
    pub const fn is_empty(&self) -> bool {
        self.0.is_empty()
    }

    /// Iterates over mismatches in deterministic evaluation order.
    pub fn iter(&self) -> slice::Iter<'_, RequirementMismatch> {
        self.0.iter()
    }

    /// Consumes the collection and returns its ordered mismatch values.
    #[must_use]
