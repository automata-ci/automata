...
impl Tree {
    ...
    fn evaluate_join(
        ...
        Ok(Evaluated::plain(functions::join(&values)?))
    }
    ...
    fn evaluate_format(
        ...
        let mut cache: Vec<Option<String>> = vec![None; value_nodes.len()];
        let value = functions::format_template(&template, value_nodes.len(), |index| {
            if let Some(value) = &cache[index] {
                return Ok(value.clone());
            }
            let value =
                coercion::to_string(&self.evaluate(value_nodes[index], context, limits)?.value);
