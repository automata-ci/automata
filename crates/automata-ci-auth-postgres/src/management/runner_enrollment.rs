...
const MAX_CERTIFICATE_LIFETIME_SECONDS: i64 = 30 * 24 * 60 * 60;
...
pub struct PreparedRunnerEnrollment {
    ...
    pub expires_at_ms: i64,
    /// Database-clock observation used to bind certificate validity to storage time.
...
pub struct ConsumeRunnerEnrollment {
    ...
    /// Complete validated capability document; duplicated projections are derived internally.
...
impl PostgresHumanRbacManagementRepository {
    ...
    pub async fn prepare_runner_enrollment(
        ...
        commit(transaction).await?;
        row.map(|row| row.prepared(now_ms))
            .transpose()?
    ...
    pub async fn consume_runner_enrollment(
        ...
        if request.certificate_expires_at_seconds <= now_seconds
            || request.certificate_expires_at_seconds
                > now_seconds.saturating_add(MAX_CERTIFICATE_LIFETIME_SECONDS)
        {
            return Err(ManagementRepositoryError::InvalidRequest);
        }
        let prepared = row.prepared(now_ms)?;
        let expected_group =
            std::collections::BTreeSet::from([automata_ci_core::RunnerGroup::new(
