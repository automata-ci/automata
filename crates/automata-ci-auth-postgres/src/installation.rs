...
const DEPLOYMENT_INSTALLATION_AUTHORITY_DOMAIN: &[u8] =
    b"automata-ci/installation/deployment-authority/v1\0";
...
impl ConfigureDeploymentInstallation {
    ...
    ///
    /// `authority_source_sha256` is a source-layer SHA-256 digest; the source
    /// bytes themselves never cross this boundary. The constructor derives the
    /// persisted authority with an installation-specific domain separator.
    ///
    /// # Errors
    ///
    /// Rejects a zero source digest or nil operation identity. The tenant has
    /// already passed the shared installation-value validation boundary.
    pub fn new(
        authority_source_sha256: [u8; 32],
        bootstrap_operation_id: Uuid,
        tenant: InstallationTenant,
    ) -> Result<Self, DeploymentInstallationRequestError> {
        if authority_source_sha256 == [0; 32] || bootstrap_operation_id.is_nil() {
            return Err(DeploymentInstallationRequestError);
        }
        let mut digest = Sha256::new();
        digest.update(DEPLOYMENT_INSTALLATION_AUTHORITY_DOMAIN);
        digest.update(authority_source_sha256);
        let installation_authority_sha256: [u8; 32] = digest.finalize().into();
        if installation_authority_sha256 == [0; 32] {
            return Err(DeploymentInstallationRequestError);
        }
        Ok(Self {
...
impl InstallationRow {
    ...
    fn state(&self) -> Result<InstallationState, InstallationRepositoryError> {
        ...
        match (self.state.as_str(), self.configuration_mode.as_deref()) {
            ...
            ("configured", Some("deployment")) => self.configured_deployment_state(revision),
    ...
    fn configured_human_state(
        ...
        let login_transaction_id = self
            ...
            .ok_or(InstallationRepositoryError::CorruptData)?;
        Ok(InstallationState::HumanConfigured {
            revision,
            tenant_id,
            principal_id,
    ...
    fn configured_deployment_state(
        &self,
        revision: InstallationRevision,
    ) -> Result<InstallationState, InstallationRepositoryError> {
        if self.bootstrap_token_hash.is_some()
        ...
        Ok(InstallationState::DeploymentConfigured {
            revision,
            tenant_id: self.exact_configured_tenant_id()?,
            configured_at: self.configured_at()?,
        })
    }
...
impl InstallationRow {
    fn deployment_proof(
        ...
        let InstallationState::DeploymentConfigured {
            revision,
            tenant_id,
            ..
        } = self.state()?
...
pub(crate) async fn revalidate_configured_deployment_installation(
    ...
    if !matches!(row.state()?, InstallationState::DeploymentConfigured { .. })
        || !row.matches_deployment_proof(proof)?
    {
...
mod tests {
    ...
    #[test]
    fn deployment_request_domain_separates_and_redacts_source_digest() {
        ...
        assert_ne!(request.installation_authority_sha256, [1; 32]);
        assert_eq!(
