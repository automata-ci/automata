...
use automata_core::{AttemptId, FencingToken, JobId, JobIrEnvelope, Lease, RunId, UnixMillis};
...
impl JobRuntimeAuthorities {
    /// Creates a non-empty, canonical authority bundle.
    ///
    /// # Errors
    ///
    /// Rejects empty, oversized, duplicate, or unsorted authority names and
    /// any execution-binding mismatch.
    pub fn new(
        authorities: Vec<JobRuntimeAuthority>,
        job: &JobIrEnvelope,
        lease: &Lease,
    ...
    pub fn validate_for(
        ...
        if self.authorities.is_empty() || self.authorities.len() > MAX_RUNTIME_AUTHORITIES {
