...
use automata_protocol::{
    ...
    LeaseRequest, MessageHeader, MessageValidationError, NegotiatedSession, ProtocolDecodeError,
    ProtocolLimits, RunnerHello, RunnerSlotOrdinal, RunnerToServer, SUPPORTED_PROTOCOL_RANGE,
    ServerCommandHeader, ServerHello, ServerTiming, SessionDisposition, SessionResume,
    ValidatedRunnerToServer, decode_runner_frame,
};
...
#[test]
fn first_lease_poll_carries_chain_origin_for_one_authenticated_slot() {
    let header = MessageHeader::request(
        SUPPORTED_PROTOCOL_RANGE.max(),
        RunnerSessionId::new(),
        OperationId::new(),
    ...
    let encoded = serde_json::to_vec(&message).expect("encode lease poll");
    let value: serde_json::Value = serde_json::from_slice(&encoded).expect("decode JSON fixture");
    let payload = value["payload"].as_object().expect("lease request object");
    assert_eq!(payload.len(), 3);
    assert!(payload.contains_key("header"));
    assert_eq!(payload["slot"], serde_json::json!(2));
    assert_eq!(
        payload["acknowledges_operation_id"],
        serde_json::Value::Null
    );
    assert!(!payload.contains_key("runner_id"));
    assert!(!payload.contains_key("session_id"));

    let mut invalid = value;
    invalid["payload"]["slot"] = serde_json::json!(0);
    let invalid = serde_json::to_vec(&invalid).expect("encode invalid slot fixture");
    assert!(matches!(
        decode_runner_frame(&invalid, &ProtocolLimits::default()),
