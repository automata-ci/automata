...
use automata_ci_protocol::{
    ...
    RunnerToServer, SUPPORTED_PROTOCOL_RANGE, ServerToRunner,
};
...
#[test]
fn managed_secret_overlay_rejects_a_changed_lease_coordinate_before_digest_validation() {
    let mut frame = fixture_lease_offer_with_managed_overlay();
    lease_offer_payload(&mut frame)
        .managed_secret_bindings
        .as_mut()
        .expect("managed-secret overlay")
        .attempt_id = Uuid::from_u128(0xfeed).as_bytes().to_vec();
...
#[test]
fn managed_secret_overlay_rejects_noncanonical_order_independently_of_digest() {
    let mut frame = fixture_lease_offer_with_managed_overlay();
    lease_offer_payload(&mut frame)
        .managed_secret_bindings
        .as_mut()
