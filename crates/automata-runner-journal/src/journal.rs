...
use crate::{
    ...
    ProviderFailureOutcome, ProviderOperation, SandboxIdentity, SessionBinding,
...
pub trait RunnerJournal: Send + Sync {
    ...
    /// # Errors
    ///
    /// Returns [`JournalError`] for a stale or legacy-fenced session, an
    /// out-of-range slot, an operation-identity conflict, or failed commit.
    ...
    /// Persists the exact rejected-offer response before it is sent. The slot
    /// remains recoverable until its response operation is acknowledged.
    ///
    /// # Errors
    ///
    ...
    /// outbox operation/content reference. Result payload bytes must be
    /// committed and fsynced before this call.
    ///
    /// # Errors
    ///
    ...
    /// Advances the locally produced log cursor by exactly one frame and
    /// optionally seals the stream at that terminal frame.
    ///
    /// # Errors
    ///
    ...
    /// Advances the remotely acknowledged log cursor monotonically and never
    /// beyond locally produced data.
    ///
    /// # Errors
    ///
    /// Returns [`JournalError`] for regression, unknown data/stream, stale
    /// credentials, or I/O.
    fn acknowledge_log(
        &self,
        session_id: RunnerSessionId,
