...
use automata_core::{JobIrVersion, JobLifecycle, OperationId, RunnerSessionId};
use automata_protocol::{PROTOCOL_MAX_VERSION, ProtocolVersion, RunnerSlotOrdinal};
...
use support::{Fixture, Scratch, journal_file, record_and_ack_terminal};
...
) -> automata_runner_journal::LeasePollCheckpoint {
    *journal
        .snapshot()
        .expect("snapshot")
        .session()
    ...
    for (stage, committed) in [
        ...
        assert_eq!(
            recovered_checkpoint
                .map(automata_runner_journal::LeasePollCheckpoint::current_operation_id,),
            committed.then_some(operation_id)
        );
    }
}

fn downgrade_current_file_to_v3(scratch: &Scratch) {
    let path = journal_file(scratch.state_root().as_path());
    ...
    fs::write(path, legacy).expect("write canonical v3 fixture");
}

#[test]
fn v3_empty_session_migrates_then_requires_one_fresh_v4_session() {
    let scratch = Scratch::new("lease-poll-v3-empty");
    let fixture = Fixture::new();
    let journal = fixture.open(&scratch);
    journal.begin_session(fixture.binding()).expect("session");
    let old_revision = journal.snapshot().expect("snapshot").revision();
    drop(journal);
    downgrade_current_file_to_v3(&scratch);

    let migrated = fixture.open(&scratch);
    let snapshot = migrated.snapshot().expect("migrated snapshot");
    assert_eq!(snapshot.revision(), old_revision + 1);
    let legacy = snapshot.session().expect("legacy session");
    assert!(legacy.lease_poll_requires_fresh_session());
    assert!(legacy.lease_poll_checkpoints().is_empty());
    assert!(matches!(
        migrated.prepare_lease_poll(fixture.session_id, fixture.slot, OperationId::new()),
        Err(JournalError::Invariant(
            JournalInvariantError::LeasePollRecoveryRequired
        ))
    ));
    ...
    let fresh = migrated
        ...
        .expect("fresh v4 session");
    assert!(
        !fresh
            .session()
            .expect("fresh session")
            .lease_poll_requires_fresh_session()
    );
}

#[test]
fn v3_active_session_retains_slots_and_fails_closed_without_orphan_authority() {
    let scratch = Scratch::new("lease-poll-v3-active");
    let fixture = Fixture::new();
    let journal = fixture.open(&scratch);
    journal.begin_session(fixture.binding()).expect("session");
    journal
    ...
    let migrated = fixture.open(&scratch);
    let snapshot = migrated.snapshot().expect("migrated active snapshot");
    assert!(
        snapshot.slot(fixture.slot).is_some(),
        "recoverable slot must remain durable"
    );
    assert!(
        snapshot
            .session()
    ...
    assert!(matches!(
        migrated.begin_session(SessionBinding::new(
            RunnerSessionId::new(),
            PROTOCOL_MAX_VERSION,
            JobIrVersion::current(),
        )),
    ...
    assert_eq!(
        snapshot.session().expect("session").selected_protocol(),
        ProtocolVersion::new(3).expect("legacy protocol")
    );
}

#[test]
fn v3_migration_commit_recovers_on_both_sides_of_atomic_rename() {
    for (stage, migrated_before_reopen) in [
        (CommitStage::FileSynced, false),
        (CommitStage::Renamed, true),
    ] {
        let scratch = Scratch::new(&format!("lease-poll-v3-migration-{stage:?}"));
        let fixture = Fixture::new();
        let journal = fixture.open(&scratch);
        journal.begin_session(fixture.binding()).expect("session");
        drop(journal);
        ...
        let session = recovered
            ...
            .session()
            .cloned()
            .expect("legacy-fenced session");
        assert!(session.lease_poll_requires_fresh_session());
        let encoded = fs::read_to_string(journal_file(scratch.state_root().as_path()))
            .expect("read recovered v4 journal");
        assert!(encoded.starts_with("{\"schema_version\":4,"));
    }
}
