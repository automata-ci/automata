...
use automata_core::{JobLifecycle, LogStreamId, OperationId, Sha256Digest};
use automata_protocol::LeaseRejectionReason;
...
fn prepare_undelivered_result_and_log(journal: &dyn RunnerJournal, fixture: &Fixture) {
    ...
    journal
        .open_log_stream(
            ...
            fixture.lease.guard(),
            stream,
            Fixture::content(automata_runner_journal::ContentKind::LogSpool, 0, 0x81),
        )
        .expect("open log");
    journal
        .record_log_produced(
            ...
            fixture.slot,
            fixture.lease.guard(),
            Fixture::log_production(stream, 0, false, 24, 0x82),
        )
        .expect("produce log");
    journal
...
fn authority_is_required_and_orphan_deliveries_remain_until_explicit_abandonment() {
    ...
    for delivery in [OrphanDelivery::TerminalResult, OrphanDelivery::LogStream] {
        journal
