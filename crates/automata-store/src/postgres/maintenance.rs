...
use automata_core::{AttemptId, JobLifecycle, RunId};
...
impl ControlPlaneMaintenanceRepository for PostgresStore {
    async fn maintain_control_plane(
        ...
        Ok(ControlPlaneMaintenanceReport::new(
            expired_attempts,
...
async fn reap_expired_attempt(
    ...
    apply_expired_attempt(&mut transaction, request, attempt_id, mutation).await?;
    ...
    .await?;
    transaction.commit().await.map_err(StoreError::operation)?;
    Ok(Some(ExpiredAttemptMaintenance::new(
        attempt_id,
...
async fn stale_session_candidates(
    ...
    .bind(request.stale_session_cutoff().get())
    .bind(i64::from(request.batch_size().get()))
    .fetch_all(&store.pool)
    .await
    .map_err(StoreError::operation)?;
...
async fn close_stale_session(
    ...
    apply_stale_session_close(&mut transaction, request, candidate, authority).await?;
    transaction.commit().await.map_err(StoreError::operation)?;
    Ok(true)
}
...
async fn lock_stale_session(
    ...
    let heartbeat_at: i64 = session
        ...
        .map_err(StoreError::operation)?;
    if disconnected_at.is_some() || heartbeat_at > request.stale_session_cutoff().get() {
        return Ok(None);
    }
