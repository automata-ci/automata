...
pub(crate) fn decode_sha256_digest(
    bytes: Vec<u8>,
) -> Result<automata_core::Sha256Digest, DurabilityValueError> {
    let length = bytes.len();
    let bytes = bytes
        .try_into()
        .map_err(|_| DurabilityValueError::InvalidDigestLength { length })?;
    Ok(automata_core::Sha256Digest::from_bytes(bytes))
}

pub(crate) fn sha256_digest(bytes: &[u8]) -> automata_core::Sha256Digest {
