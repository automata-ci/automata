...
async fn sessions_resume_and_replay_exact_server_commands_and_rpc_receipts() -> TestResult {
    run_with_database(|database| async move {
        ...
        let first_command = command(first_fence, OperationId::new(), "automata.lease.v1", 3, b"lease")?;
        ...
        assert!(
            replay
                .iter()
                .all(automata_store::DurableRunnerCommand::was_replayed)
        );

        let through_one = CommandCursor::through(CommandSequence::new(1)?);
        let heartbeat = database
            .store()
            .heartbeat_session(HeartbeatRunnerSession::new(first_fence, through_one, UnixMillis::new(5)))
            .await?;
        ...
        assert!(matches!(
            database
                ...
                .await,
            Err(StoreError::SessionFenceRejected(id)) if id == first_fence.session_id()
        ));
...
async fn claim_receipts_bind_one_based_slots_and_cancellation_delivery() -> TestResult {
    run_with_database(|database| async move {
        ...
        let initial_claim = claim(&database, operation_id, attempt_id, fence, 1, 20, 100).await?;
        ...
        let changed_page = scan(&database, fence, 1, 20).await?;
        ...
        let expired = database
            .store()
            .requeue_expired(UnixMillis::new(100), 10, 10)
            .await?;
    ...
    run_with_database(|database| async move {
        ...
        assert_eq!(
            runnable
                .candidates()
                .iter()
                .map(automata_store::RunnableAttempt::attempt_id)
                .collect::<Vec<_>>(),
            vec![attempt_b],
        ...
        let reselected_page = scan(&database, fence, 1, 21).await?;
        ...
        claim_success(
            &database,
            claim(&database, OperationId::new(), attempt_b, fence, 2, 22, 100).await?,
        )
        .await?;
        let no_work_key =
        ...
        let no_work_page = scan(&database, fence, 3, 23).await?;
        ...
        let no_work_retry_page = scan(&database, fence, 3, 25).await?;
...
async fn runnable_scan_is_authoritatively_tenant_scoped() -> TestResult {
    run_with_database(|database| async move {
        ...
        let first_page = scan(&database, first.session_fences[0], 1, 20).await?;
        assert_eq!(
            first_page
                .candidates()
                .iter()
        ...
        let second_page = scan(&database, second.session_fences[0], 1, 20).await?;
        assert_eq!(
            second_page
                .candidates()
                .iter()
...
async fn bounded_cursor_progresses_past_more_than_one_page_of_incompatible_work() -> TestResult {
    run_with_database(|database| async move {
        ...
        let first = scan_with_limit(&database, fence, 1, 30, 2).await?;
        assert_eq!(
            first
                .candidates()
                .iter()
        ...
        assert!(matches!(
            record_page_no_work(&database, fence, 1, 30, &first)
                .await?
                .outcome(),
            TryClaimOutcome::NoWork
        ));

        let second = scan_with_limit(&database, fence, 1, 30, 2).await?;
        assert_eq!(
            second
                .candidates()
                .iter()
        ...
        record_page_no_work(&database, fence, 1, 30, &second).await?;

        let third = scan_with_limit(&database, fence, 1, 30, 2).await?;
        ...
        let receipt = database
            ...
            .try_claim(TryClaimAttempt::new(
                ...
                UnixMillis::new(30),
                UnixMillis::new(100),
                third.claim_advance(compatible_attempt)?,
            )?)
            .await?;
...
async fn finite_high_water_cycle_wraps_and_eventually_reaches_new_arrivals() -> TestResult {
    run_with_database(|database| async move {
        ...
        let first_page = scan_with_limit(&database, fence, 1, 50, 1).await?;
        assert_eq!(first_page.candidates()[0].attempt_id(), old_first);
        record_page_no_work(&database, fence, 1, 50, &first_page).await?;
        ...
        let second_page = scan_with_limit(&database, fence, 1, 50, 1).await?;
        assert_eq!(
            second_page.candidates()[0].attempt_id(),
            old_second,
            "an arrival beyond the captured high-water mark cannot extend the active cycle"
        );
        record_page_no_work(&database, fence, 1, 50, &second_page).await?;

        let mut claimed_new = false;
        for _ in 0..3 {
            let page = scan_with_limit(&database, fence, 1, 50, 1).await?;
            ...
            if selected == new_attempt {
                ...
                let request = TryClaimAttempt::new(
                    ...
                    UnixMillis::new(50),
                    UnixMillis::new(100),
                    page.claim_advance(new_attempt)?,
                )?;
                assert!(matches!(
            ...
            record_page_no_work(&database, fence, 1, 50, &page).await?;
        }
        assert!(
            claimed_new,
...
async fn routing_fingerprint_change_supersedes_stale_cursor_proofs() -> TestResult {
    run_with_database(|database| async move {
        let seed = seed_control_plane(database.pool(), 1).await?;
        let fence = seed.session_fences[0];
        let stale_page = scan(&database, fence, 1, 20).await?;
        ...
        let stale = record_page_no_work(&database, fence, 1, 20, &stale_page).await?;
        assert!(matches!(
            stale.outcome(),
        ...
        let refreshed = scan(&database, fence, 1, 20).await?;
        assert_eq!(refreshed.expected_cursor_version(), 0);
        assert!(matches!(
            record_page_no_work(&database, fence, 1, 20, &refreshed)
                .await?
                .outcome(),
            TryClaimOutcome::NoWork
        ));
...
async fn concurrent_exact_retry_replays_without_double_cursor_advancement() -> TestResult {
    run_with_database(|database| async move {
        let seed = seed_control_plane(database.pool(), 1).await?;
        let fence = seed.session_fences[0];
        let first_page = scan(&database, fence, 1, 20).await?;
        let first_key =
            LeaseRequestKey::first(fence, OperationId::new(), StableRunnerSlot::new(1)?);
...
async fn receipt_completion_fault_rolls_back_cursor_and_pending_receipt() -> TestResult {
    run_with_database(|database| async move {
        let seed = seed_control_plane(database.pool(), 1).await?;
        let fence = seed.session_fences[0];
        let page = scan(&database, fence, 2, 20).await?;
        let request_key =
...
async fn exact_slot_availability_is_fenced_bounded_and_assignment_aware() -> TestResult {
    run_with_database(|database| async move {
        ...
        assert_eq!(
            database
                .store()
                .slot_availability(original, StableRunnerSlot::new(1)?, UnixMillis::new(10),)
                .await?,
        ...
        assert_eq!(
            database
                .store()
                .slot_availability(original, StableRunnerSlot::new(2)?, UnixMillis::new(10),)
                .await?,
        ...
        assert_eq!(
            database
                .store()
                .slot_availability(original, StableRunnerSlot::new(1)?, UnixMillis::new(13),)
                .await?,
        ...
        assert!(
            database
                .store()
                .slot_availability(original, StableRunnerSlot::new(1)?, UnixMillis::new(21),)
                .await
                .is_err(),
...
async fn draining_between_availability_and_claim_rejects_without_dequeuing() -> TestResult {
    run_with_database(|database| async move {
        ...
        assert_eq!(
            database
                .store()
                .slot_availability(fence, StableRunnerSlot::new(1)?, UnixMillis::new(10),)
                .await?,
            RunnerSlotAvailability::Available
        );

        let attempt_id = insert_attempt(database.store(), seed.job_id, 1, 11).await?;
        let page = scan(&database, fence, 1, 12).await?;
        sqlx::query("UPDATE runners SET desired_state = 'draining' WHERE id = $1")
            .bind(fence.runner_id().as_uuid())
            .execute(database.pool())
            .await?;
        ...
        let claim_request = TryClaimAttempt::new(
            claim_key,
            attempt_id,
            LeaseId::new(),
            UnixMillis::new(12),
        ...
        begin_isolated_lease_request(&database, no_work_key).await?;
        let no_work_request =
            NoWorkLeaseRequest::new(no_work_key, UnixMillis::new(13), page.no_work_advance())?;
        let no_work = database
            .store()
        ...
        assert_eq!(
            database
                .store()
                .slot_availability(fence, StableRunnerSlot::new(1)?, UnixMillis::new(13),)
                .await?,
        ...
        database
            .store()
            .close_session(CloseRunnerSession::new(fence, UnixMillis::new(16)))
            .await?;
...
async fn routing_labels_are_case_insensitive_and_case_folded_duplicates_are_corrupt() -> TestResult
{
    run_with_database(|database| async move {
        ...
        let receipt = database
            .store()
            .try_claim(claim(&database, OperationId::new(), attempt_id, fence, 1, 11, 100).await?)
            .await?;
        ...
        assert!(matches!(
            database
                ...
                .open_session(OpenRunnerSession::new(
                    ...
                    RunnerProtocolVersion::new(4)?,
                    JobIrVersion::current(),
                    automata_store::RoutingDocument::new("{}")?,
                    UnixMillis::new(9),
                ))
                .await,
...
async fn expiry_only_requeues_work_that_never_started_running() -> TestResult {
    run_with_database(|database| async move {
        ...
        let processed = database
            .store()
            .requeue_expired(UnixMillis::new(50), 10, 10)
            .await?;
...
async fn runnable_scan_and_claim_recheck_run_dag_cancellation_and_concurrency() -> TestResult {
    run_with_database(|database| async move {
        ...
        let stale_dependent_page = scan(&database, fence, 2, 20).await?;
        database
            .store()
        ...
        let blocked_claim = TryClaimAttempt::new(
            blocked_key,
            dependent_attempt,
            LeaseId::new(),
            UnixMillis::new(20),
            UnixMillis::new(100),
            stale_dependent_page.claim_advance(dependent_attempt)?,
        )?;
        ...
        for (lifecycle, observed_at) in [
            (JobLifecycle::Preparing, 22),
            (JobLifecycle::Running, 23),
            (JobLifecycle::Finalizing, 24),
            (JobLifecycle::Succeeded, 25),
        ] {
            database
                .store()
...
async fn blocked_dependency_reconciliation_uses_latest_prerequisite_attempt() -> TestResult {
    run_with_database(|database| async move {
        ...
        let blocked = database
            .store()
            .scan_blocked(RunnableScanLimit::new(10)?, UnixMillis::new(50))
            .await?;
        assert_eq!(
            blocked
                .iter()
        ...
        assert!(
            database
                .store()
                .scan_blocked(RunnableScanLimit::new(10)?, UnixMillis::new(52))
                .await?
                .is_empty()
        );
...
fn command(
    ...
    Ok(EnqueueRunnerCommand::new(
        ...
        automata_store::RunnerCommandPayload::new(DocumentSchema::new(1)?, payload.to_vec())?,
...
fn cancellation_command(
    ...
    guard: automata_core::LeaseGuard,
    ...
    Ok(EnqueueRunnerCommand::new(
        ...
        automata_store::RunnerCommandPayload::new(
...
) -> TestResult<automata_store::RoutingDocument> {
    Ok(automata_store::RoutingDocument::new(
...
async fn scan(
    ...
    slot: u16,
    observed_at: i64,
) -> TestResult<automata_store::RunnableScanPage> {
    scan_with_limit(database, fence, slot, observed_at, 10).await
}
...
async fn record_page_no_work(
    ...
    slot: u16,
    observed_at: i64,
    page: &automata_store::RunnableScanPage,
) -> TestResult<automata_store::TryClaimReceipt> {
    let request_key =
...
async fn claim(
    ...
    begin_isolated_lease_request(database, request_key).await?;
    let page = scan(database, fence, slot, observed_at).await?;
...
async fn insert_attempt(
    store: &automata_store::PostgresStore,
    job_id: JobId,
    attempt_number: u32,
    queued_at: i64,
...
async fn transition_path(
    database: &TestDatabase,
    attempt_id: AttemptId,
    claimed: &automata_store::ClaimedAttempt,
    path: &[JobLifecycle],
    ...
    for (offset, lifecycle) in path.iter().copied().enumerate() {
        database
            ...
            .transition(TransitionAttempt::new(
                ...
                claimed.lease().guard(),
                lifecycle,
                UnixMillis::new(first_observed_at + i64::try_from(offset)?),
            ))
            .await?;
    }
    Ok(())
...
) -> TestResult<automata_store::ClaimedAttempt> {
