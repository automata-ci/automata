use automata_core::{
    AttemptId, FencingToken, JobConclusion, JobId, JobIrVersion, Lease, LeaseGuard, LeaseId,
    LogSequence, LogStreamId, OperationId, RunId, RunnerId, RunnerRequirements, RunnerSessionId,
    Sha256Digest, UnixMillis,
};
use automata_store::{
    AttemptAssignment, BeginLeaseRequest, CancelJobCommandPayload, CancellationRepository,
    CommandCursor, CommandReplayLimit, CommandSequence, CommitRunnerLogSegment,
    CompleteLeaseRequest, CurrentRunnerSessionRepository, DocumentSchema, JobDependency,
    JobIrMetadata, LeaseRequestKey, LogMetadataRepository, LogSegmentMetadata, LogStreamMetadata,
    MAX_JOB_IR_BYTES, MAX_LOG_SEGMENT_BYTES, MAX_TERMINAL_RESULT_BYTES, ObjectKey, PostgresStore,
    RoutingDocument, RoutingLabel, RunnableAttempt, RunnableAttemptRepository, RunnableScanLimit,
    RunnerClaimRepository, RunnerCommandOutbox, RunnerControlTransactionRepository,
    RunnerGeneration, RunnerLeaseOfferRepository, RunnerLeaseRequestRepository,
    RunnerOperationKind, RunnerOperationReceiptRepository, RunnerProtocolVersion,
    RunnerRoutingRepository, RunnerSessionFence, RunnerSessionRepository, RunnerSlotCount,
    SessionEpoch, StableRunnerSlot, TerminalResultMetadata, TerminalResultRepository,
...
fn durable_scalar_types_match_wire_ranges_and_core_digest_identity() {
    ...
    let store_digest = automata_store::Sha256Digest::from_bytes([9; 32]);
...
fn durable_command_and_receipt_debug_never_expose_payload_bytes() {
    ...
    let command = automata_store::RunnerCommandPayload::new(
        DocumentSchema::new(2).expect("schema"),
        secret.as_bytes().to_vec(),
    )
    .expect("command payload");
    let response = automata_store::RunnerOperationResponse::new(
...
fn lease_request_chain_values_bind_predecessor_and_exact_rpc_digest() {
    ...
    .expect("response");
    let complete = CompleteLeaseRequest::new(begin, response.clone(), UnixMillis::new(9));
...
fn immutable_execution_metadata_is_bounded_and_fenced() {
    ...
    let result = TerminalResultMetadata::new(
        OperationId::new(),
        attempt_id,
        assignment,
        LeaseGuard::new(lease_id, fence),
        DocumentSchema::new(1).expect("schema"),
        MAX_TERMINAL_RESULT_BYTES,
        digest,
        ObjectKey::new("results/final").expect("key"),
    ...
    assert_eq!(stream.assignment(), assignment);
    assert!(
        LogSegmentMetadata::new(
            OperationId::new(),
            stream.stream_id(),
            LogSequence::new(2),
            LogSequence::new(1),
            ObjectKey::new("logs/segment").expect("key"),
            digest,
            1,
            1,
            UnixMillis::new(30),
...
fn scheduler_and_durability_ports_remain_backend_neutral_and_dyn_compatible() {
    fn assert_postgres_ingress_adapters<T: TerminalResultRepository + LogMetadataRepository>() {}
    ...
    let ingress_request = automata_store::RunnerOperationRequest::new(
    ...
    assert!(
        CommitRunnerLogSegment::new(
            ...
            ObjectKey::new("logs/out-of-range").expect("key"),
            Sha256Digest::from_bytes([6; 32]),
            1,
            1,
            UnixMillis::new(1),
            false,
            automata_store::RunnerOperationResponse::new(
                DocumentSchema::new(1).expect("schema"),
                vec![1],
            )
            .expect("response"),
