...
use automata_core::{
    AttemptId, AttemptNumber, JobConclusion, JobLifecycle, Lease, LeaseId, LogSequence,
    LogStreamId, OperationId, Sha256Digest, UnixMillis,
};
use automata_store::{
    AcknowledgeRunnerCommands, AttemptAssignment, BeginLeaseRequest, CANCEL_JOB_COMMAND_KIND,
    ...
    LeaseOfferCommandIdentity, LeaseRequestKey, LeaseResponseAction, LogMetadataRepository as _,
    LogSegmentMetadata, LogStreamMetadata, MaintenanceBatchSize, NoWorkLeaseRequest, ObjectKey,
    OpenRunnerSession, PublishLeaseOffer, QueuedAttempt, RenewLease, RequestCancellation,
    RunnableAttemptRepository as _, RunnableScanLimit, RunnableScanRequest,
    RunnerClaimRepository as _, RunnerCommandOutbox as _, RunnerCommandPayload,
    RunnerControlTransactionRepository as _, RunnerGeneration, RunnerLeaseOfferRepository as _,
    RunnerLeaseRequestRepository as _, RunnerOperationKind, RunnerOperationReceiptRepository as _,
    ...
    TerminalResultMetadata, TerminalResultRepository as _, TryClaimAttempt, TryClaimOutcome,
};
...
async fn reaped_claim_completes_no_work_and_evicts_without_offer_publication() -> TestResult {
    run_with_database(|database| async move {
        ...
        let maintenance = database
            .store()
            .maintain_control_plane(maintenance_request(100)?)
            .await?;
...
#[tokio::test]
#[ignore = "requires AUTOMATA_TEST_DATABASE_URL and creates a temporary schema"]
async fn durable_publication_wins_over_later_claim_reaping() -> TestResult {
    run_with_database(|database| async move {
        let (seed, lease, metadata, slot, request_operation_id, request_digest) =
            active_lease(&database).await?;
        let fence = seed.session_fences[0];
        ...
        let LeaseOfferClaimStatus::Published(replayed) =
            database.store().inspect_lease_offer_claim(claim).await?
        else {
            panic!("durable publication must win over a later attempt transition");
        };
        assert!(replayed.was_replayed());
        assert_eq!(replayed.command().request(), committed.command().request());
        assert_eq!(
            replayed.command().sequence(),
...
#[tokio::test]
#[ignore = "requires AUTOMATA_TEST_DATABASE_URL and creates a temporary schema"]
async fn concurrent_publish_and_reap_linearize_without_partial_offer() -> TestResult {
    run_with_database(|database| async move {
        let (seed, lease, metadata, slot, request_operation_id, request_digest) =
            active_lease(&database).await?;
        let fence = seed.session_fences[0];
        ...
        let (publication_result, maintained) = tokio::join!(
            ...
            reaper.maintain_control_plane(maintenance_request(100)?)
        );
        ...
        match publication_result {
            Ok(committed) => {
                ...
                let LeaseOfferClaimStatus::Published(replayed) =
                    ...
                    panic!("committed publication must remain authoritative");
                };
                assert_eq!(replayed.command().request(), committed.command().request());
            }
            Err(StoreError::AttemptFenceRejected(attempt_id)) => {
                assert_eq!(attempt_id, lease.attempt_id());
...
async fn heartbeat_and_ack_transactions_roll_back_and_replay_exact_responses() -> TestResult {
    run_with_database(|database| async move {
        ...
        assert_eq!(lease_expiry(&database, lease.attempt_id()).await?, 200);
    ...
    run_with_database(|database| async move {
        ...
        let state: (String, i64) =
            sqlx::query_as("SELECT lifecycle, lease_expires_at_ms FROM job_attempts WHERE id = $1")
                ...
                .fetch_one(database.pool())
                .await?;
        assert_eq!(state, ("running".to_owned(), 200));
...
async fn log_and_terminal_ingress_commit_contiguously_and_roll_back_with_receipts() -> TestResult {
    run_with_database(|database| async move {
        ...
        let replay = database
            .store()
            .commit_runner_log_segment(first_segment)
            .await?;
        assert!(!first.was_replayed());
        assert!(replay.was_replayed());
        ...
        assert_eq!(
            ...
            "a terminal result closes cancellation even if its command ACK was lost"
        );
...
#[tokio::test]
#[ignore = "requires AUTOMATA_TEST_DATABASE_URL and creates a temporary schema"]
async fn general_log_and_terminal_ports_are_backed_by_postgres() -> TestResult {
    run_with_database(|database| async move {
        let (seed, lease, _metadata, slot, _request_operation_id, _request_digest) =
            active_lease(&database).await?;
        let fence = seed.session_fences[0];
        let assignment = AttemptAssignment::new(fence, slot);
        let stream_id = LogStreamId::new();
        let stream = LogStreamMetadata::new(
            OperationId::new(),
            stream_id,
            lease.attempt_id(),
            assignment,
        ...
        assert!(
            !database
                .store()
                .create_log_stream(stream.clone())
                .await?
                .was_replayed()
        );
        assert!(
            database
                .store()
                .create_log_stream(stream)
                .await?
                .was_replayed()
        );
        let segment = LogSegmentMetadata::new(
            OperationId::new(),
            stream_id,
            LogSequence::new(0),
            LogSequence::new(0),
            ObjectKey::new("logs/general.json.gz")?,
        ...
        assert!(
            !database
                .store()
                .append_log_segment(segment.clone())
                .await?
                .was_replayed()
        );
        assert!(
            database
                .store()
                .append_log_segment(segment)
                .await?
                .was_replayed()
        );

        let terminal = TerminalResultMetadata::new(
            OperationId::new(),
            lease.attempt_id(),
            assignment,
            lease.guard(),
            ...
            ObjectKey::new("results/general.json")?,
            JobConclusion::Success,
            UnixMillis::new(29),
            UnixMillis::new(30),
        )?;
        assert!(
            !database
                .store()
                .commit_terminal_result(terminal.clone())
                .await?
                .was_replayed()
        );
        assert!(
            database
                .store()
                .commit_terminal_result(terminal)
                .await?
                .was_replayed()
        );
        Ok(())
...
fn maintenance_request(observed_at: i64) -> TestResult<ControlPlaneMaintenanceRequest> {
    Ok(ControlPlaneMaintenanceRequest::new(
        UnixMillis::new(observed_at),
        LeaseFailureLimit::new(3)?,
        MaintenanceBatchSize::new(10)?,
        StaleSessionTimeoutMillis::new(10_000)?,
    )?)
}
