...
-- Preserve legacy administrator intent before narrowing `status` to observed
-- connectivity only. No legacy certificate fingerprint is promoted into the
-- new authority table: it lacks a verified identity and expiration.
UPDATE runners
...
COMMENT ON COLUMN runners.certificate_fingerprint IS
    'Legacy non-authoritative fingerprint; never used for runner authentication';
