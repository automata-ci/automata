...
use automata_core::{
    ExpressionComparison, ExpressionDialect, ExpressionInstruction, ExpressionLiteral,
    ExpressionLogical, ExpressionProgram, MAX_EXPRESSION_DEPTH, MAX_EXPRESSION_INSTRUCTIONS,
    MAX_EXPRESSION_SOURCE_BYTES, MAX_EXPRESSION_TEXT_BYTES,
};
...
impl GithubConditionCompiler {
    ...
    pub fn compile_condition(
        ...
        let mut parser = Parser::new(source, range, phase, self.limits)?;
...
pub(crate) fn condition_has_expression_opening(source: &str) -> bool {
    first_unquoted_delimiter(source, 0, source.len())
        .is_some_and(|(offset, length)| length == 3 && source[offset..].starts_with("${{"))
}
...
impl<'source> Parser<'source> {
        ...
        match token.kind {
            ...
            TokenKind::Identifier(name) => {
                ...
                if self.current.kind == TokenKind::LeftParen {
                    ...
                    if !context_allowed(self.phase, &canonical) {
                        return Err(context_error(
                            "github.expression.unrecognized_context",
                            format!(
                                "context `{name}` is unavailable in {} conditions",
    ...
    fn parse_call(
        ...
        let canonical = name.to_ascii_lowercase();
        let Some(signature) = function_signature(self.phase, &canonical) else {
            return Err(context_error(
                "github.expression.unrecognized_function",
                format!(
                    "function `{name}` is unavailable in {} conditions",
                    phase_name(self.phase)
                ),
                name_start,
                name_end - name_start,
...
fn function_signature(phase: GithubConditionPhase, name: &str) -> Option<FunctionSignature> {
    ...
    match (phase, name) {
        (GithubConditionPhase::Job, "always" | "cancelled") => Some(FunctionSignature {
            minimum: 0,
            maximum: 0,
        }),
        (GithubConditionPhase::Job, "failure" | "success") => Some(FunctionSignature {
            minimum: 0,
            maximum: u16::MAX as usize,
        }),
        (GithubConditionPhase::Step, "always" | "cancelled" | "failure" | "success") => {
            Some(FunctionSignature {
                minimum: 0,
                maximum: 0,
            })
        }
        (GithubConditionPhase::Step, "hashfiles") => Some(FunctionSignature {
            minimum: 1,
            maximum: 255,
        }),
        _ => None,
...
fn context_allowed(phase: GithubConditionPhase, name: &str) -> bool {
    match phase {
        GithubConditionPhase::Job => matches!(name, "github" | "needs" | "vars"),
        GithubConditionPhase::Step => matches!(
            name,
            "strategy"
                | "matrix"
                | "steps"
...
const fn phase_name(phase: GithubConditionPhase) -> &'static str {
    match phase {
        GithubConditionPhase::Job => "job",
        GithubConditionPhase::Step => "step",
    }
}
