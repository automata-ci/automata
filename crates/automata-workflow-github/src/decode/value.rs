...
pub(super) fn boolean(
    ...
    let value = match scalar.decoded.as_str() {
...
pub(super) fn value_map(node: &YamlNode, path: &str, context: &mut DecodeContext<'_>) -> ValueMap {
    ...
    for entry in entries {
        ...
        let entry_path = format!("{path}.{}", key.decoded);
...
pub(super) fn permissions(
    ...
    for entry in entries {
        ...
        let entry_path = format!("{path}.{}", name.decoded);
    ...
    for entry in entries {
        match field_name(entry) {
            Some("run") if run.is_none() => {
                run = parse_run_defaults(&entry.value, &format!("{path}.run"), context);
            }
            Some("run") => {}
    ...
    for entry in entries {
        match field_name(entry) {
            ...
            Some("cancel-in-progress") if cancel_in_progress.is_none() => {
                cancel_in_progress =
                    boolean(&entry.value, &format!("{path}.cancel-in-progress"), context);
            }
            Some("queue") if queue.is_none() => {
                queue = parse_queue(&entry.value, &format!("{path}.queue"), context);
            }
