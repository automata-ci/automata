...
fn hostile_git_environment_cannot_execute_or_mutate_and_never_leaks() {
    ...
    fixture.commit_all();
    let hostile = fixture.path().join("hostile-git-environment");
    let fake_bin = hostile.join("bin");
    let home = hostile.join("home");
    let xdg = hostile.join("xdg");
    let markers = hostile.join("markers");
    fs::create_dir_all(&fake_bin).expect("create fake executable directory");
    fs::create_dir_all(&home).expect("create hostile home");
    fs::create_dir_all(&xdg).expect("create hostile XDG directory");
    fs::create_dir_all(&markers).expect("create hostile marker directory");

    let execution_marker = markers.join("executed");
    let fake_git = fake_bin.join("git");
    install_marker_program(&fake_git, &execution_marker);
    let helper = hostile.join("host-helper");
    ...
    let global_config = hostile.join("global.gitconfig");
    let system_config = hostile.join("system.gitconfig");
    let config = format!(
        "[core]\n\tfsmonitor = {}\n\thooksPath = {}\n[credential]\n\thelper = !{}\n[maintenance]\n\tauto = true\n",
        helper.display(),
        hooks.display(),
    ...
    let trace = hostile.join("git-trace-private-path-marker");
    let trace2 = hostile.join("git-trace2-private-path-marker");
    let trace2_perf = hostile.join("git-trace2-perf-private-path-marker");
    ...
    let private_environment_value = "hostile-git-private-environment-value";
    let status_before = fixture.git_stdout(&["status", "--porcelain=v2", "--untracked-files=all"]);
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_automata"))
        ...
        .env("GIT_DIR", hostile.join("fake.git"))
        .env("GIT_COMMON_DIR", hostile.join("fake-common.git"))
        .env("GIT_WORK_TREE", &hostile)
        .env("GIT_INDEX_FILE", hostile.join("fake-index"))
        .env("GIT_OBJECT_DIRECTORY", hostile.join("fake-objects"))
        .env(
            "GIT_ALTERNATE_OBJECT_DIRECTORIES",
            hostile.join("fake-alternates"),
        )
        .env("GIT_NAMESPACE", private_environment_value)
        .env("GIT_SHALLOW_FILE", hostile.join("fake-shallow"))
        .env("GIT_REPLACE_REF_BASE", private_environment_value)
        .env("GIT_EXEC_PATH", &fake_bin)
        .env("GIT_PAGER", &helper)
        .env("GIT_EDITOR", &helper)
        .env("GIT_SEQUENCE_EDITOR", &helper)
        ...
        .env("GIT_TRACE_SHALLOW", &trace)
        .env("GIT_TRACE_CURL", &trace)
        .env("GIT_TRACE2", &trace2)
        .env("GIT_TRACE2_PERF", &trace2_perf)
        .env(
            "GIT_TRACE2_EVENT",
            format!("af_unix:{}", socket_path.display()),
        )
        .env("AUTOMATA_HOSTILE_PRIVATE", private_environment_value)
        .output()
    ...
    let hostile_path = hostile.to_string_lossy();
    for private in [
        fixture_path.as_ref(),
        hostile_path.as_ref(),
