...
impl GithubWebhookFixtureIngress {
    ...
    ///
    /// [`RawWebhookFixture`] construction or decoding has already verified the
    /// immutable body lock. Signature verification and all durable replay
    /// decisions remain inside the production ingress.
    ///
    /// # Errors
    ///
    /// Returns an error when retained header values cannot form an HTTP request.
    pub async fn inject(
        &self,
