...
impl RunnerCertificateIssuer {
    pub(crate) fn from_pem(
        ...
        if issuer_not_before_seconds > now_seconds || issuer_not_after_seconds <= now_seconds {
            return Err(RunnerCertificateIssuerError);
        }
    ...
    fn issue(
        ...
        let observed_at_seconds = observed_at_ms
            .checked_div(1_000)
            .ok_or(RunnerCertificateIssuerError)?;
        if observed_at_seconds < self.issuer_not_before_seconds
            || observed_at_seconds >= self.issuer_not_after_seconds
        {
            return Err(RunnerCertificateIssuerError);
        }
        let now = OffsetDateTime::from_unix_timestamp(observed_at_seconds)
            .map_err(|_| RunnerCertificateIssuerError)?;
        let requested_not_after = now
            .checked_add(time::Duration::seconds(CERTIFICATE_LIFETIME_SECONDS))
            .ok_or(RunnerCertificateIssuerError)?;
        let not_after_seconds = requested_not_after
            .unix_timestamp()
            .min(self.issuer_not_after_seconds);
        if not_after_seconds <= observed_at_seconds {
            return Err(RunnerCertificateIssuerError);
        }
        let not_after = OffsetDateTime::from_unix_timestamp(not_after_seconds)
            .map_err(|_| RunnerCertificateIssuerError)?;
        let not_before_seconds = observed_at_seconds
            .saturating_sub(60)
            .max(self.issuer_not_before_seconds);
        let mut distinguished_name = DistinguishedName::new();
        distinguished_name.push(DnType::CommonName, runner_id.hyphenated().to_string());
        request.params.not_before = OffsetDateTime::from_unix_timestamp(not_before_seconds)
            .map_err(|_| RunnerCertificateIssuerError)?;
        ...
        let expires_at_seconds = not_after_seconds;
...
async fn redeem_enrollment(
    ...
    let Ok(issued) = state
        .issuer
        .issue(runner_id, &document.csr_pem, prepared.observed_at_ms)
    else {
        return ApiError::InvalidRequest.into_response();
    };
