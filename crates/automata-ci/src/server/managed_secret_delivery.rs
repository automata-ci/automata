...
use automata_ci_store::{
    AcknowledgeManagedSecretDelivery, BUILTIN_SECRET_PROVIDER_ID, ManagedSecretAuthorityBinding,
    ...
    IssueLeasedJobSecretGrants, ProtectedEnvironmentRepository, ProtectedEnvironmentStoreError,
...
impl ManagedSecretBindingIssuer for LeasedManagedSecretBindingIssuer {
    async fn issue(
        ...
        ManagedSecretBindingOverlay::new(
            lease,
            issued.into_iter().map(|entry| {
                (entry.canonical_name().to_owned(), entry.binding().clone())
            }),
        )
        .map_err(|_| ControlPortError::Corrupt)
    }
...
fn protected_environment_port_error(error: ProtectedEnvironmentStoreError) -> ControlPortError {
    match error {
        ...
        ProtectedEnvironmentStoreError::Operation(_) | ProtectedEnvironmentStoreError::CorruptData => {
