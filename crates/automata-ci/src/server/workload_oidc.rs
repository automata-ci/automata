//! Product composition for the optional Actions-compatible workload OIDC surface.
...
use automata_ci_store::{
    ReserveWorkloadOidcAuthority, WORKLOAD_OIDC_REQUEST_BEARER_KEY_FINGERPRINT_DOMAIN,
    WorkloadOidcAuthorityProposal, WorkloadOidcAuthorityRepository, WorkloadOidcCurrentPolicy,
    WorkloadOidcCurrentnessClock, WorkloadOidcCurrentnessClockError, WorkloadOidcExecutionIdentity,
    WorkloadOidcKeyUse, WorkloadOidcLoadedKey, WorkloadOidcStoreError,
    WorkloadOidcSubjectPolicyMode, WorkloadOidcSubjectPolicyRevision,
    workload_oidc_rs256_public_key_fingerprint,
};
use automata_ci_store_postgres::{
    PostgresStore, PostgresWorkloadOidcAuthorityRepository, PostgresWorkloadOidcIssuanceRepository,
};
use automata_ci_workload_oidc::{
    MAXIMUM_OIDC_KEYS_PER_KEYRING, MAXIMUM_REQUEST_BEARER_CLOCK_SKEW_SECONDS, OidcClock,
    OidcClockError, OidcIssuanceRepository, OidcIssuer, OidcKeyId, OidcService,
    OidcSupportedClaims, OidcTokenLifetime, RequestBearerConfig, RequestBearerKey,
    RequestBearerKeyring, Rs256Keyring, Rs256SigningKey, RsaPublicJwk, WorkloadOidcApi,
};
use axum::{
    Router,
...
const REQUEST_BEARER_AUDIENCE: &str = "automata-ci:workload-oidc-mint";
const SUBJECT_POLICY_FINGERPRINT_DOMAIN: &[u8] =
    b"automata/workload-oidc/stable-owner-subject-policy:v1\0";
const CONFIGURATION_FINGERPRINT_DOMAIN: &[u8] =
    b"automata/workload-oidc/product-configuration:v1\0";
...
#[derive(Debug, Error)]
pub enum WorkloadOidcProductError {
    ...
    #[error("Automata workload OIDC durable key readiness failed")]
    KeyReadiness,
}

/// Validated optional product configuration with no loaded key material.
#[derive(Clone)]
pub struct WorkloadOidcConfig {
    issuer: OidcIssuer,
    subject_policy_revision: WorkloadOidcSubjectPolicyRevision,
...
impl WorkloadOidcConfig {
    ...
    ) -> Result<Self, WorkloadOidcProductError> {
        let bytes = source
            .load_bytes(MAXIMUM_MANIFEST_BYTES)
            .map_err(|_| WorkloadOidcProductError::InvalidConfiguration)?;
    ...
    fn from_raw(
        ...
        if raw.schema != MANIFEST_SCHEMA || raw.subject_policy.mode != "stable_owner_evidence" {
            return Err(WorkloadOidcProductError::InvalidConfiguration);
        }
        let issuer = OidcIssuer::https(results_endpoint.url().clone())
            .map_err(|_| WorkloadOidcProductError::InvalidConfiguration)?;
        let subject_policy_revision =
            WorkloadOidcSubjectPolicyRevision::new(raw.subject_policy.revision)
                .map_err(|_| WorkloadOidcProductError::InvalidConfiguration)?;
    ...
    fn current_policy(&self) -> Result<WorkloadOidcCurrentPolicy, WorkloadOidcProductError> {
    ...
    fn load_keyrings(&self) -> Result<LoadedKeyrings, WorkloadOidcProductError> {
        ...
        for spec in &self.request_bearer.keys {
            ...
            let key = RequestBearerKey::new(spec.key_id.clone(), &secret)
                .map_err(|_| WorkloadOidcProductError::InvalidConfiguration)?;
            request_key_evidence.push(WorkloadOidcLoadedKey::new(
        ...
        for spec in &self.id_token.keys {
            let fingerprint = workload_oidc_rs256_public_key_fingerprint(&spec.public_jwk);
            ...
            let private_key = std::str::from_utf8(&private_key)
                .map_err(|_| WorkloadOidcProductError::InvalidConfiguration)?;
            signing_key_evidence.push(WorkloadOidcLoadedKey::new(
...
impl fmt::Debug for WorkloadOidcConfig {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("WorkloadOidcConfig")
...
impl RequestBearerManifest {
    fn validate(
        ...
        let active_key_id = active_key_id.ok_or(WorkloadOidcProductError::InvalidConfiguration)?;
        Ok(Self {
...
impl IdTokenManifest {
    fn validate(raw: RawIdTokenManifest) -> Result<Self, WorkloadOidcProductError> {
...
fn parse_secret_source(value: &str) -> Result<SecretSource, WorkloadOidcProductError> {
...
fn validate_supported_claims(
    claims: &[String],
) -> Result<BTreeSet<String>, WorkloadOidcProductError> {
...
struct LoadedKeyrings {
    ...
    request_key_evidence: Vec<WorkloadOidcLoadedKey>,
    signing_key_evidence: Vec<WorkloadOidcLoadedKey>,
    request_key_fingerprints: BTreeMap<OidcKeyId, Sha256Digest>,
}

trait WorkloadOidcProvisionerClock: fmt::Debug + Send + Sync {
    fn now_millis(&self) -> Result<UnixMillis, ()>;
}

#[derive(Clone, Copy, Debug, Default)]
struct SystemWorkloadOidcClock;

impl WorkloadOidcProvisionerClock for SystemWorkloadOidcClock {
    fn now_millis(&self) -> Result<UnixMillis, ()> {
...
impl OidcClock for SystemWorkloadOidcClock {
    fn now_seconds(&self) -> Result<u64, OidcClockError> {
        let millis = WorkloadOidcProvisionerClock::now_millis(self).map_err(|()| OidcClockError)?;
        u64::try_from(millis.get() / 1_000).map_err(|_| OidcClockError)
    }
}

impl WorkloadOidcCurrentnessClock for SystemWorkloadOidcClock {
    fn now_millis(&self) -> Result<UnixMillis, WorkloadOidcCurrentnessClockError> {
        WorkloadOidcProvisionerClock::now_millis(self)
            .map_err(|()| WorkloadOidcCurrentnessClockError)
    }
}

struct ProductWorkloadOidcProvisioner {
    repository: Arc<dyn WorkloadOidcAuthorityRepository>,
    clock: Arc<dyn WorkloadOidcProvisionerClock>,
    current_policy: WorkloadOidcCurrentPolicy,
    request_key_fingerprints: BTreeMap<OidcKeyId, Sha256Digest>,
}

impl ProductWorkloadOidcProvisioner {
    fn new(
        repository: Arc<dyn WorkloadOidcAuthorityRepository>,
        clock: Arc<dyn WorkloadOidcProvisionerClock>,
        current_policy: WorkloadOidcCurrentPolicy,
...
impl fmt::Debug for ProductWorkloadOidcProvisioner {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("ProductWorkloadOidcProvisioner")
...
#[async_trait]
impl WorkloadOidcAuthorityProvisioner for ProductWorkloadOidcProvisioner {
    async fn reserve_workload_oidc_runtime_authority(
        ...
        let execution = WorkloadOidcExecutionIdentity::new(
            job.workflow_id(),
            repository_name,
            job.job().run_id(),
            job.job().job_id(),
        ...
        let reserve = ReserveWorkloadOidcAuthority::new(
            execution,
            self.current_policy,
            proposal,
            observed_at,
        ...
        let reserved = self
            .repository
            .reserve_workload_oidc_authority(reserve)
            .await
            .map_err(map_store_error)?;
        Ok(ReservedWorkloadOidcRuntimeAuthority::new(
...
const fn map_store_error(error: WorkloadOidcStoreError) -> ControlPortError {
    match error {
        WorkloadOidcStoreError::Unavailable => ControlPortError::Unavailable,
        WorkloadOidcStoreError::CorruptData => ControlPortError::Corrupt,
        WorkloadOidcStoreError::Unauthorized
        | WorkloadOidcStoreError::Conflict
        | WorkloadOidcStoreError::ResourceExhausted => ControlPortError::Conflict,
    }
}

/// OIDC routes and optional authority contribution admitted for one replica.
pub(crate) struct WorkloadOidcProduct {
    pub(crate) router: Router,
    pub(crate) authority_issuer: Arc<dyn OptionalRuntimeAuthorityIssuer>,
    operationally_ready: bool,
}

impl WorkloadOidcProduct {
    pub(crate) fn unavailable() -> Self {
        Self {
            router: Router::new(),
            authority_issuer: Arc::new(UnavailableWorkloadOidcRuntimeAuthorityIssuer),
...
pub(crate) fn compose_runtime_authority_issuer(
    ...
    workload_oidc: Arc<dyn OptionalRuntimeAuthorityIssuer>,
    ...
    let composite = CompositeRuntimeAuthorityIssuer::new(vec![required_results])?
        .with_optional_issuers(vec![workload_oidc, github_repository])?;
    Ok(Arc::new(composite))
}

/// Builds the complete OIDC product or the zero-credential entitlement guard.
pub(crate) async fn build_workload_oidc_product(
    config: Option<&WorkloadOidcConfig>,
    store: &PostgresStore,
) -> Result<WorkloadOidcProduct, WorkloadOidcProductError> {
    ...
    let Some(config) = config else {
        return Ok(WorkloadOidcProduct::unavailable());
    };
    let loaded = config.load_keyrings()?;
    let current_policy = config.current_policy()?;
    let clock = Arc::new(SystemWorkloadOidcClock);
    let issuance_repository = Arc::new(
        PostgresWorkloadOidcIssuanceRepository::new(
            store.clone(),
            current_policy,
    ...
    let now_seconds = OidcClock::now_seconds(clock.as_ref())
        .map_err(|_| WorkloadOidcProductError::KeyReadiness)?;
    admit_key_readiness(
        issuance_repository
            .verify_workload_oidc_key_readiness(now_seconds, &loaded.request_key_evidence)
            .await,
    )?;
    ...
    let authority_repository: Arc<dyn WorkloadOidcAuthorityRepository> = Arc::new(
        PostgresWorkloadOidcAuthorityRepository::new(store.clone(), clock.clone()),
    );
    let provisioner_clock: Arc<dyn WorkloadOidcProvisionerClock> = clock;
    let provisioner: Arc<dyn WorkloadOidcAuthorityProvisioner> =
        Arc::new(ProductWorkloadOidcProvisioner::new(
            authority_repository,
            provisioner_clock,
            current_policy,
            loaded.request_key_fingerprints,
        ));
    let authority_ids = Arc::new(RandomWorkloadOidcAuthorityIdGenerator);
    let authority_issuer = WorkloadOidcRuntimeAuthorityIssuer::new(
        config.issuer.clone(),
...
fn admit_key_readiness(
    result: Result<(), WorkloadOidcStoreError>,
) -> Result<(), WorkloadOidcProductError> {
    result.map_err(|_| WorkloadOidcProductError::KeyReadiness)
}
...
fn oidc_router_with_deadline(
    ...
    WorkloadOidcApi::new(service, clock).router().layer(
        ServiceBuilder::new()
...
mod tests {
    ...
    use automata_ci_store::{
        JobIrMetadata, ObjectKey, ReservedWorkloadOidcAuthority, RunnerGeneration,
        RunnerSessionFence, SessionEpoch, StableRunnerSlot, WorkloadOidcStoreError,
    };
    use automata_ci_workload_oidc::{
...
norlX3KEHNe7cTke5cP4OA==";
    ...
    fn write_private_test_file(name: &str, bytes: &[u8]) -> PathBuf {
        ...
        let directory = test_root.join(format!(
            "automata-ci-workload-oidc-product-{}",
            std::process::id()
        ));
    ...
    fn manifest_is_atomic_bounded_and_stable_owner_policy_only() {
        let config = WorkloadOidcConfig::from_raw(raw_manifest(), &https_results())
        ...
        assert!(matches!(
            WorkloadOidcConfig::from_raw(repository_name_fallback, &https_results()),
            Err(WorkloadOidcProductError::InvalidConfiguration)
        ));

        let mut partial_claims = raw_manifest();
        partial_claims.supported_claims.pop();
        assert!(WorkloadOidcConfig::from_raw(partial_claims, &https_results()).is_err());

        let mut hmac_prepublished = raw_manifest();
        hmac_prepublished.request_bearer.keys[1].lifecycle = RawKeyLifecycle::Prepublished;
        assert!(WorkloadOidcConfig::from_raw(hmac_prepublished, &https_results()).is_err());
        ...
        assert!(WorkloadOidcConfig::from_raw(raw_manifest(), &development).is_err());
        ...
        assert!(WorkloadOidcConfig::load(&source, &https_results()).is_ok());
        exact_bytes.push(b' ');
        write_private_test_file("manifest-size.json", &exact_bytes);
        assert!(matches!(
            WorkloadOidcConfig::load(&source, &https_results()),
            Err(WorkloadOidcProductError::InvalidConfiguration)
        ));
    }
    ...
    fn manifest_enforces_exact_shared_key_and_time_limits() {
        ...
        exact.request_bearer.maximum_lifetime_seconds =
            automata_ci_workload_oidc::MAXIMUM_REQUEST_BEARER_LIFETIME_SECONDS;
        exact.request_bearer.allowed_clock_skew_seconds = MAXIMUM_REQUEST_BEARER_CLOCK_SKEW_SECONDS;
        exact.id_token.lifetime_seconds =
            automata_ci_workload_oidc::MAXIMUM_ID_TOKEN_LIFETIME_SECONDS;
        ...
        exact.id_token.keys = (0..MAXIMUM_OIDC_KEYS_PER_KEYRING)
            ...
            .collect();
        assert!(WorkloadOidcConfig::from_raw(exact, &https_results()).is_ok());
        ...
        excessive_hmac.request_bearer.keys = (0..=MAXIMUM_OIDC_KEYS_PER_KEYRING)
            ...
            .collect();
        assert!(WorkloadOidcConfig::from_raw(excessive_hmac, &https_results()).is_err());
        ...
        excessive_rsa.id_token.keys = (0..=MAXIMUM_OIDC_KEYS_PER_KEYRING)
            ...
            .collect();
        assert!(WorkloadOidcConfig::from_raw(excessive_rsa, &https_results()).is_err());

        let mut excessive_lifetime = raw_manifest();
        excessive_lifetime.request_bearer.maximum_lifetime_seconds =
            automata_ci_workload_oidc::MAXIMUM_REQUEST_BEARER_LIFETIME_SECONDS + 1;
        assert!(WorkloadOidcConfig::from_raw(excessive_lifetime, &https_results()).is_err());
        ...
        assert!(WorkloadOidcConfig::from_raw(excessive_skew, &https_results()).is_err());
    }
    ...
    fn complete_keyrings_load_with_exact_non_secret_fingerprints() {
        ...
        let config = WorkloadOidcConfig::from_raw(
        ...
        assert_eq!(
            loaded
                .signing_key_evidence
                .iter()
                .map(WorkloadOidcLoadedKey::key_sha256)
                .collect::<BTreeSet<_>>(),
            expected_signing_fingerprints
        );
        assert_eq!(loaded.signing_keys.jwks().keys().len(), 2);

        let retained = WorkloadOidcConfig::from_raw(
    ...
    fn different_request_kids_cannot_share_hmac_material() {
        ...
        raw.id_token.keys[1].private_key_source =
            format!("file:{}", replacement_private_key_path.display());
        let config = WorkloadOidcConfig::from_raw(raw, &https_results()).expect("configuration");
        let Err(error) = config.load_keyrings() else {
            panic!("different request kids must have different fingerprints");
        };
        assert!(matches!(
            &error,
            WorkloadOidcProductError::InvalidConfiguration
        ));
    ...
    impl WorkloadOidcProvisionerClock for FixedClock {
        fn now_millis(&self) -> Result<UnixMillis, ()> {
            Ok(self.0)
        }
    }
    ...
    struct CaptureAuthorityRepository {
        requests: Mutex<Vec<ReserveWorkloadOidcAuthority>>,
    }

    #[async_trait]
    impl WorkloadOidcAuthorityRepository for CaptureAuthorityRepository {
    ...
    fn current_policy() -> WorkloadOidcCurrentPolicy {
        WorkloadOidcCurrentPolicy::new(
            WorkloadOidcSubjectPolicyMode::StableOwnerEvidence,
            WorkloadOidcSubjectPolicyRevision::new(1).expect("revision"),
    ...
    async fn provisioner_maps_only_execution_current_policy_and_trusted_time() {
        ...
        let issuer = WorkloadOidcRuntimeAuthorityIssuer::new(
            OidcIssuer::https("https://results.example.test/".parse().expect("issuer URL"))
                .expect("issuer"),
            test_request_keyring(),
            Arc::new(RandomWorkloadOidcAuthorityIdGenerator),
            provisioner,
        )
    ...
    async fn disabled_product_guard_blocks_only_entitled_github_jobs() {
        let unavailable = WorkloadOidcProduct::unavailable();
    ...
    async fn disabled_optional_products_decline_only_unentitled_jobs() {
        let composite = compose_runtime_authority_issuer(
            Arc::new(RequiredResultsIssuer),
            WorkloadOidcProduct::unavailable().authority_issuer,
