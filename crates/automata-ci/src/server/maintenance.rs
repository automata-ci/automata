...
impl ControlPlaneMaintenanceLoop {
    ...
    /// Attaches in-memory observations without changing maintenance correctness.
    #[must_use]
        ...
        loop {
            ...
            let Ok(request) = ControlPlaneMaintenanceRequest::new(
                ...
                if let Some(metrics) = &self.metrics {
                    metrics.observe_maintenance_pass("invalid_observation", started.elapsed());
                }
                tracing::error!(
            ...
            let result = tokio::select! {
                ...
                () = cancellation.cancelled() => {
                    if let Some(metrics) = &self.metrics {
                        metrics.observe_maintenance_pass("cancelled", started.elapsed());
                    }
                    break;
                },
            ...
            if let Ok(report) = result {
                ...
                let saturated = report.expired_attempts().len() >= batch_size
                    || usize::from(report.skipped_blocked_attempts()) >= batch_size
                    || usize::from(report.closed_stale_sessions()) >= batch_size;
                if let Some(metrics) = &self.metrics {
                    metrics.observe_maintenance_success(
                        started.elapsed(),
                        requeued,
                        lost,
                        report.skipped_blocked_attempts(),
                ...
                if let Some(metrics) = &self.metrics {
                    metrics.observe_maintenance_pass("error", started.elapsed());
                }
                tracing::warn!(
