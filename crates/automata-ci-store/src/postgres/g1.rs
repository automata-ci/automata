...
use crate::{
    ...
    RoutingLabel, RunnableAttempt, RunnableAttemptRepository, RunnableCursorAdvance,
    RunnableQueueKey, RunnableScanLimit, RunnableScanPage, RunnableScanRequest,
    RunnerClaimRepository, RunnerCommandOutbox, RunnerCommandPayload,
    RunnerControlTransactionRepository, RunnerGeneration, RunnerGroupId,
...
impl RunnerCommandEnvelopeMetadata {
    ...
    fn from_row(row: &sqlx::postgres::PgRow) -> Result<Self, StoreError> {
        let metadata = Self {
            ...
            digest: crate::value::decode_sha256_digest(
                row.try_get("command_digest").map_err(operation_error)?,
            )
...
fn decode_runner_payload_tombstone(
    ...
    match (reason, tombstoned_at) {
        ...
        (Some(reason), Some(tombstoned_at)) => {
            let reason = RunnerPayloadTombstoneReason::from_durable(&reason).ok_or_else(|| {
...
impl RunnerSessionRepository for PostgresStore {
    ...
    async fn close_session(&self, request: CloseRunnerSession) -> Result<(), StoreError> {
        ...
        if let Some(disconnected_at) = snapshot.disconnected_at() {
            ...
            .await?;
            mark_runner_offline(&mut transaction, request.fence(), disconnected_at).await?;
            transaction.commit().await.map_err(operation_error)?;
            return Ok(());
        }
        let database_now = super::runner_attempt_database_now(&mut transaction).await?;
        ...
        .await?;
        mark_runner_offline(&mut transaction, request.fence(), decision_at).await?;
        transaction.commit().await.map_err(operation_error)?;
        Ok(())
    }
...
impl RunnerControlTransactionRepository for PostgresStore {
    ...
    async fn authorize_lease_renewal(
        &self,
        request: crate::RenewLease,
        reported_lifecycle: JobLifecycle,
    ) -> Result<crate::RenewLease, StoreError> {
        let mut transaction = self.pool.begin().await.map_err(operation_error)?;
        super::pin_runner_attempt_read_committed(&mut transaction).await?;
...
async fn verify_replayed_runner_log_admission(
    ...
    .map_err(|error| StoreError::corrupt_data(error.to_string()))?;
    let digest =
        crate::value::decode_sha256_digest(row.try_get("object_digest").map_err(operation_error)?)
            .map_err(|error| StoreError::corrupt_data(error.to_string()))?;
...
async fn lock_exact_command_session(
    ...
    .await
    .map_err(operation_error)?
    .ok_or_else(|| StoreError::fence_rejected(session.session_id()))?;
...
fn decode_receipt_lease_offer_completion(
    ...
    match (
        ...
        (
            ...
            let fallback = RevokedLeaseOfferFallback::from_persisted(
                ...
                crate::value::decode_sha256_digest(fallback_digest)
                    .map_err(|error| StoreError::corrupt_data(error.to_string()))?,
            )
            .map_err(|error| StoreError::corrupt_data(error.to_string()))?;
            Ok(Some(ReceiptLeaseOfferCompletion {
                ...
                primary_response_digest: crate::value::decode_sha256_digest(primary_digest)
                    .map_err(|error| StoreError::corrupt_data(error.to_string()))?,
                fallback,
            }))
        }
