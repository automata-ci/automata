...
async fn insert_preparation_renewal_receipt(
    transaction: &mut Transaction<'_, Postgres>,
    request: &RenewLogicalActivationPreparation,
    runtime_policy: &crate::WorkflowRuntimePolicyPin,
    ...
    .bind(request.claim().target().invocation_id().as_uuid())
    .bind(request.claim().owner().as_uuid())
    .bind(runtime_policy.revision().as_i64())
...
async fn insert_claim(
    ...
    .bind(descriptor.runtime_policy().pin().revision().as_i64())
    .bind(
        descriptor
            .runtime_policy()
            .pin()
...
async fn load_receipt(
    ...
    .map_err(corrupt_value)?;
    let selection_origin = crate::LogicalWorkSelectionId::from_uuid(
