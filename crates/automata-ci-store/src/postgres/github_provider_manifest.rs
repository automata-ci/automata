...
impl GithubProviderManifestRepository for PostgresStore {
    async fn bootstrap_github_provider_repository(
        ...
        .await?;
        let receipt =
            GithubProviderRepositoryBootstrapReceipt::new(policy_receipt, manifest_receipt)
...
async fn bootstrap_locked_manifest(
    ...
    let (record, replay) = match current {
        Some(current) => {
            ...
            if current.manifest() == desired {
                ...
                .await?;
                (
                    GithubProviderManifestRecord::new(
                        desired.clone(),
                        authoritative_at,
    ...
    GithubProviderManifestBootstrapReceipt::new(record, replay)
        .map_err(|_| GithubProviderManifestStoreError::CorruptData)
}
...
async fn insert_manifest_revision(
    ...
    .bind(manifest.runtime_policy_revision().as_i64())
...
async fn advance_current_manifest(
    ...
    .bind(prior.revision().as_i64())
    .bind(replacement.revision().as_i64())
...
fn decode_manifest_record(
    ...
    let activated_at = row
        ...
        .map_err(operation_error)?
        .map(UnixMillis::new);
    GithubProviderManifestRecord::new(manifest, registered_at, activated_at)
        .map_err(|_| GithubProviderManifestStoreError::CorruptData)
}
