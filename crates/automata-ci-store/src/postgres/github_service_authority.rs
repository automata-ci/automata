...
use crate::{
    ...
    MIN_GITHUB_SERVICE_READY_USE_MILLIS, PostgresStore, ProtectedGithubServerServiceCredential,
...
impl GithubServerServiceAuthorityRepository for PostgresStore {
    ...
    async fn claim_github_server_service_mint(
        ...
        .bind(request.generation().as_i64())
    ...
    async fn reclaim_github_server_service_mint(
        ...
        .bind(request.key().generation().as_i64())
    ...
    async fn begin_github_server_service_mint(
            ...
            && optional_i64(&existing, "mint_started_at_ms")? == Some(request.started_at().get())
        {
            ...
            let start = crate::GithubServerServiceMintStart::from_request(&request, receipt)
        ...
        .bind(claim.key().generation().as_i64())
...
async fn finish_ready(
    ...
    .bind(claim.key().generation().as_i64())
    .bind(committed_at.get())
    .bind(old_current.map(GithubServerServiceGeneration::as_i64))
    .execute(&mut *transaction)
    .await
    .map_err(operation_error)?;
...
async fn finish_revoke_only(
    ...
    let metadata = protected.metadata();
    if !claim.selector().matches(metadata.identity())
        || metadata.generation() != claim.key().generation()
    ...
    if !matches!(
        ...
        || optional_i64(&existing, "mint_started_claim_fence")? != Some(claim.fence().as_i64())
...
async fn finish_mint_without_credential(
    ...
    if issuance_state(&existing)?.as_str() == effective_state
        && positive_u64(&existing, "mint_claim_fence")? == claim.fence().get()
...
async fn finish_revocation(
    ...
    if issuance_state(&existing)?.as_str() == effective_state
        ...
        && optional_i64(&existing, "revoke_result_claim_fence")? == Some(claim.fence().as_i64())
        && i64_column(&existing, "state_updated_at_ms")? == observed_at.get()
...
async fn recompute_github_server_service_failure_gate(
    ...
    .bind(transitioned_key.authority_id().as_uuid())
    .bind(next_mint_at)
    .bind(gate_generation.as_i64())
    .bind(transition_at.get())
    .bind(transitioned_key.generation().as_i64())
    .bind(
        descriptor
            .next_mint_not_before()
            .ok_or(GithubServerServiceStoreError::CorruptData)?
...
async fn select_authority_for_update(
    ...
    sqlx::query(AssertSqlSafe(query))
        ...
        .bind(selector.app_configuration_revision().as_i64())
        .bind(selector.policy_revision().as_i64())
...
fn decode_authority(
    ...
    let app_client_id =
        GithubServerServiceAppClientId::new(string_column(row, "github_app_client_id")?)
            .map_err(|_| GithubServerServiceStoreError::CorruptData)?;
    let jwt_issuer = GithubServerServiceJwtIssuer::from_durable(&string_column(
        row,
    ...
    let github_repository_name =
        GithubRepositoryName::new(string_column(row, "github_repository_name")?)
            .map_err(|_| GithubServerServiceStoreError::CorruptData)?;
    let scope = GithubServerServiceScope::from_durable(&string_column(row, "service_scope")?)
        .map_err(|_| GithubServerServiceStoreError::CorruptData)?;
    ...
    let state = GithubServerServiceAuthorityState::from_durable(&string_column(row, "state")?)
        .map_err(|_| GithubServerServiceStoreError::CorruptData)?;
...
fn decode_consumer_claim(
    ...
    let fence = GithubServerServiceClaimFence::new(positive_u64(row, "consumer_claim_fence")?)
        .map_err(|_| GithubServerServiceStoreError::CorruptData)?;
    let action = GithubServerServiceAction::from_durable(&string_column(row, "consumer_action")?)
        .map_err(|_| GithubServerServiceStoreError::CorruptData)?;
...
fn require_exact_begin_evidence(
    ...
    if optional_uuid(row, "mint_started_owner_id")? != Some(claim.worker().as_uuid())
        || optional_i64(row, "mint_started_claim_fence")? != Some(claim.fence().as_i64())
...
fn issuance_state(
    ...
    GithubServerServiceIssuanceState::from_durable(&string_column(row, "state")?)
        .map_err(|_| GithubServerServiceStoreError::CorruptData)
}
