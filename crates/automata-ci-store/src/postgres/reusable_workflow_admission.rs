...
use crate::{
    ...
    reusable_workflow_admission::{input_type_name, permission_level_name, sensitivity_name},
};
...
async fn insert_invocation_descriptor(
    ...
    .bind(
        invocation
            .parent_id()
            .map(crate::LogicalWorkflowInvocationId::as_uuid),
    )
    .bind(
        invocation
            .caller_job_id()
            .map(crate::LogicalWorkflowJobId::as_uuid),
    )
