//! Durable execution authority and key-retention ports for Automata workload OIDC.
...
pub use automata_ci_workload_oidc::{
...
use automata_ci_workload_oidc::{OidcAudience, OidcClaimSet, OidcSubject};
...
pub const MAX_WORKLOAD_OIDC_ISSUANCE_SLOTS: usize = 64;
...
pub const WORKLOAD_OIDC_REQUEST_BEARER_KEY_FINGERPRINT_DOMAIN: &[u8] =
    b"automata/workload-oidc/request-bearer-key-fingerprint:v1\0";
/// Domain prefix for the canonical public RS256 JWK fingerprint.
pub const WORKLOAD_OIDC_RS256_PUBLIC_KEY_FINGERPRINT_DOMAIN: &[u8] =
    b"automata/workload-oidc/rs256-public-key-fingerprint:v1\0";
#[cfg(feature = "adapter-spi")]
const WORKLOAD_OIDC_CLAIM_EVIDENCE_DOMAIN: &[u8] = b"automata/workload-oidc/claim-evidence:v1\0";
...
#[must_use]
pub fn workload_oidc_rs256_public_key_fingerprint(key: &RsaPublicJwk) -> Sha256Digest {
    let mut hasher = Sha256::new();
    hasher.update(WORKLOAD_OIDC_RS256_PUBLIC_KEY_FINGERPRINT_DOMAIN);
...
pub struct WorkloadOidcExecutionIdentity {
...
impl WorkloadOidcExecutionIdentity {
    ...
    pub fn new(
        ...
        lease
            .validate()
            .map_err(|_| WorkloadOidcValueError::InvalidExecution)?;
        ...
        {
            return Err(WorkloadOidcValueError::InvalidExecution);
        }
        Ok(Self {
            workflow_id,
...
pub enum WorkloadOidcSubjectPolicyMode {
    /// Immutable-ID policy requiring signed positive numeric owner evidence.
    StableOwnerEvidence,
}

impl WorkloadOidcSubjectPolicyMode {
    #[cfg(feature = "adapter-spi")]
...
pub struct WorkloadOidcSubjectPolicyRevision(u64);

impl WorkloadOidcSubjectPolicyRevision {
    /// Creates a positive policy revision.
    ///
    /// # Errors
    ///
    /// Rejects zero.
    pub const fn new(value: u64) -> Result<Self, WorkloadOidcValueError> {
...
#[allow(clippy::too_many_arguments)]
#[cfg(feature = "adapter-spi")]
pub(crate) fn workload_oidc_claim_evidence_digest(
    permission_evidence_sha256: Sha256Digest,
    subject_policy_mode: WorkloadOidcSubjectPolicyMode,
    ...
    let mut hasher = Sha256::new();
    hasher.update(WORKLOAD_OIDC_CLAIM_EVIDENCE_DOMAIN);
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct WorkloadOidcCurrentPolicy {
    subject_policy_mode: WorkloadOidcSubjectPolicyMode,
    subject_policy_revision: WorkloadOidcSubjectPolicyRevision,
    subject_policy_sha256: Sha256Digest,
    configuration_sha256: Sha256Digest,
...
impl WorkloadOidcCurrentPolicy {
    ...
    #[must_use]
    pub const fn subject_policy_mode(&self) -> WorkloadOidcSubjectPolicyMode {
        self.subject_policy_mode
    }

    /// Returns the exact current subject-policy revision.
    #[must_use]
    pub const fn subject_policy_revision(&self) -> WorkloadOidcSubjectPolicyRevision {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorkloadOidcAuthorityProposal {
...
impl WorkloadOidcAuthorityProposal {
    ...
    pub fn new(
        ...
        if expires_at_seconds <= issued_at_seconds
            ...
            || expires_at_seconds
                ...
                .is_none()
        {
            return Err(WorkloadOidcValueError::InvalidBearerInterval);
        }
        Ok(Self {
            authority_id,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReserveWorkloadOidcAuthority {
    execution: WorkloadOidcExecutionIdentity,
    current_policy: WorkloadOidcCurrentPolicy,
    proposal: WorkloadOidcAuthorityProposal,
    observed_at: UnixMillis,
}

impl ReserveWorkloadOidcAuthority {
    ...
    pub fn new(
        ...
        let bearer_expires_at_ms = proposal
            ...
            .checked_mul(1_000)
            .and_then(|value| i64::try_from(value).ok())
            .ok_or(WorkloadOidcValueError::InvalidBearerInterval)?;
    ...
    #[must_use]
    pub const fn execution(&self) -> &WorkloadOidcExecutionIdentity {
        &self.execution
    }

    /// Returns the exact current stable-owner policy configuration.
    #[must_use]
    pub const fn current_policy(&self) -> WorkloadOidcCurrentPolicy {
        self.current_policy
    }

    /// Returns the exact new-record proposal.
    #[must_use]
    pub const fn proposal(&self) -> &WorkloadOidcAuthorityProposal {
        &self.proposal
    }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReservedWorkloadOidcAuthority {
    authority_id: OidcAuthorityId,
    request_bearer_key_id: OidcKeyId,
    issued_at_seconds: u64,
    expires_at_seconds: u64,
...
impl ReservedWorkloadOidcAuthority {
    #[cfg(feature = "adapter-spi")]
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorkloadOidcLoadedKey {
    use_domain: WorkloadOidcKeyUse,
    id: OidcKeyId,
    fingerprint: Sha256Digest,
}

impl WorkloadOidcLoadedKey {
    /// Describes one loaded key without exposing its HMAC or private-key bytes.
    ///
    /// RS256 callers use [`workload_oidc_rs256_public_key_fingerprint`]. HMAC
    ...
    #[must_use]
    pub const fn key_use(&self) -> WorkloadOidcKeyUse {
        self.use_domain
    }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RetainWorkloadOidcKey {
    key_use: WorkloadOidcKeyUse,
    key_id: OidcKeyId,
    key_sha256: Sha256Digest,
...
impl RetainWorkloadOidcKey {
    ...
    pub fn request_bearer(
        ...
        if verification_skew_seconds > MAXIMUM_REQUEST_BEARER_CLOCK_SKEW_SECONDS {
            return Err(WorkloadOidcValueError::InvalidKeyDeadline);
        }
        let not_after_seconds = bearer_expires_at_seconds
            .checked_add(verification_skew_seconds)
            .ok_or(WorkloadOidcValueError::InvalidKeyDeadline)?;
        Self::new(
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorkloadOidcKeyDeadline {
    key_use: WorkloadOidcKeyUse,
    key_id: OidcKeyId,
...
impl WorkloadOidcKeyDeadline {
    ...
    #[must_use]
    pub fn from_retention(request: &RetainWorkloadOidcKey) -> Self {
        Self::new(
            request.key_use(),
...
/// Sanitized durable Automata workload OIDC store failure.
#[derive(Clone, Copy, Debug, Eq, Error, PartialEq)]
pub enum WorkloadOidcStoreError {
    /// No current execution authority permits the operation.
    #[error("Automata workload OIDC execution authority is unavailable")]
    Unauthorized,
    /// Immutable proposal or key identity conflicts with durable state.
    #[error("Automata workload OIDC durable identity conflicts")]
    Conflict,
    /// A configured durable count ceiling was reached.
    #[error("Automata workload OIDC durable capacity is exhausted")]
    ResourceExhausted,
    /// Persisted state violates the current contract.
    #[error("Automata workload OIDC durable state is corrupt")]
    CorruptData,
    /// The durable provider is temporarily unavailable.
    #[error("Automata workload OIDC durable state is unavailable")]
    Unavailable,
}

/// Sanitized value-construction failure at the durable OIDC boundary.
#[derive(Clone, Copy, Debug, Eq, Error, PartialEq)]
pub enum WorkloadOidcValueError {
    /// Execution coordinates do not form one exact current identity.
    #[error("Automata workload OIDC execution identity is invalid")]
    InvalidExecution,
    /// Permission or subject-policy evidence is incomplete or incoherent.
    #[error("Automata workload OIDC policy evidence is invalid")]
    InvalidPolicy,
    /// The request-bearer interval is not exact or bounded.
    #[error("Automata workload OIDC request-bearer interval is invalid")]
    InvalidBearerInterval,
    /// A key-retention deadline is invalid.
    #[error("Automata workload OIDC key-retention deadline is invalid")]
    InvalidKeyDeadline,
    /// A loaded key set is empty, duplicated, excessive, or incoherent.
    #[error("Automata workload OIDC loaded-key configuration is invalid")]
    InvalidKeyConfiguration,
}

/// Failure to sample the trusted OIDC currentness clock.
#[derive(Clone, Copy, Debug, Eq, Error, PartialEq)]
#[error("Automata workload OIDC trusted time is unavailable")]
pub struct WorkloadOidcCurrentnessClockError;

/// Trusted clock sampled only after durable OIDC currentness locks are held.
pub trait WorkloadOidcCurrentnessClock: fmt::Debug + Send + Sync {
...
/// Object-safe authority reservation boundary for a current workload OIDC execution.
#[async_trait]
pub trait WorkloadOidcAuthorityRepository: fmt::Debug + Send + Sync {
...
#[async_trait]
pub trait WorkloadOidcKeyRetentionRepository: fmt::Debug + Send + Sync {
    ...
    async fn verify_workload_oidc_key_readiness(
        &self,
        observed_at_seconds: u64,
        loaded_keys: &[WorkloadOidcLoadedKey],
        ...
        for deadline in self
            .required_workload_oidc_keys(observed_at_seconds)
            .await?
        {
