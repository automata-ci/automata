...
async fn active_legacy_instance_refuses_atomically_then_terminal_history_migrates() -> TestResult {
    run_with_unmigrated_database(|database| async move {
        ...
        insert_legacy_activation_instance(&database, seed.run_id.as_uuid()).await?;
        ...
        sqlx::query("ALTER TABLE workflow_runs ENABLE TRIGGER USER")
            .execute(&mut *transaction)
            .await?;
        ...
        let applied: (i64, i64, i64, Option<String>, String) = sqlx::query_as(
            r"
            SELECT
...
async fn assert_0066_absent(database: &TestDatabase) -> TestResult {
    ...
    .fetch_one(database.pool())
    .await?;
    assert_eq!(absent, (0, None, None, None, None, 0));
    Ok(())
}

async fn insert_legacy_activation_instance(database: &TestDatabase, run_id: Uuid) -> TestResult {
    ...
    for table in [
        ...
        let statement = format!("ALTER TABLE {table} DISABLE TRIGGER USER");
    ...
    for table in [
        ...
        let statement = format!("ALTER TABLE {table} ENABLE TRIGGER USER");
        sqlx::query(sqlx::AssertSqlSafe(statement))
            .execute(&mut *transaction)
            .await?;
    }
