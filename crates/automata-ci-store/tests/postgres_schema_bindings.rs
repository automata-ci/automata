...
fn admission_epoch_and_workflow_plan_use_independent_sql_parameters() {
    let postgres = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/postgres");
    ...
    assert!(
        ...
        "expected the Store's independently versioned run-schema queries"
    );

    for (file, minimum_binds) in [
        ("github_job_runtime_authority.rs", 1),
        ("github_oidc.rs", 1),
        ("github_subject_evidence.rs", 2),
        ("logical_activation.rs", 5),
        ("logical_activation_preparation.rs", 2),
        ("logical_instance_result.rs", 1),
        ("logical_job_result.rs", 1),
        ("logical_materialization.rs", 3),
        ("logical_run_finalization.rs", 2),
        ("logical_work_selection.rs", 6),
