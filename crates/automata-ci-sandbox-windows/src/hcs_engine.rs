//! Fixed Windows container-engine implementation of the host-compute boundary.
//!
...
impl WindowsHostComputeAdapter for WindowsEngineHostComputeAdapter {
    ...
    fn create(&self, request: &HostComputeCreateRequest) -> Result<(), HostComputeAdapterError> {
        ...
        let image = self.attest_profile(&HostComputeProfileRequest::from_create(request))?;
...
impl HostComputeProfileRequest {
    fn from_create(request: &HostComputeCreateRequest) -> Self {
        Self::new(request.profile().clone(), request.image().clone())
    }
}
