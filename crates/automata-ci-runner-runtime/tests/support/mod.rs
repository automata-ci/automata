...
#[derive(Debug)]
pub struct BlockedAuthorityClient {
    session_id: automata_ci_core::RunnerSessionId,
    lease: Lease,
    limits: automata_ci_protocol::ProtocolLimits,
...
impl BlockedAuthorityClient {
    pub fn new(fixture: &AcceptedFixture) -> Self {
        Self {
    ...
    pub async fn wait_for_request(&self) {
        loop {
            let changed = self.changed.notified();
            if self.request_started.load(Ordering::SeqCst) {
                return;
            }
            changed.await;
        }
...
impl RunnerRuntimeControlClient for BlockedAuthorityClient {
    fn exchange<'a>(
        &'a self,
        request: &'a PreparedRequest,
        cancellation: CancellationToken,
            ...
            let response = match request.message() {
                RunnerToServer::Hello(hello) => {
                    ...
                    ServerToRunner::Hello(ServerHello::new(
                        ...
                        ServerTiming::new(UnixMillis::new(10_000), 20, 5_000),
                    ))
                }
                RunnerToServer::LeaseResponse(response)
                    if response.disposition()
                ...
                RunnerToServer::RuntimeAuthorityRequest(request) => {
                    assert_eq!(request.binding().attempt_id(), self.lease.attempt_id());
                ...
                RunnerToServer::Heartbeat(heartbeat) => {
                    ...
                    let mut heartbeats = self
                        ...
                        .lock()
                        .unwrap_or_else(PoisonError::into_inner);
                    let ordinal = u64::try_from(heartbeats.len()).unwrap_or(u64::MAX) + 1;
                    heartbeats.push(ordinal);
                    self.changed.notify_waiters();
                    ServerToRunner::LeaseRenewal(LeaseRenewal::new(
                        reply_header(heartbeat.header()),
