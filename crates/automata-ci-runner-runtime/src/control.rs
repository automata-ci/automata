...
use automata_ci_protocol::{ProtocolLimits, ServerToRunner, ValidatedServerToRunner};
...
impl RuntimeControlReply {
    ...
    pub fn from_message(
        ...
        let canonical_bytes = automata_ci_protocol_protobuf::encode_server_frame(&message, limits)
    ...
    pub(crate) fn from_transport(reply: automata_ci_runner_transport::ControlReply) -> Self {
...
pub enum RuntimeControlReplyError {
    ...
    Encode(#[source] automata_ci_protocol_protobuf::EncodeError),
    /// Domain validation failed.
    #[error("runtime control reply validation failed")]
    Validation(#[source] automata_ci_protocol::MessageValidationError),
}
