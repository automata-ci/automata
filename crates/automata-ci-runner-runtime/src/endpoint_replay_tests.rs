...
impl FakeProvider {
    fn new(inspection: SandboxInspection, restart_safe: bool) -> Self {
        ...
        if restart_safe {
            capabilities.push(SandboxCapability::RestartSafeEndpointReplay);
        }
        Self {
...
fn completed_result_replays_without_backend_and_result_wins_later_cancellation() {
    let fixture = Fixture::new("completed", false);
...
fn secret_bearing_requests_never_serialize_into_journal_spool_identity_or_debug() {
    let fixture = Fixture::new("request-secret-redaction", false);
    let secret = "pin-0427";
...
fn multiple_bindings_share_one_host_operation_linearization_gate() {
    let fixture = Fixture::new("multiple-bindings", false);
...
#[test]
fn ambiguous_invocation_fails_closed_without_restart_safe_provider() {
    let fixture = Fixture::new("ambiguous", false);
    let operation_id = OperationId::new();
...
#[test]
fn cancelling_recovered_invocation_keeps_ambiguity_pending_until_sandbox_absence() {
    let fixture = Fixture::new("ambiguous-cancellation", true);
    let operation_id = OperationId::new();
...
fn termination_is_durable_before_backend_observes_it() {
    let fixture = Fixture::new("termination", false);
    let operation_id = OperationId::new();
...
fn pre_invocation_termination_resolves_acceptance_without_backend_exposure() {
    let fixture = Fixture::new("pre-invocation-termination", false);
...
#[test]
fn restart_safe_replay_requires_fresh_exact_live_generation() {
    let fixture = Fixture::new("generation-proof", true);
    let operation_id = OperationId::new();
    let request = Fixture::request(operation_id, "/workspace/replay");
    fixture
        .spool_fault
        .arm_after(ContentCommitStage::FileSynced, 1);
    fixture
        .endpoint()
        .copy_from(&request, &automata_ci_execution::NeverCancelled)
        .expect_err("seed invocation-committed operation");
    assert_eq!(fixture.calls.load(Ordering::SeqCst), 1);

    let stopped = SandboxInspection::new(
    ...
    *fixture.provider.inspection.lock().expect("inspection lock") = stopped;
    let replay = fixture
        .endpoint()
        .copy_from(&request, &automata_ci_execution::NeverCancelled);
    assert_eq!(
        replay.expect_err("stopped generation fails closed").kind(),
        ExecutionErrorKind::InvalidState
    );
    assert_eq!(fixture.provider.inspections.load(Ordering::SeqCst), 2);
    assert_eq!(fixture.calls.load(Ordering::SeqCst), 1);
}

#[test]
fn restart_safe_provider_reexecutes_only_the_exact_request_on_the_same_live_generation() {
    let fixture = Fixture::new("restart-safe-exact", true);
    let operation_id = OperationId::new();
    let request = Fixture::request(operation_id, "/workspace/exact");
    fixture
        .spool_fault
        .arm_after(ContentCommitStage::FileSynced, 1);
    let first = fixture
        ...
        .copy_from(&request, &automata_ci_execution::NeverCancelled);
    assert_eq!(
        first.expect_err("lose first result publication").kind(),
        ExecutionErrorKind::LocalStorage
    );
    let replay = fixture
        .endpoint()
        .copy_from(&request, &automata_ci_execution::NeverCancelled)
        .expect("exact same-generation retry");
    assert_eq!(replay, b"protected replay result");
    assert_eq!(fixture.calls.load(Ordering::SeqCst), 2);
    let conflict = fixture.endpoint().copy_from(
        &Fixture::request(operation_id, "/workspace/not-exact"),
        &automata_ci_execution::NeverCancelled,
    );
    assert_eq!(
        conflict
            .expect_err("different request cannot reuse result")
            .kind(),
        ExecutionErrorKind::InvalidState
    );
    assert_eq!(fixture.calls.load(Ordering::SeqCst), 2);
...
fn request_spool_or_acceptance_fault_never_reaches_the_backend() {
    let spool_fault = Fixture::new("request-spool-fault", false);
    ...
    let journal_fault = Fixture::new("request-journal-fault", false);
...
fn invocation_commit_fault_never_reaches_the_backend() {
    let fixture = Fixture::new("invocation-commit-fault", false);
    let operation_id = OperationId::new();
...
fn result_journal_fault_preserves_payload_first_ambiguous_state() {
    let fixture = Fixture::new("result-journal-fault", true);
    let operation_id = OperationId::new();
...
fn pre_invocation_cancellation_storage_failure_never_reaches_provider_or_backend() {
    let fixture = Fixture::new("pre-invocation-cancellation-journal-fault", false);
