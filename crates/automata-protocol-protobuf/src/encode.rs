...
/// [`automata_runner_spool::ContentProtector`](https://docs.rs/) boundary by
/// the caller; this adapter keeps no credential copy after encoding.
///
/// # Errors
///
...
fn job_ir(value: &core::JobIr) -> wire::JobIr {
    wire::JobIr {
        ...
        working_directory: value.working_directory().map(str::to_owned),
...
fn step_ir(value: &core::StepIr) -> wire::StepIr {
    wire::StepIr {
        ...
        timeout_seconds: value.timeout_seconds(),
        environment: value_entries(value.environment()),
        kind: Some(semantic_step(value.kind())),
    }
}
...
fn semantic_step(value: &core::SemanticStep) -> wire::SemanticStep {
    use wire::semantic_step::Value;
    let value = match value {
        core::SemanticStep::Run {
            command,
            shell,
            working_directory,
        } => Value::Run(wire::RunStep {
            command: command.clone(),
            shell: Some(shell_spec(shell)),
...
fn shell_spec(value: &core::ShellSpec) -> wire::ShellSpec {
    use wire::shell_spec::Value;
    let value = match value {
        core::ShellSpec::Default => Value::DefaultShell(wire::Unit {}),
        core::ShellSpec::Named(item) => Value::Named(item.clone()),
        core::ShellSpec::CommandTemplate(item) => Value::CommandTemplate(item.clone()),
    };
    wire::ShellSpec { value: Some(value) }
}
...
fn job_result(value: &core::JobResult) -> wire::JobResult {
    wire::JobResult {
        ...
        outputs: string_entries(value.outputs()),
