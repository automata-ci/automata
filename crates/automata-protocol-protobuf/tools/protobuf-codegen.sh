...
cleanup() {
    ...
    if [[ ! -d "${cleanup_path}" || -L "${cleanup_path}" \
        ...
        || "${cleanup_path}" != "${scratch_parent}"/automata-protocol-protobuf-codegen.* ]]; then
...
umask 077
scratch="$(mktemp -d "${scratch_parent}/automata-protocol-protobuf-codegen.XXXXXXXX")"
scratch="$(
    automata_canonical_exact_target_child \
        "${scratch}" \
        "protobuf codegen scratch directory"
)"
[[ "${scratch}" == "${scratch_parent}"/automata-protocol-protobuf-codegen.* ]] || {
