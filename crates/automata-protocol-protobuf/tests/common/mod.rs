...
use automata_core::{
    ...
    ExpressionLogical, ExpressionProgram, FencingToken, IsolationLevel, JobConclusion, JobId,
    JobIr, JobIrEnvelope, JobLifecycle, JobResult, JobSource, Lease, LeaseGuard, LeaseId, LogAck,
    LogChannel, LogFrame, LogSequence, LogStreamId, MountSource, OperatingSystem, OperationId,
    ResourceCapacity, RunId, RunnerCapabilities, RunnerFeature, RunnerGroup, RunnerId, RunnerLabel,
    RunnerPlatform, RunnerRequirements, RunnerSessionId, SandboxCapabilities, SandboxFeature,
    SemanticStep, Sha256Digest, ShellSpec, StepId, StepIr, StepResult, TransportProtocol,
    UnixMillis, ValueSource, VolumeMount, WorkflowId,
};
        ...
        (
            ...
            RunnerToServer::Hello(
                RunnerHello::new(
                    ...
                    automata_core::JobIrVersionRange::current(),
                    capabilities(),
...
pub fn rich_job() -> JobIrEnvelope {
    ...
    .with_condition(github_condition("github.ref == 'refs/heads/main'"))
    .with_timeout_seconds(1_800)
    .with_environment(step_environment)
    .with_working_directory("source")
    ...
    JobIrEnvelope::new(
        ...
        automata_core::JobExecutionContext::new(
            ...
            automata_core::JobContentReference::new(
                ...
                automata_core::Sha256Digest::from_bytes([0xa5; 32]),
                2,
                "application/json",
            ),
        )
    ...
    vec![
        StepIr::new(
            ...
            SemanticStep::Run {
                ...
                working_directory: Some("source".to_owned()),
            },
        )
        ...
        StepIr::new(
            ...
            "Named shell",
            SemanticStep::run("printf '%s\\n' named", ShellSpec::Named("bash".to_owned())),
        ),
        StepIr::new(
            ...
            SemanticStep::run(
                "printf '%s\\n' template",
                ShellSpec::CommandTemplate("bash -e {0}".to_owned()),
            ),
        ),
        StepIr::new(
...
fn github_condition(source: &str) -> ExpressionProgram {
    ExpressionProgram::new(
        dialect(),
        source,
        vec![
            ...
            ExpressionInstruction::Literal {
                value: ExpressionLiteral::String {
                    value: "refs/heads/main".to_owned(),
                },
            },
            ExpressionInstruction::Compare {
                operator: ExpressionComparison::Equal,
            },
...
fn rich_container(image: &str) -> ContainerSpec {
    ContainerSpec::new(image)
        ...
        .with_ports([
            ContainerPort::new(5432, TransportProtocol::Tcp),
            ContainerPort::new(5353, TransportProtocol::Udp),
        ])
        .with_volumes([
            VolumeMount::new(
...
fn job_result(attempt: AttemptId) -> JobResult {
    ...
    .with_outputs(BTreeMap::from([
        ("artifact-digest".to_owned(), "abc123".to_owned()),
