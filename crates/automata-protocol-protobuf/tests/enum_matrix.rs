...
use automata_core::{
    ...
    OperatingSystem, OperationId, RunnerCapabilities, RunnerId, RunnerPlatform, RunnerSessionId,
    SandboxCapabilities, UnixMillis,
};
...
fn every_handshake_and_session_disposition_variant_round_trips() {
    ...
    for (disposition, cursor) in [
        ...
        let cursor = automata_protocol::CommandCursor::through(cursor);
...
fn every_job_conclusion_variant_round_trips() {
    for conclusion in [
        ...
        roundtrip_runner(&RunnerToServer::JobResult(JobResultMessage::new(
            ...
            common::guard(),
            JobResult::new(attempt, conclusion, UnixMillis::new(10)),
        )));
    }
}
