...
/// Pure verifier for GitHub push webhooks.
///
...
impl GithubWebhookVerifier {
    ...
    pub fn verify(
        ...
        self.verify_bounded(headers, raw_body)
    }
        ...
        C: AsRef<[u8]>,
    {
        ...
        self.verify_bounded(headers, raw_body.freeze())
    }
    ...
    fn verify_bounded(
        ...
        if headers.request.event_name.as_ref() != "push" {
            return Err(GithubWebhookError::UnsupportedEvent);
        }

        let payload: PushPayload =
            serde_json::from_slice(&raw_body).map_err(|_| GithubWebhookError::MalformedPayload)?;
        normalize_push(headers.request, raw_body, payload)
    }
}
...
pub enum GithubWebhookError {
    ...
    /// The event is validly signed but is not a supported push event.
    ...
    MalformedPayload,
    /// Authenticated provider fields violate push invariants.
    #[error("the GitHub push payload is inconsistent")]
    InvalidPayload,
}
...
fn normalize_push(
    ...
    let repository_id = durable_provider_id(payload.repository.id)?;
    let repository_owner_id = durable_provider_id(payload.repository.owner.id)?;
    let repository_visibility = normalized_repository_visibility(
        payload.repository.private,
        &payload.repository.visibility,
    )?;
    validate_repository_component(&payload.repository.owner.login)?;
    validate_repository_component(&payload.repository.name)?;
    if has_ascii_case_insensitive_suffix(&payload.repository.name, ".git") {
    ...
    if payload.repository.full_name != expected_full_name {
        return Err(GithubWebhookError::InvalidPayload);
    }
    ...
    Ok(VerifiedGithubPush {
        ...
        repository: GithubPushRepository {
            ...
            owner: payload.repository.owner.login.into_boxed_str(),
            name: payload.repository.name.into_boxed_str(),
            full_name: payload.repository.full_name.into_boxed_str(),
        },
        git_ref,
...
fn durable_provider_id(value: u64) -> Result<NonZeroU64, GithubWebhookError> {
...
fn validate_stored_identity(
    ...
    if i64::try_from(evidence.installation_id).is_err()
        || evidence.installation_id == 0
        || i64::try_from(evidence.repository_id).is_err()
        || evidence.repository_id == 0
        || i64::try_from(evidence.repository_owner_id).is_err()
        || evidence.repository_owner_id == 0
        || !valid_delivery_id(evidence.delivery_id.as_bytes())
        || validate_repository_component(&evidence.repository_owner).is_err()
        || validate_repository_component(&evidence.repository_name).is_err()
        || has_ascii_case_insensitive_suffix(&evidence.repository_name, ".git")
    {
...
fn parse_git_ref(value: String) -> Result<GithubPushRef, GithubWebhookError> {
