...
impl ResourceNames {
    ...
    pub(crate) fn from_handle(
    ...
    ) -> Result<Self, automata_execution::ProviderError> {
        if handle.provider() != provider_id {
            return Err(provider_error::ownership_mismatch(
                automata_execution::ProviderStage::Validate,
            ));
        }
        ...
        OperationId::from_str(&hyphenated).map_err(|_| {
            provider_error::invalid_state(automata_execution::ProviderStage::Validate)
        })?;
