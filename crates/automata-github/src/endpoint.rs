...
use automata_auth::{
    github::{
        ...
        GithubMembershipSnapshot, GithubOrganizationName, GithubTeam, GithubTeamSlug,
...
impl GithubHttpEndpoint {
    ...
    ) -> Result<BTreeSet<GithubOrganizationName>, GithubEndpointError> {
        ...
        loop {
            ...
            for membership in memberships {
                ...
                organizations.insert(
                    GithubOrganizationName::new(membership.organization.login)
                        .map_err(|_| GithubEndpointError::InvalidResponse)?,
                );
            }
    ...
    async fn teams(
        ...
        token: &SecretString,
        budget: &mut PageBudget,
    ) -> Result<BTreeSet<GithubTeam>, GithubEndpointError> {
        ...
        loop {
            ...
            budget.consume_items(memberships.len())?;
            for membership in memberships {
                teams.insert(GithubTeam {
                    organization: GithubOrganizationName::new(membership.organization.login)
                        .map_err(|_| GithubEndpointError::InvalidResponse)?,
                    slug: GithubTeamSlug::new(membership.slug)
                        .map_err(|_| GithubEndpointError::InvalidResponse)?,
                });
            }
            let Some(next) = next_page(
