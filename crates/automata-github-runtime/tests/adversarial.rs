...
fn malformed_names_and_scope_identifiers_fail_closed() {
    ...
    assert!(automata_github_runtime::StepId::new("").is_err());
    assert!(automata_github_runtime::StepId::new("has whitespace").is_err());
    assert!(automata_github_runtime::ActionInvocationId::new("line\nbreak").is_err());
}
