...
use automata_blob::ImmutableBlobStore as _;
...
use automata_results_github::{
    ArtifactName, ArtifactPublicationState, ArtifactRepository as _, ArtifactService,
    ExecutionAuthority, PostgresArtifactRepository, ResultsClock, ResultsIdGenerator,
    ResultsLimits, UploadId,
};
use automata_store::{
...
async fn finalized_manifest_and_blocks_are_verified_in_rustfs() -> TestResult {
    let Some(endpoint) = env::var_os("AUTOMATA_TEST_S3_ENDPOINT") else {
        return Ok(());
    };
    run_with_database(|database| async move {
        let authority = active_attempt(&database).await?;
        let endpoint = Url::parse(&endpoint.to_string_lossy())?;
        ...
        let published = match repository
            .publication_state(authority, &ArtifactName::new("rustfs-dist", 255)?)
            .await?
        {
            ArtifactPublicationState::Published(published) => published,
            ArtifactPublicationState::Committed(_) => panic!("artifact must be published"),
        };
