...
use crate::{
    ExecutionAuthority, ResultsClock, RuntimeTokenClaims, RuntimeTokenIssuer, RuntimeTokenVerifier,
    SignedUploadCapability, TokenError, UploadId,
};
use automata_protocol::RuntimeAuthorityEndpoint;
...
pub struct HmacResultsAuthorityConfig {
    ...
    maximum_upload_lifetime_seconds: u64,
...
impl HmacResultsAuthorityConfig {
    ...
    pub fn new(
        ...
        if maximum_token_lifetime_seconds == 0 || maximum_upload_lifetime_seconds == 0 {
            return Err(TokenError::Policy);
        }
        Ok(Self {
            issuer,
            ...
            maximum_upload_lifetime_seconds,
...
impl RuntimeTokenIssuer for HmacResultsAuthority {
    fn issue(
        ...
        self.issue_at(authority, issued_at, valid_for_seconds)
    }
}
...
impl SignedUploadCapability for HmacResultsAuthority {
    fn issue_url(&self, upload_id: UploadId, expires_at_seconds: u64) -> Result<Url, TokenError> {
        ...
        let expires_at_seconds =
            expires_at_seconds.min(now.saturating_add(self.config.maximum_upload_lifetime_seconds));
    ...
    fn verify(
        ...
        self.verify_expiry(
            expires_at_seconds,
            self.config.maximum_upload_lifetime_seconds,
        )?;
        let signature =
