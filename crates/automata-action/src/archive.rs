...
use automata_scm::RepositorySnapshot;
use bytes::Bytes;
...
/// Validates already verified immutable archive bytes and selects one action
/// definition.
///
/// This is the cache-hit counterpart to [`inspect_archive`]. It intentionally
/// repeats the complete gzip, tar, path, link, and resource-limit inspection;
/// an object-store integrity check alone is not treated as semantic archive
/// validation.
///
/// # Errors
///
...
pub fn inspect_archive_bytes(
    ...
    let definitions = inspect_entries(&mut archive, &target, limits)?;

    let mut decoder = archive.into_inner();
    io::copy(&mut decoder, &mut ZeroPaddingVerifier).map_err(|_| ActionArchiveError::Malformed)?;
    definitions
        .into_iter()
        .next()
...
fn inspect_entries<R: io::Read>(
    ...
    limits: ActionBundleLimits,
) -> Result<DefinitionCandidates, ActionArchiveError> {
    ...
    for entry in entries {
        ...
        if let Some(candidate) = target.get(&relative_path) {
            if declared_size > limits.maximum_definition_bytes() {
                return Err(ActionArchiveError::ResourceLimit);
            }
            let bytes =
            ...
            let copied =
                io::copy(&mut entry, &mut io::sink()).map_err(|_| ActionArchiveError::Malformed)?;
