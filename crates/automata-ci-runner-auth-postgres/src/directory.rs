...
///
/// Every lookup closes certificate validity at equality against the database
/// clock before returning authority to a caller.
#[derive(Clone)]
...
impl RunnerMachineDirectory for PostgresRunnerMachineDirectory {
    async fn find_by_leaf_sha256(
        ...
        let certificate_is_current = row
            .try_get::<bool, _>("certificate_is_current")
            .map_err(|_| RunnerMachineDirectoryError::Corrupt)?;
        Ok(certificate_is_current.then_some(record))
    }
}
