...
use automata_ci_runner_results::{
    CacheAccessScope, CachePermission, CacheRepositoryMetadata, HmacResultsAuthority,
    HmacResultsAuthorityConfig, RUNNER_RESULTS_RUNTIME_AUTHORITY, ResultsPublicEndpoint,
    RunnerResultsRuntimeAuthorityIssuer, RuntimeTokenVerifier as _, TokenError,
};
...
async fn deterministic_issuance_replays_exact_bytes_and_binding() {
    ...
    let issuer =
        RunnerResultsRuntimeAuthorityIssuer::new(authority.clone(), 120, [cache_repository()])
...
async fn default_branch_metadata_is_routed_by_exact_repository() {
    ...
    let authority = authority(clock);
    let issuer = RunnerResultsRuntimeAuthorityIssuer::new(
        authority.clone(),
        120,
        [
            cache_repository(),
...
async fn cross_attempt_fence_and_expiry_are_rejected() {
    ...
    let issuer =
        RunnerResultsRuntimeAuthorityIssuer::new(authority.clone(), 60, [cache_repository()])
            .expect("runtime issuer");
    let job = job();
    let lease = make_lease(
        AttemptId::new(),
...
fn closed_private_network_endpoint_is_ipv4_and_results_port_exact() {
    use automata_ci_runner_results::PrivateNetworkResultsEndpoint;
