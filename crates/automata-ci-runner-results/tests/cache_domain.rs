use automata_ci_runner_results::{
    ActionsCacheHttpLimits, CacheAccessScope, CacheAuthority, CacheKey, CacheLimits,
    CachePermission, CacheProtocolEntryId, CacheRepositoryMetadata, CacheVersion,
...
fn repository_scope_and_protocol_values_are_bounded_and_unambiguous() {
    ...
    assert!(ActionsCacheHttpLimits::new(64 * 1024, 128 * 1024 * 1024).is_ok());
    assert!(ActionsCacheHttpLimits::new(64 * 1024 + 1, 1).is_err());
    assert!(ActionsCacheHttpLimits::new(1, 128 * 1024 * 1024 + 1).is_err());
}
