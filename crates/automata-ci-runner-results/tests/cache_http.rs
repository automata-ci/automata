...
use automata_ci_runner_results::{
    ActionsCacheApi, ActionsCacheHttpLimits, CacheAuthority, CacheBlock, CacheEntryId,
    CacheFinalizationPreparation, CacheLimits, CachePermission, CacheProtocolEntryId,
    CacheRepository, CacheRepositoryError, CacheRepositoryErrorKind, CacheService,
    CommitCacheBlocks, CompleteCacheBlock, CompleteCacheFinalization, CreateCacheEntry,
    CreatedCacheEntry, ExecutionAuthority, FinalizedCacheEntry, HmacResultsAuthority,
...
struct MemoryEntry {
    ...
    cache_ref: String,
    key: automata_ci_runner_results::CacheKey,
    version: automata_ci_runner_results::CacheVersion,
...
fn fixture_with_url_and_observer(public_url: &str, observer: Arc<dyn ResultsObserver>) -> Fixture {
    fixture_with_url_observer_and_limits(public_url, observer, ActionsCacheHttpLimits::default())
}

fn fixture_with_limits(limits: ActionsCacheHttpLimits) -> Fixture {
...
fn fixture_with_url_observer_and_limits(
    ...
    let router = ActionsCacheApi::new(service, authority.clone(), authority, limits)
...
async fn cache_router_and_extractor_rejections_are_private() {
    let fixture =
        fixture_with_limits(ActionsCacheHttpLimits::new(64, 8).expect("focused cache HTTP limits"));
