...
use automata_ci_runner_results::{
    ActionsResultsApi, ActionsResultsHttpLimits, ArtifactBlock, ArtifactBlockReservation,
    ArtifactFinalizationClaim, ArtifactFinalizationReservation, ArtifactFinalizationWork,
    ArtifactId, ArtifactRepository, ArtifactRepositoryError, ArtifactRepositoryErrorKind,
    ArtifactService, BeginArtifactFinalization, CommitArtifactBlocks, CommittedArtifact,
    CompleteArtifactBlock, CompleteArtifactFinalization, CreateArtifact, CreateArtifactOutcome,
    ExecutionAuthority, FinalizeArtifactOutcome, HmacResultsAuthority, HmacResultsAuthorityConfig,
    ...
    ResultsPublicEndpoint, RuntimeTokenIssuer as _, UploadId, VerifiedArtifactFinalization,
};
use axum::{
    body::Body,
...
fn fixture_with_url(public_url: &str) -> Fixture {
    fixture_with_url_and_limits(public_url, ActionsResultsHttpLimits::default())
}

fn fixture_with_limits(limits: ActionsResultsHttpLimits) -> Fixture {
    fixture_with_url_and_limits("http://results.automata.localhost:8080/", limits)
}

fn fixture_with_url_and_limits(public_url: &str, limits: ActionsResultsHttpLimits) -> Fixture {
    let now = 1_000_000;
...
async fn artifact_router_and_extractor_rejections_are_private() {
    let fixture = fixture_with_limits(
        ActionsResultsHttpLimits::new(64, 8).expect("focused artifact HTTP limits"),
    );
