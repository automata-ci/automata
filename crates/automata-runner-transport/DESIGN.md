...
`automata.runner.v1`. It is not linked to a product binary or SQL adapter.
...
- The server accepts a pre-bound, dedicated TCP listener and terminates mTLS
  ...
  to `automata_auth::machine::MachineIdentityVerifier` on every request.
...
- TLS 1.3 is the default and recommended policy. TLS 1.2 is an explicit legacy
  interoperability option, never silently enabled.
...
features), http-body-util 0.1.4, and hyper-rustls 0.27.9 (`http2`, `ring`,
`tls12`, no default features), plus tower-service 0.3.3 for the connector's
