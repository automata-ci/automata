...
impl GithubOidcKeyRetentionRepository for PostgresStore {
    ...
    async fn github_oidc_key_deadline(
        ...
        .bind(github_oidc_key_use_name(key_use))
        .bind(key_id.as_str())
...
async fn reserve_oidc_issuance(
    ...
    let subject_policy_mode =
        decode_github_oidc_subject_policy_mode(&authority.subject_policy_mode)?;
    let subject =
    ...
    let computed_claim_evidence = automata_ci_store::adapter_spi::github_oidc_claim_evidence_digest(
...
fn derive_authority_policy(
    ...
    let claim_evidence_sha256 = automata_ci_store::adapter_spi::github_oidc_claim_evidence_digest(
...
async fn insert_authority(
    ...
    .bind(github_oidc_subject_policy_mode_name(
...
fn authority_replay_matches(
    ...
    let proposal = request.proposal();
    let subject_policy_mode =
        decode_github_oidc_subject_policy_mode(&existing.subject_policy_mode)?;
    let subject_policy_revision =
...
fn decode_reserved_authority(
    ...
    Ok(
        automata_ci_store::adapter_spi::reserved_github_oidc_authority(
...
async fn extend_existing_key_in_transaction(
    ...
    .bind(github_oidc_key_use_name(request.key_use()))
...
fn decode_key_deadline(
    ...
    let not_after: i64 = row
        .try_get("max_not_after_seconds")
        .map_err(|_| GithubOidcStoreError::CorruptData)?;
    Ok(automata_ci_store::adapter_spi::github_oidc_key_deadline(
        decode_github_oidc_key_use(&key_use)?,
...
const fn github_oidc_subject_policy_mode_name(mode: GithubOidcSubjectPolicyMode) -> &'static str {
    match mode {
        GithubOidcSubjectPolicyMode::StableOwnerEvidence => "stable_owner_evidence",
    }
}

fn decode_github_oidc_subject_policy_mode(
    value: &str,
) -> Result<GithubOidcSubjectPolicyMode, GithubOidcStoreError> {
    match value {
        "stable_owner_evidence" => Ok(GithubOidcSubjectPolicyMode::StableOwnerEvidence),
        _ => Err(GithubOidcStoreError::CorruptData),
    }
}

const fn github_oidc_key_use_name(key_use: GithubOidcKeyUse) -> &'static str {
    match key_use {
        GithubOidcKeyUse::RequestBearer => "request_bearer",
        GithubOidcKeyUse::IdTokenSigning => "id_token_signing",
    }
}

fn decode_github_oidc_key_use(value: &str) -> Result<GithubOidcKeyUse, GithubOidcStoreError> {
    match value {
        "request_bearer" => Ok(GithubOidcKeyUse::RequestBearer),
        "id_token_signing" => Ok(GithubOidcKeyUse::IdTokenSigning),
        _ => Err(GithubOidcStoreError::CorruptData),
    }
}
