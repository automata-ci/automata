...
async fn revalidate_delivery_consumer(
    ...
    let attempt = i16::try_from(consumer.revision().get())
        .map_err(|_| GithubServerServiceStoreError::HandoffRejected)?;
    sqlx::query_scalar::<_, i64>(
        r"
        ...
        WHERE invocation.invocation_id = $1
          ...
          AND invocation.claim_started_at_ms <= $5
          AND invocation.claim_expires_at_ms > $5
          AND provider_connection.workspace_id = $6
          AND delivery.connection_id = $8
          AND provider_instance.provider_type = 'github'
          AND provider_connection.external_repository_id = $9
          AND repository.owner || '/' || repository.name = $10
          AND (NOT $11 OR delivery.event_type = 'pull_request')
...
#[cfg(feature = "test-support")]
impl PostgresStore {
    /// Revalidates a common trigger lease through the production credential-handoff query.
    ///
    /// # Errors
    ///
    /// Returns the same fail-closed storage and handoff errors as the production revalidation.
    pub async fn revalidate_github_trigger_consumer_for_test(
        &self,
        identity: &GithubServerServiceAuthorityIdentity,
        consumer: GithubServerServiceConsumerClaim,
        observed_at: UnixMillis,
