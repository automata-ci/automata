...
use automata_ci_store::{
    MAX_WORKLOAD_OIDC_ISSUANCE_SLOTS, MAXIMUM_OIDC_KEYS_PER_KEYRING, ReserveWorkloadOidcAuthority,
    ReservedWorkloadOidcAuthority, RetainWorkloadOidcKey, WorkloadOidcAuthorityRepository,
    WorkloadOidcCurrentPolicy, WorkloadOidcCurrentnessClock, WorkloadOidcKeyDeadline,
    ...
    WorkloadOidcValueError,
};

const AUDIENCE_SLOT_DOMAIN: &[u8] = b"automata/workload-oidc/audience-slot:v1\0";
...
struct DerivedAuthorityPolicy {
    ...
    current_policy: WorkloadOidcCurrentPolicy,
...
#[derive(Clone)]
pub struct PostgresWorkloadOidcAuthorityRepository {
    store: PostgresStore,
    clock: Arc<dyn WorkloadOidcCurrentnessClock>,
}

impl PostgresWorkloadOidcAuthorityRepository {
    /// Creates an authority adapter whose clock is sampled only after all
    /// mutable currentness dependencies have been locked.
    #[must_use]
    pub fn new(store: PostgresStore, clock: Arc<dyn WorkloadOidcCurrentnessClock>) -> Self {
        Self { store, clock }
    }
}

impl fmt::Debug for PostgresWorkloadOidcAuthorityRepository {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("PostgresWorkloadOidcAuthorityRepository")
...
#[derive(Clone)]
pub struct PostgresWorkloadOidcIssuanceRepository {
    store: PostgresStore,
    current_policy: WorkloadOidcCurrentPolicy,
...
impl PostgresWorkloadOidcIssuanceRepository {
    ...
    pub fn new(
        store: PostgresStore,
        current_policy: WorkloadOidcCurrentPolicy,
        signing_keys: impl IntoIterator<Item = WorkloadOidcLoadedKey>,
        clock: Arc<dyn WorkloadOidcCurrentnessClock>,
    ) -> Result<Self, WorkloadOidcValueError> {
        if current_policy.subject_policy_mode()
            != WorkloadOidcSubjectPolicyMode::StableOwnerEvidence
        {
            return Err(WorkloadOidcValueError::InvalidPolicy);
        }
        let mut configured = BTreeMap::new();
        for key in signing_keys {
            if key.key_use() != WorkloadOidcKeyUse::IdTokenSigning
                || configured.len() >= MAXIMUM_OIDC_KEYS_PER_KEYRING
                || configured
                    .insert(key.key_id().clone(), key.key_sha256())
                    .is_some()
            {
                return Err(WorkloadOidcValueError::InvalidKeyConfiguration);
            }
        }
    ...
    pub async fn verify_workload_oidc_key_readiness(
        &self,
        observed_at_seconds: u64,
        request_bearer_keys: &[WorkloadOidcLoadedKey],
    ) -> Result<(), WorkloadOidcStoreError> {
        if request_bearer_keys.len() > MAXIMUM_OIDC_KEYS_PER_KEYRING {
            return Err(WorkloadOidcStoreError::ResourceExhausted);
        }
        if request_bearer_keys.is_empty()
            || request_bearer_keys
                .iter()
                .any(|key| key.key_use() != WorkloadOidcKeyUse::RequestBearer)
        {
            return Err(WorkloadOidcStoreError::Conflict);
        }
        ...
        self.store
            .verify_workload_oidc_key_readiness(observed_at_seconds, &loaded)
            .await
    }
}

impl fmt::Debug for PostgresWorkloadOidcIssuanceRepository {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("PostgresWorkloadOidcIssuanceRepository")
            .field("store", &"[CONFIGURED]")
...
#[async_trait]
impl WorkloadOidcAuthorityRepository for PostgresWorkloadOidcAuthorityRepository {
    async fn reserve_workload_oidc_authority(
        &self,
        request: ReserveWorkloadOidcAuthority,
    ) -> Result<ReservedWorkloadOidcAuthority, WorkloadOidcStoreError> {
        ...
        let reserved = if inserted {
            ...
            let retention = RetainWorkloadOidcKey::request_bearer(
                ...
                u64::try_from(fresh_observed_at)
                    .map_err(|_| WorkloadOidcStoreError::CorruptData)?
                    / 1_000,
            )
            .map_err(|_| WorkloadOidcStoreError::CorruptData)?;
            ...
            lock_and_observe_current_authority(
                ...
                request
                    ...
                    .get()
                    .checked_add(1)
                    .ok_or(WorkloadOidcStoreError::CorruptData)?,
            ...
            let existing = lock_reserved_authority(&mut transaction, attempt_id, fencing_token)
                .await?
                .ok_or(WorkloadOidcStoreError::Conflict)?;
...
#[async_trait]
impl WorkloadOidcKeyRetentionRepository for PostgresStore {
    async fn retain_workload_oidc_key(
        &self,
        request: RetainWorkloadOidcKey,
    ...
    async fn workload_oidc_key_deadline(
        ...
        sqlx::query(
            ...
            FROM workload_oidc_key_deadlines
            WHERE key_use = $1 AND key_id = $2
            ",
        )
        .bind(workload_oidc_key_use_name(key_use))
        .bind(key_id.as_str())
    ...
    async fn required_workload_oidc_keys(
        ...
        for row in &rows {
            ...
            match deadline.key_use() {
                WorkloadOidcKeyUse::RequestBearer => request_bearers += 1,
                WorkloadOidcKeyUse::IdTokenSigning => signing_keys += 1,
            }
...
#[async_trait]
impl OidcIssuanceRepository for PostgresWorkloadOidcIssuanceRepository {
    async fn reserve(
        &self,
...
async fn reserve_oidc_issuance(
    repository: &PostgresWorkloadOidcIssuanceRepository,
    request: ReserveOidcIssuance,
) -> Result<AuthorizedOidcIssuance, WorkloadOidcStoreError> {
    ...
    .await?;
    let authorized_at_seconds = u64::try_from(fresh_observed_at_ms)
        .map_err(|_| WorkloadOidcStoreError::CorruptData)?
        / 1_000;
    if authorized_at_seconds >= request.maximum_expires_at_seconds()
        || authorized_at_seconds >= request.request_expires_at_seconds()
    {
        return Err(WorkloadOidcStoreError::Unauthorized);
    }
    let subject_policy_revision =
        WorkloadOidcSubjectPolicyRevision::new(u64_from_i64(authority.subject_policy_revision)?)
            .map_err(|_| WorkloadOidcStoreError::CorruptData)?;
    let subject_policy_mode =
        decode_workload_oidc_subject_policy_mode(&authority.subject_policy_mode)?;
    let subject =
        OidcSubject::new(authority.subject).map_err(|_| WorkloadOidcStoreError::CorruptData)?;
    ...
    let computed_claim_evidence =
        automata_ci_store::adapter_spi::workload_oidc_claim_evidence_digest(
    ...
    let (issuance, is_live_replay) = if let Some(slot) = existing {
        ...
        let count: i64 = sqlx::query_scalar(
            "SELECT count(*) FROM workload_oidc_issuance_slots WHERE authority_id = $1",
        )
        .bind(request.authority_id().as_uuid())
        .fetch_one(&mut *transaction)
        .await
        .map_err(oidc_operation_error)?;
        if usize::try_from(count).map_err(|_| WorkloadOidcStoreError::CorruptData)?
            >= MAX_WORKLOAD_OIDC_ISSUANCE_SLOTS
        {
            return Err(WorkloadOidcStoreError::ResourceExhausted);
        }
    ...
    .await?;
    let final_authorized_at_seconds = u64::try_from(final_observed_at_ms)
        .map_err(|_| WorkloadOidcStoreError::CorruptData)?
        / 1_000;
...
async fn lock_current_execution(
    ...
    request: &ReserveWorkloadOidcAuthority,
) -> Result<CurrentExecutionRow, WorkloadOidcStoreError> {
...
fn post_lock_observation(
    clock: &dyn WorkloadOidcCurrentnessClock,
    request: &ReserveWorkloadOidcAuthority,
) -> Result<i64, WorkloadOidcStoreError> {
    let observed_at_ms = clock
        .now_millis()
        .map_err(|_| WorkloadOidcStoreError::Unavailable)?
        .get();
...
fn derive_authority_policy(
    request: &ReserveWorkloadOidcAuthority,
    current: &CurrentExecutionRow,
) -> Result<DerivedAuthorityPolicy, WorkloadOidcStoreError> {
    let current_policy = request.current_policy();
    if current_policy.subject_policy_mode() != WorkloadOidcSubjectPolicyMode::StableOwnerEvidence
    ...
    let subject = OidcSubject::new(subject).map_err(|_| WorkloadOidcStoreError::CorruptData)?;
    ...
    let claim_evidence_sha256 = automata_ci_store::adapter_spi::workload_oidc_claim_evidence_digest(
...
async fn insert_authority(
...
) -> Result<bool, WorkloadOidcStoreError> {
    let schemas = current_durable_schemas();
    let execution = request.execution();
    let lease = execution.lease();
    let session = execution.session();
    ...
    let inserted = sqlx::query(
        r"
        INSERT INTO workload_oidc_authorities (
    ...
    .bind(workload_oidc_subject_policy_mode_name(
        policy.current_policy.subject_policy_mode(),
    ))
    .bind(positive_i64(
...
async fn lock_reserved_authority(
    ...
    attempt_id: Uuid,
    fencing_token: i64,
) -> Result<Option<ReservedAuthorityRow>, WorkloadOidcStoreError> {
    let row = sqlx::query(
        r"
...
impl ReservedAuthorityRow {
    fn decode(row: &sqlx::postgres::PgRow) -> Result<Self, WorkloadOidcStoreError> {
        macro_rules! field {
            ($name:literal) => {
                row.try_get($name)
                    .map_err(|_| WorkloadOidcStoreError::CorruptData)?
...
fn authority_replay_matches(
    ...
    let proposal = request.proposal();
    let subject_policy_mode =
        decode_workload_oidc_subject_policy_mode(&existing.subject_policy_mode)?;
    let subject_policy_revision =
        WorkloadOidcSubjectPolicyRevision::new(u64_from_i64(existing.subject_policy_revision)?)
            .map_err(|_| WorkloadOidcStoreError::CorruptData)?;
...
fn decode_reserved_authority(
    ...
    Ok(
        automata_ci_store::adapter_spi::reserved_workload_oidc_authority(
...
fn proposal_result(request: &ReserveWorkloadOidcAuthority) -> ReservedWorkloadOidcAuthority {
    let proposal = request.proposal();
    automata_ci_store::adapter_spi::reserved_workload_oidc_authority(
...
) -> Result<MintAuthorityRow, WorkloadOidcStoreError> {
    let row = sqlx::query(
        r"
...
async fn lock_and_observe_current_authority(
    ...
    let locked: bool = sqlx::query_scalar(
        r"
        SELECT automata_lock_workload_oidc_authority_dependencies(authority)
        FROM workload_oidc_authorities AS authority
    ...
    let current: bool = sqlx::query_scalar(
        r"
        SELECT automata_workload_oidc_authority_is_current(authority, $2, $3)
        FROM workload_oidc_authorities AS authority
        WHERE authority.authority_id = $1
        ",
    )
...
) -> Result<Option<IssuanceSlotRow>, WorkloadOidcStoreError> {
    let row = sqlx::query(
        r"
...
async fn insert_issuance_slot(
    ...
    sqlx::query(
        r"
        INSERT INTO workload_oidc_issuance_slots (
...
async fn replace_issuance_slot(
    ...
    let generation = old_generation
        .checked_add(1)
        .ok_or(WorkloadOidcStoreError::ResourceExhausted)?;
    let result = sqlx::query(
        r"
        UPDATE workload_oidc_issuance_slots
...
fn decode_live_issuance(
    ...
    authorized_at_seconds: u64,
) -> Result<OidcIssuance, WorkloadOidcStoreError> {
    let durable_audience = OidcAudience::new(slot.resolved_audience)
        .map_err(|_| WorkloadOidcStoreError::CorruptData)?;
    ...
    OidcIssuance::new(
        ...
        OidcTokenId::from_uuid(slot.token_id).map_err(|_| WorkloadOidcStoreError::CorruptData)?,
        OidcKeyId::new(slot.signing_key_id).map_err(|_| WorkloadOidcStoreError::CorruptData)?,
        subject.clone(),
        durable_audience,
        claims.clone(),
        issued_at,
...
async fn ensure_signing_key_retained(
    ...
    let fingerprint = signing_keys
        ...
        .copied()
        .ok_or(WorkloadOidcStoreError::Conflict)?;
    let retention = RetainWorkloadOidcKey::id_token_signing(
...
async fn ensure_request_bearer_key_retained(
    ...
    let required = u64_from_i64(authority.request_bearer_exp_seconds)?
        .checked_add(skew)
        .ok_or(WorkloadOidcStoreError::CorruptData)?;
    let deadline = lock_key_deadline(transaction, WorkloadOidcKeyUse::RequestBearer, &key_id)
        .await?
        .ok_or(WorkloadOidcStoreError::CorruptData)?;
...
async fn extend_existing_key_in_transaction(
    ...
    .bind(workload_oidc_key_use_name(request.key_use()))
    ...
    let updated = sqlx::query(
        r"
        UPDATE workload_oidc_key_deadlines
...
async fn retain_key_in_transaction(
    ...
    if let Some(row) = existing.as_ref() {
        ...
        sqlx::query(
            r"
            INSERT INTO workload_oidc_key_deadlines (
...
async fn ensure_active_key_bound(
    ...
    let active_count: i64 = sqlx::query_scalar(
        r"
        SELECT count(*) FROM workload_oidc_key_deadlines
    ...
    .await
    .map_err(oidc_operation_error)?;
    if usize::try_from(active_count).map_err(|_| WorkloadOidcStoreError::CorruptData)?
...
async fn lock_key_deadline_row(
    transaction: &mut Transaction<'_, Postgres>,
    request: &RetainWorkloadOidcKey,
) -> Result<Option<LockedKeyDeadlineRow>, WorkloadOidcStoreError> {
    let row = sqlx::query(
        r"
        SELECT key_sha256, max_not_after_seconds, updated_at_seconds
        FROM workload_oidc_key_deadlines
...
fn decode_key_deadline(
    ...
    let not_after: i64 = row
        .try_get("max_not_after_seconds")
        .map_err(|_| WorkloadOidcStoreError::CorruptData)?;
    Ok(automata_ci_store::adapter_spi::workload_oidc_key_deadline(
        decode_workload_oidc_key_use(&key_use)?,
        OidcKeyId::new(key_id).map_err(|_| WorkloadOidcStoreError::CorruptData)?,
        key_sha256.as_deref().map(digest).transpose()?,
        u64_from_i64(not_after)?,
    ))
}

const fn workload_oidc_subject_policy_mode_name(
    mode: WorkloadOidcSubjectPolicyMode,
) -> &'static str {
    match mode {
        WorkloadOidcSubjectPolicyMode::StableOwnerEvidence => "stable_owner_evidence",
    }
}

fn decode_workload_oidc_subject_policy_mode(
    value: &str,
) -> Result<WorkloadOidcSubjectPolicyMode, WorkloadOidcStoreError> {
    match value {
        "stable_owner_evidence" => Ok(WorkloadOidcSubjectPolicyMode::StableOwnerEvidence),
        _ => Err(WorkloadOidcStoreError::CorruptData),
    }
}

const fn workload_oidc_key_use_name(key_use: WorkloadOidcKeyUse) -> &'static str {
    match key_use {
        WorkloadOidcKeyUse::RequestBearer => "request_bearer",
        WorkloadOidcKeyUse::IdTokenSigning => "id_token_signing",
    }
}

fn decode_workload_oidc_key_use(value: &str) -> Result<WorkloadOidcKeyUse, WorkloadOidcStoreError> {
    match value {
        "request_bearer" => Ok(WorkloadOidcKeyUse::RequestBearer),
        "id_token_signing" => Ok(WorkloadOidcKeyUse::IdTokenSigning),
        _ => Err(WorkloadOidcStoreError::CorruptData),
    }
}

fn validate_mint_request(request: &ReserveOidcIssuance) -> Result<(), WorkloadOidcStoreError> {
...
fn decode_claims(value: &Value) -> Result<OidcClaimSet, WorkloadOidcStoreError> {
    let object = value
        .as_object()
        .ok_or(WorkloadOidcStoreError::CorruptData)?;
    let mut claims = Vec::with_capacity(object.len());
    for (name, value) in object {
        let value = value.as_str().ok_or(WorkloadOidcStoreError::CorruptData)?;
        claims.push((name.clone(), value.to_owned()));
    }
    OidcClaimSet::new(claims).map_err(|_| WorkloadOidcStoreError::CorruptData)
}
...
fn map_repository_error(error: WorkloadOidcStoreError) -> OidcRepositoryError {
    let kind = match error {
        WorkloadOidcStoreError::Unauthorized => OidcRepositoryErrorKind::Unauthorized,
        WorkloadOidcStoreError::Conflict => OidcRepositoryErrorKind::Conflict,
        WorkloadOidcStoreError::ResourceExhausted => OidcRepositoryErrorKind::ResourceExhausted,
        WorkloadOidcStoreError::CorruptData => OidcRepositoryErrorKind::CorruptData,
        WorkloadOidcStoreError::Unavailable => OidcRepositoryErrorKind::Unavailable,
    };
...
fn oidc_operation_error(error: sqlx::Error) -> WorkloadOidcStoreError {
    match &error {
        sqlx::Error::Io(_)
        | sqlx::Error::PoolTimedOut
        | sqlx::Error::PoolClosed
        | sqlx::Error::WorkerCrashed => WorkloadOidcStoreError::Unavailable,
        sqlx::Error::Database(database) => {
            let constraint = database.constraint().unwrap_or_default();
            if matches!(
                constraint,
                "workload_oidc_authority_current_execution"
                    | "workload_oidc_issuance_current_authority"
            ) {
                return WorkloadOidcStoreError::Unauthorized;
            }
            if constraint == "workload_oidc_issuance_slot_bound" {
                return WorkloadOidcStoreError::ResourceExhausted;
            }
            let code = database.code().unwrap_or_default();
            if code == "23505" {
                WorkloadOidcStoreError::Conflict
            } else if code.starts_with("08")
                || matches!(code.as_ref(), "40001" | "40P01" | "55P03" | "57014")
            {
                WorkloadOidcStoreError::Unavailable
            } else {
                WorkloadOidcStoreError::CorruptData
            }
        }
        _ => WorkloadOidcStoreError::CorruptData,
    }
}

fn digest(bytes: &[u8]) -> Result<Sha256Digest, WorkloadOidcStoreError> {
...
fn seconds_i64(seconds: u64) -> Result<i64, WorkloadOidcStoreError> {
    i64::try_from(seconds).map_err(|_| WorkloadOidcStoreError::CorruptData)
}

fn seconds_to_millis(seconds: u64) -> Result<i64, WorkloadOidcStoreError> {
    seconds
        .checked_mul(1_000)
...
fn whole_second_millis(seconds: u64) -> Result<(i64, i64), WorkloadOidcStoreError> {
...
fn u64_from_i64(value: i64) -> Result<u64, WorkloadOidcStoreError> {
    u64::try_from(value).map_err(|_| WorkloadOidcStoreError::CorruptData)
}

fn positive_i64(value: u64) -> Result<i64, WorkloadOidcStoreError> {
    if value == 0 {
        return Err(WorkloadOidcStoreError::CorruptData);
    }
    i64::try_from(value).map_err(|_| WorkloadOidcStoreError::CorruptData)
}
