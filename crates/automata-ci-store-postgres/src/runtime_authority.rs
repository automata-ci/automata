...
impl GithubRuntimeAuthorityRepository for PostgresStore {
    ...
    async fn mark_github_runtime_authority_indeterminate(
        ...
        .bind(fencing_i64(key.fencing_token()))
        .bind(request.owner().as_uuid())
        .bind(pg_bigint(request.fence().get()))
        .bind(database_now.get())
        .fetch_optional(&mut *transaction)
        .await
        .map_err(operation_error)?
    ...
    async fn quarantine_github_runtime_authority(
            ...
            && optional_digest_column(&durable, "aad_digest")? == Some(request.aad_digest())
        {
            ...
            .bind(corruption_kind_str(request.kind()))
            .fetch_optional(&mut *transaction)
            .await
            .map_err(operation_error)?
            .ok_or(GithubRuntimeAuthorityStoreError::QuarantineRejected)?;
        ...
        while remaining > 0 {
            ...
            let reduction = match action {
                RuntimeAuthorityReconciliationAction::QuarantinedEnvelopeErased => {
                    automata_ci_store::adapter_spi::GithubRuntimeAuthorityReconciliationReduction::QuarantinedEnvelopeErased
                }
                RuntimeAuthorityReconciliationAction::ExpiredEnvelopeErased => {
                    automata_ci_store::adapter_spi::GithubRuntimeAuthorityReconciliationReduction::ExpiredEnvelopeErased
                }
                RuntimeAuthorityReconciliationAction::IndeterminateAuthorityExpired => {
                    automata_ci_store::adapter_spi::GithubRuntimeAuthorityReconciliationReduction::IndeterminateAuthorityExpired
                }
                RuntimeAuthorityReconciliationAction::RevokedBeforeMint => {
                    automata_ci_store::adapter_spi::GithubRuntimeAuthorityReconciliationReduction::RevokedBeforeMint
                }
                RuntimeAuthorityReconciliationAction::MintRetryRejected => {
                    automata_ci_store::adapter_spi::GithubRuntimeAuthorityReconciliationReduction::MintRetryRejected
                }
                RuntimeAuthorityReconciliationAction::MintingMarkedIndeterminate => {
                    automata_ci_store::adapter_spi::GithubRuntimeAuthorityReconciliationReduction::MintingMarkedIndeterminate
                }
                RuntimeAuthorityReconciliationAction::ReadyMarkedRevokePending => {
                    automata_ci_store::adapter_spi::GithubRuntimeAuthorityReconciliationReduction::ReadyMarkedRevokePending
                }
            };
            if !automata_ci_store::adapter_spi::record_github_runtime_authority_reconciliation_reduction(
                &mut report,
                reduction,
            ) {
                return Err(GithubRuntimeAuthorityStoreError::CorruptData);
...
fn revocation_outcome_is_terminal_erasable(
    ...
    ) || current_fence != pg_bigint(fence.get())
...
fn quarantine_operation_digest(request: QuarantineGithubRuntimeAuthority) -> Sha256Digest {
    ...
    hash_bytes(&mut digest, corruption_kind_str(request.kind()).as_bytes());
...
fn decode_identity(
    ...
    .map_err(|_| GithubRuntimeAuthorityStoreError::CorruptData)?;
    let github_app_jwt_issuer_kind = decode_github_server_service_jwt_issuer(
...
fn decode_mint_claim(
    ...
    let (attempt, fence) = decode_mint_history(row)?;
    ClaimedGithubRuntimeAuthorityMint::from_repository_parts(
        decode_identity(row)?,
        worker_column(row, "mint_claim_owner_id")?,
        fence,
        attempt,
        timestamp_column(row, "mint_claimed_at_ms")?,
        timestamp_column(row, "mint_claim_expires_at_ms")?,
    )
    .map_err(|_| GithubRuntimeAuthorityStoreError::CorruptData)
}
...
const fn corruption_kind_str(kind: GithubRuntimeAuthorityCorruptionKind) -> &'static str {
    match kind {
        GithubRuntimeAuthorityCorruptionKind::InvalidEnvelope => "invalid_envelope",
        ...
        GithubRuntimeAuthorityCorruptionKind::InvalidWrappedDataKey => "invalid_wrapped_data_key",
        GithubRuntimeAuthorityCorruptionKind::UnknownWrappingKey => "unknown_wrapping_key",
        GithubRuntimeAuthorityCorruptionKind::RetiredWrappingKey => "retired_wrapping_key",
        GithubRuntimeAuthorityCorruptionKind::CryptographicFailure => "cryptographic_failure",
    }
}
...
fn decode_inspection(
    ...
    Ok(
        automata_ci_store::adapter_spi::github_runtime_authority_inspection(
            receipt,
            mint_attempts,
            next_action_at,
            decode_commit_disposition(row)?,
            optional_timestamp_column(row, "provider_expires_at_ms")?.is_some(),
            optional_timestamp_column(row, "safe_erase_after_ms")?,
            decode_corruption_kind(row)?,
        ),
    )
}
