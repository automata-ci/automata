    ...
    TestFuture: Future<Output = TestResult> + Send + 'static,
{
    ...
    let database_name = format!("automata_m64_{}", Uuid::new_v4().simple());
    debug_assert!(
        database_name
            .bytes()
...
async fn require_postgres_18(connection: &mut PgConnection) -> TestResult {
    ...
    if version < MINIMUM_POSTGRES_VERSION {
        return Err(message_error(format!(
            "migration 0064 live tests require PostgreSQL 18 or newer; server_version_num is {version}"
        )));
    }
    ...
    if !can_create_database {
        return Err(message_error(
            "migration 0064 live tests require CREATEDB on the isolated PostgreSQL server",
        ));
    }
    Ok(())
}
...
impl TestClock {
    async fn freeze_at_database_now(pool: &PgPool) -> TestResult<Self> {
        ...
        sqlx::query(
            r"
            CREATE TABLE automata_test.__automata_m64_clock (
        ...
        sqlx::query(
            r"
            INSERT INTO automata_test.__automata_m64_clock (singleton, now_ms)
            VALUES (TRUE, $1)
            ",
        )
        .bind(now_ms)
        ...
        sqlx::query(
            ...
            LANGUAGE SQL
            VOLATILE
            AS $automata_m64_clock$
                SELECT TIMESTAMPTZ 'epoch' + now_ms * INTERVAL '1 millisecond'
                FROM automata_test.__automata_m64_clock
                WHERE singleton
            $automata_m64_clock$
            ",
        )
    ...
    async fn set(&self, now_ms: i64) -> TestResult {
        let updated =
            sqlx::query("UPDATE automata_test.__automata_m64_clock SET now_ms=$1 WHERE singleton")
                .bind(now_ms)
                .execute(&self.pool)
                .await?;
...
async fn seed_0051_human_upgrade_fixture(
    ...
    .bind(format!("migration-0064-{session_id}"))
...
async fn migration_0064_fresh_database_supports_exact_deployment_bootstrap() -> TestResult {
...
async fn migration_0064_upgrades_0051_human_installation_and_token_exactly() -> TestResult {
    run_with_unmigrated_database(|database| async move {
...
async fn migration_0064_upgrades_configured_human_and_consumed_token_exactly() -> TestResult {
    run_with_unmigrated_database(|database| async move {
        let fixture = seed_0051_human_upgrade_fixture(&database).await?;
        let LegacyHumanUpgradeFixture {
            now_ms,
