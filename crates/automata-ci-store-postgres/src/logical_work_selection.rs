...
impl LogicalWorkSelectionRepository for PostgresStore {
    ...
    async fn consume_selected_logical_job_orchestration(
        ...
        let consumed =
            automata_ci_store::adapter_spi::repository_verified_logical_job_orchestration(
                selected.clone(),
                consumed,
                validated_at,
            )
    ...
    ) -> Result<ConsumedSelectedLogicalInstanceMaterialization, LogicalWorkSelectionStoreError>
    {
        ...
        let validated_at =
            require_selection_handoff_budget(&mut transaction, consumed.claim().expires_at())
                .await?;
        let consumed =
            automata_ci_store::adapter_spi::repository_verified_logical_instance_materialization(
                selected.clone(),
                consumed,
                validated_at,
            )
...
async fn insert_activation_receipt(
    ...
    .bind(generation.map(|generation| pg_bigint(generation.get())))
...
async fn quarantine_activation_authority(
    ...
    .bind(pg_bigint(selected.generation().get()))
...
async fn quarantine_materialization_authority(
    ...
    .bind(pg_bigint(authority.generation().get()))
