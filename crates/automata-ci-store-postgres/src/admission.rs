...
impl RunPublicationSnapshot {
    ...
    pub(super) fn from_durable_run(
        ...
        if effective_dashboard != Some(snapshot.dashboard())
            ...
            || safety_schema != Some(automata_ci_store::HUMAN_OUTPUT_PUBLICATION_SAFETY_SCHEMA)
        {
            return Err(StoreError::corrupt_data(
...
async fn insert_run(
    ...
    .bind(automata_ci_store::HUMAN_OUTPUT_PUBLICATION_SAFETY_SCHEMA)
    .bind(i16::try_from(RUNNER_REQUIREMENTS_SCHEMA_VERSION).unwrap_or(i16::MAX))
    .bind(automata_ci_store::WorkflowRunPriority::NORMAL.storage_value())
    .execute(&mut **transaction)
    .await
    .map_err(operation_error)?;
    Ok(())
...
async fn insert_preemption_intent(
    ...
    delivery: Option<&automata_ci_store::DurableRunnerCommand>,
) -> Result<(), WorkflowAdmissionStoreError> {
    let inserted = sqlx::query(
        r"
