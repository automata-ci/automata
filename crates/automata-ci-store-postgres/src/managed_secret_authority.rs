...
async fn resolve_managed_secret_execution_scope(
    ...
    Ok(automata_ci_store::adapter_spi::managed_secret_execution_scope(tenant, repository_id))
}
...
async fn acknowledge_managed_secret_delivery(
    ...
    transaction.commit().await.map_err(operation_error)?;
    Ok(
        automata_ci_store::adapter_spi::managed_secret_delivery_acknowledgement(
...
async fn check_managed_secret_authority(
    ...
    transaction.commit().await.map_err(operation_error)?;
    Ok(
        automata_ci_store::adapter_spi::repository_verified_managed_secret_authority_receipt(
...
fn validate_grant(
    ...
    let mode = decode_managed_secret_grant_mode(&grant.grant_mode)
        .ok_or(ManagedSecretAuthorityStoreError::CorruptData)?;
...
fn verified_binding(
    ...
    Ok(
        automata_ci_store::adapter_spi::repository_verified_managed_secret_authority_binding(
            grant_id,
            provider_id,
            secret_id,
            version_id,
            version_number,
            grant.canonical_name.clone(),
            scope,
            mode,
            grant.provider_supports_dynamic_leases,
        ),
...
fn decode_managed_secret_grant_mode(value: &str) -> Option<ManagedSecretGrantMode> {
    match value {
        "readable_secret" => Some(ManagedSecretGrantMode::ReadableSecret),
        "capability_only" => Some(ManagedSecretGrantMode::CapabilityOnly),
        _ => None,
    }
}
