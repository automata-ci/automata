...
fn decode_instance_evidence(
    ...
    let expected_commit_digest =
        automata_ci_store::adapter_spi::logical_instance_result_commit_digest(
            instance_id,
...
fn decode_prerequisite(
    ...
    if output_count != outputs.len()
        || automata_ci_store::adapter_spi::logical_job_outputs_digest(outputs) != outputs_digest
    {
        return Err(StoreError::corrupt_data(
...
async fn insert_instance_evidence(
    ...
    for instance in descriptor.instances() {
        ...
        .bind(pg_bigint(instance.terminal_ordinal().get()))
