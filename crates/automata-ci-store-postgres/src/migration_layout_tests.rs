...
const CANONICAL_MIGRATIONS: &[(&str, &str)] = &[
    ...
    (
        "0066_provider_result_projection_digest.sql",
    ...
    (
        "0070_managed_tenant_terminology.sql",
...
fn provider_result_projection_digest_is_exact_and_provider_neutral() {
    let source = include_str!("../migrations/0066_provider_result_projection_digest.sql");
    for required in [
...
fn provider_workflow_result_identity_is_unique_and_provider_neutral() {
    let source = include_str!("../migrations/0067_provider_workflow_result_identity.sql");
    for required in [
...
fn externally_managed_boundaries_use_tenant_terminology() {
    let source = include_str!("../migrations/0070_managed_tenant_terminology.sql");
