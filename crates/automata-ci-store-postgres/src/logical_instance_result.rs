...
impl LogicalInstanceResultRepository for PostgresStore {
    ...
    async fn commit_logical_instance_result(
        ...
        let rows = sqlx::query(
            r"
            UPDATE logical_workflow_instance_result_claims
...
async fn insert_initial_claim(
    ...
    let inserted = sqlx::query(
        r"
        INSERT INTO logical_workflow_instance_result_claims (
...
async fn lock_selection_request(
    ...
    let selection = sqlx::query(
        ...
        FROM logical_workflow_instance_result_selections
...
async fn reserve_selection_request(
    ...
    sqlx::query(
        ...
        DELETE FROM logical_workflow_instance_result_selections AS selection
        USING expired
    ...
    let inserted = sqlx::query(
        r"
        INSERT INTO logical_workflow_instance_result_selections (
...
async fn finalize_idle_selection(
    ...
    let rows = sqlx::query(
        r"
        UPDATE logical_workflow_instance_result_selections
        SET outcome = 'idle'
...
async fn finalize_claimed_selection(
    ...
    .bind(pg_bigint(claimed.claim().generation().get()))
...
async fn insert_quarantine(
    ...
    let inserted = sqlx::query_scalar::<_, i64>(
        r"
        INSERT INTO logical_workflow_instance_result_quarantines (
...
async fn load_durable_claim(
    ...
    let row = sqlx::query(
        ...
        FROM logical_workflow_instance_result_claims
        WHERE attempt_id = $1
        FOR UPDATE
        ",
    )
...
impl DurableResultClaim {
    ...
    fn matches_fence(&self, fence: &LogicalInstanceResultClaimFence) -> bool {
        self.state == "projecting"
            ...
            && self.generation == pg_bigint(fence.generation().get())
...
fn decode_descriptor(
    ...
    let terminal_ordinal = LogicalInstanceTerminalOrdinal::new(
        u64::try_from(
            row.try_get::<i64, _>("logical_workflow_terminal_ordinal")
                .map_err(operation_error)?,
        )
...
async fn insert_instance_result(
    ...
    let (
        ...
        automata_ci_store::LogicalInstanceTerminalAuthority::Runner(result) => (
            "runner",
        ...
        automata_ci_store::LogicalInstanceTerminalAuthority::ServerCancellation(cancellation) => (
            "server_cancellation",
            None,
            None,
            None,
    ...
    sqlx::query(
        r"
        INSERT INTO logical_workflow_instance_results (
    ...
    .bind(pg_bigint(descriptor.terminal_ordinal().get()))
...
async fn insert_outputs(
    ...
    for output in request.outputs() {
        sqlx::query(
            r"
            INSERT INTO logical_workflow_instance_result_outputs (
...
fn receipt_query() -> &'static str {
    ...
    SELECT result.instance_id, result.run_id, result.invocation_id,
           ...
           (SELECT count(*) FROM logical_workflow_instance_result_outputs AS output
...
fn terminal_projection_evidence_matches(
    ...
    match descriptor.terminal_authority() {
        automata_ci_store::LogicalInstanceTerminalAuthority::Runner(result) => Ok(authority
            == "runner"
        ...
        automata_ci_store::LogicalInstanceTerminalAuthority::ServerCancellation(cancellation) => {
            Ok(authority == "server_cancellation"
...
fn decode_receipt(
    ...
    let exact = instance_id == descriptor.instance_id()
        ...
        && match descriptor.terminal_authority() {
            automata_ci_store::LogicalInstanceTerminalAuthority::Runner(_) => {
        ...
        && row
            ...
            .map_err(operation_error)?
            == pg_bigint(descriptor.terminal_ordinal().get())
...
async fn verify_finalized_result_evidence(
    ...
    if u32::try_from(outputs.len()).unwrap_or(u32::MAX) != receipt.output_count()
        || automata_ci_store::adapter_spi::logical_instance_output_set_digest(&outputs)
            != receipt.outputs_digest()
...
async fn load_result_outputs(
    ...
    rows.into_iter()
        .map(|row| {
            automata_ci_store::adapter_spi::logical_instance_result_output(
