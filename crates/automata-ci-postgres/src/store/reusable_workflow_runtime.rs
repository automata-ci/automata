...
async fn load_next_call(
    ...
    let run_id = preparation.target().run_id();
    let inputs = load_call_inputs(&mut transaction, run_id, child_invocation_id).await?;
    let secrets = load_call_secrets(&mut transaction, run_id, child_invocation_id).await?;
    let permissions =
        load_permission_snapshot(&mut transaction, run_id, child_invocation_id, &row).await?;
    if usize::try_from(
...
fn publication_row_matches(
    ...
    Ok(row
        ...
        && row
            ...
            .map_err(operation_error)?
            == pg_bigint(request.runtime_policy().revision().get())
...
async fn insert_publication(
    ...
    .bind(pg_bigint(request.runtime_policy().revision().get()))
