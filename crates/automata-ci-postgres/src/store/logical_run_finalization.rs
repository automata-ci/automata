...
fn row_matches_fence(
    ...
    Ok(row
        ...
        && row
            ...
            .map_err(operation_error)?
            == pg_bigint(fence.generation().get())
        && row
...
async fn insert_result(
    ...
    .bind(logical_run_open_state_name(descriptor.marker_state()))
    .bind(revision_i64(descriptor.marker_revision())?)
    .bind(descriptor.marker_updated_at().get())
    .bind(logical_run_open_state_name(descriptor.invocation_state()))
    .bind(revision_i64(descriptor.invocation_revision())?)
    .bind(descriptor.invocation_updated_at().get())
    .bind(logical_run_workflow_status_name(
        descriptor.workflow_status(),
    ))
...
const fn logical_run_open_state_name(value: LogicalRunFinalizationOpenState) -> &'static str {
    match value {
        LogicalRunFinalizationOpenState::Pending => "pending",
        LogicalRunFinalizationOpenState::Active => "active",
    }
}
...
const fn logical_run_workflow_status_name(
    value: LogicalRunFinalizationWorkflowStatus,
) -> &'static str {
    match value {
        LogicalRunFinalizationWorkflowStatus::Queued => "queued",
        LogicalRunFinalizationWorkflowStatus::InProgress => "in_progress",
        LogicalRunFinalizationWorkflowStatus::Cancelled => "cancelled",
    }
}
