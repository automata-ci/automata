...
impl SecretCustodyRepository for PostgresSecretCustodyRepository {
    ...
    async fn verify_or_create_secret_custody(
        ...
        let Some(configured_keys) =
            automata_ci_store::adapter_spi::configured_secret_custody_keys(&request)
        else {
        ...
        let receipt = automata_ci_store::adapter_spi::verified_secret_custody(
            configured_keys,
            &requirements,
            bindings,
        )
...
async fn inspect_requirements(
    ...
    let required_key_ids = encoded_key_ids
        ...
        .map_err(|_| SecretCustodyRepositoryError::CorruptData)?;
    automata_ci_store::adapter_spi::secret_custody_requirements(
...
fn require_durable_keys_configured(
    ...
    if requirements.required_key_ids().iter().any(|required| {
        !automata_ci_store::adapter_spi::secret_custody_key_set_contains(configured_keys, required)
    }) {
...
async fn verify_canaries(
    ...
    for canary in canaries {
        ...
        bindings.push(
            automata_ci_store::adapter_spi::secret_custody_canary_binding(
                canary.key_id,
                canary.generation,
            ),
        );
...
fn decode_canary(
    ...
    let generation = row
        ...
        .and_then(|value| {
            automata_ci_store::adapter_spi::secret_custody_canary_generation(value)
                .map_err(|_| SecretCustodyRepositoryError::CorruptData)
        })?;
