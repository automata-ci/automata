...
#[cfg(target_os = "windows")]
const EXAMPLE_CONFIG_BYTES: &[u8] = include_bytes!("../config/runner.windows.example.json");
#[cfg(not(target_os = "windows"))]
const EXAMPLE_CONFIG: &str = "runner.local.example.json";
#[cfg(not(target_os = "windows"))]
const EXAMPLE_CONFIG_BYTES: &[u8] = include_bytes!("../config/runner.local.example.json");
...
fn capabilities_command_emits_only_the_canonical_validated_inventory() {
    ...
    let config = RunnerProductConfig::from_json(EXAMPLE_CONFIG_BYTES)
