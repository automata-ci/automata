...
fn records_reject_sentinel_authority_values() {
    let identity = automata_ci_auth::machine::ExternalRunnerIdentity::new("runner.example/one")
        .expect("external identity");
    let generation = automata_ci_store::RunnerGeneration::new(1).expect("generation");
    ...
    assert!(
        RunnerMachineRecord::new(
            ...
            generation,
            valid_digest,
            automata_ci_auth::time::UnixTimestamp::from_seconds(EXPIRES_AT),
            DesiredRunnerState::Active,
        )
        .is_err()
    );
    assert!(
        RunnerMachineRecord::new(
            ...
            RunnerId::new(),
            generation,
            automata_ci_core::Sha256Digest::from_bytes([0; 32]),
            automata_ci_auth::time::UnixTimestamp::from_seconds(EXPIRES_AT),
            DesiredRunnerState::Active,
        )
        .is_err()
    ...
    assert!(
        RunnerMachineRecord::new(
            ...
            generation,
            valid_digest,
            automata_ci_auth::time::UnixTimestamp::from_seconds(0),
            DesiredRunnerState::Active,
        )
        .is_err()
    );
