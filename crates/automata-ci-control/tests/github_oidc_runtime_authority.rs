...
use automata_ci_control::github_oidc::{
    GithubOidcAuthorityIdGenerator, GithubOidcAuthorityProvisioner,
    GithubOidcRuntimeAuthorityIssuer, ReserveGithubOidcRuntimeAuthority,
...
use automata_ci_oidc_github::{
    ...
    RequestBearerConfig, RequestBearerKey, RequestBearerKeyring,
};
...
async fn unavailable_guard_declines_other_jobs_and_blocks_entitled_jobs() {
    let guard = UnavailableGithubOidcRuntimeAuthorityIssuer;
    ...
    assert!(
        guard
            .issue_optional(foreign.request())
            .await
            .expect("a foreign job is not a GitHub OIDC availability failure")
            .is_none()
    );
...
async fn write_permissions_emit_exact_bound_tls_authority_and_verified_bearer() {
    ...
    for permission in permissions {
        ...
        let authority = bundle
            .get(GITHUB_OIDC_RUNTIME_AUTHORITY_NAMESPACE)
        ...
        assert_eq!(
            ...
            GITHUB_OIDC_RUNTIME_AUTHORITY_NAMESPACE
        );
...
async fn forged_reservations_and_provisioner_errors_fail_closed() {
    ...
    let forged = [
        ReservedGithubOidcRuntimeAuthority::new(
            authority_id,
            key_id.clone(),
            ISSUED_AT_SECONDS + 1,
            ISSUED_AT_SECONDS + 30,
...
async fn issuer_composes_as_an_optional_authority_without_placeholder() {
    ...
    assert_eq!(
        issued
            ...
            .collect::<Vec<_>>(),
        ["base", GITHUB_OIDC_RUNTIME_AUTHORITY_NAMESPACE]
    );
...
) -> GithubOidcRuntimeAuthorityIssuer {
    GithubOidcRuntimeAuthorityIssuer::new(
...
) -> ReservedGithubOidcRuntimeAuthority {
    let bearer = keyring
...
impl GithubOidcAuthorityIdGenerator for FixedAuthorityIds {
...
impl ReservationObservation {
    fn as_reserved(&self) -> ReservedGithubOidcRuntimeAuthority {
        ReservedGithubOidcRuntimeAuthority::new(
...
enum ProvisionerResponse {
    AcceptProposal,
    Fixed(ReservedGithubOidcRuntimeAuthority),
...
impl TestProvisioner {
    ...
    fn fixed(reservation: ReservedGithubOidcRuntimeAuthority) -> Self {
...
#[async_trait]
impl GithubOidcAuthorityProvisioner for TestProvisioner {
