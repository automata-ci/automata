...
fn simultaneous_renders_observe_non_blocking_admission() {
    ...
    let request = Arc::new(large_valid_request().to_string());
    ...
    request["page"]["runs"] = Value::Array(
        (0..250)
            .map(|index| {
                ...
                run["href"] = json!(format!("/automata/automata/actions/runs/{index}"));
                run
            })
            .collect(),
    );
