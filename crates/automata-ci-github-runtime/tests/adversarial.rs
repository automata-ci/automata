use automata_ci_github_runtime::{
    ...
    GithubCommandFileDecoder, GithubWorkflowCommandSession, WorkflowCommandError,
...
fn stream_limits_count_plain_output_as_well_as_commands() {
    ...
    let mut session = GithubWorkflowCommandSession::new(limits, WorkflowCommandPolicy::default());
...
fn malformed_names_and_scope_identifiers_fail_closed() {
    ...
    assert!(automata_ci_github_runtime::StepId::new("").is_err());
    assert!(automata_ci_github_runtime::StepId::new("has whitespace").is_err());
    assert!(automata_ci_github_runtime::ActionInvocationId::new("line\nbreak").is_err());
}
