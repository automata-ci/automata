...
use automata_ci_action::{ActionDefinitionDocument, ActionDefinitionKind};
...
/// Strict GitHub action-metadata decoder configured with explicit resource limits.
///
/// The decoder accepts only metadata documents already selected from an action bundle. It
/// performs no network access, action-reference resolution, expression evaluation, or execution.
...
impl GithubActionMetadataDecoder {
    ...
    /// Returns the resource limits applied to each decode operation.
    #[must_use]
