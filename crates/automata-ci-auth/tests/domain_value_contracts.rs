...
fn token_metadata_and_secret_material_cannot_become_inconsistent() {
    ...
    let versioned = VersionedProviderTokens::new(TokenVersion::new(3).expect("version"), tokens);
    ...
    assert!(!rendered.contains("access-token-value"));
    assert!(!rendered.contains("refresh-token-value"));
    assert!(TokenVersion::new(0).is_err());
    assert!(TokenVersion::new(9_223_372_036_854_775_807).is_ok());
    assert!(TokenVersion::new(9_223_372_036_854_775_808).is_err());
}
