...
#[test]
fn pending_response_without_a_legal_future_poll_expires_terminally() {
    let protocol = GithubAppProtocol::new(config());
    let endpoint = MockGithubEndpoint::default();
    let mut response = device_response("https://github.com/login/device");
    response.expires_in = 6;
    response.interval = 5;
    endpoint.push_device_code(Ok(response));
    endpoint.push_device_poll(Ok(GithubDevicePollResponse::AuthorizationPending));
    let mut authorization =
        block_on(protocol.begin_device(&endpoint, UnixTimestamp::from_seconds(100)))
            .expect("begin short device flow");

    let outcome = block_on(protocol.poll_device(
        &endpoint,
        &mut authorization,
        UnixTimestamp::from_seconds(105),
    ))
    .expect("terminal local expiry outcome");
...
#[test]
fn final_provider_failure_cannot_leave_an_unpersistable_poll_deadline() {
    let protocol = GithubAppProtocol::new(config());
    let endpoint = MockGithubEndpoint::default();
    let mut response = device_response("https://github.com/login/device");
    response.expires_in = 6;
...
#[test]
fn encrypted_device_transaction_parts_round_trip_with_poll_fencing_state() {
    let protocol = GithubAppProtocol::new(config());
    let endpoint = MockGithubEndpoint::default();
    endpoint.push_device_code(Ok(device_response("https://github.com/login/device")));
    endpoint.push_device_poll(Ok(GithubDevicePollResponse::AuthorizationPending));
    let authorization =
        block_on(protocol.begin_device(&endpoint, UnixTimestamp::from_seconds(100)))
            .expect("begin device flow");
    let parts = authorization.into_parts();
    let device_code = parts.device_code().expose_secret().to_owned();
    let user_code = parts.user_code().expose_secret().to_owned();
    ...
    let mut restored = automata_ci_auth::github::DeviceAuthorization::from_parts(
        parts,
        protocol.config().endpoints(),
    )
    .expect("restore device transaction");
    assert_eq!(restored.next_poll_at(), UnixTimestamp::from_seconds(105));
    let result =
        block_on(protocol.poll_device(&endpoint, &mut restored, UnixTimestamp::from_seconds(105)))
            .expect("poll restored transaction");
    assert!(matches!(
        result,
