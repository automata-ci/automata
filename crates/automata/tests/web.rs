...
#[tokio::test]
async fn run_list_is_rendered_on_the_server_with_a_per_response_csp_nonce() {
    ...
    let response = router_with_renderer(renderer.clone())
        .oneshot(
            Request::builder()
                .uri("/runs?status=in_progress&branch=feature%2Fclean")
                .body(Body::empty())
                .expect("request must be valid"),
        )
        .await
        .expect("run-list request must succeed");
    ...
    assert_eq!(model["page"]["kind"], "run-list");
    assert_eq!(model["page"]["filters"]["status"], "in_progress");
    assert_eq!(model["page"]["filters"]["branch"], "feature/clean");
    assert_eq!(
...
async fn production_router_returns_complete_react_html_without_javascript_execution() {
    let response = router()
        .expect("embedded renderer must initialize")
        .oneshot(
            Request::builder()
                .uri("/")
                .body(Body::empty())
    ...
    assert!(html.contains("<h1>Workflow runs</h1>"));
    assert!(html.contains("No workflow runs have been recorded"));
