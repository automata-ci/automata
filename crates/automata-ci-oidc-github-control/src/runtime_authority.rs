...
pub struct ReserveGithubOidcRuntimeAuthority<'a> {
...
impl<'a> ReserveGithubOidcRuntimeAuthority<'a> {
    fn new(
        runtime_authority_request: RuntimeAuthorityIssueRequest<'a>,
        proposed_authority_id: OidcAuthorityId,
        proposed_request_bearer_key_id: OidcKeyId,
...
impl fmt::Debug for ReserveGithubOidcRuntimeAuthority<'_> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        let request = self.runtime_authority_request;
        formatter
            .debug_struct("ReserveGithubOidcRuntimeAuthority")
...
pub struct ReservedGithubOidcRuntimeAuthority {
...
impl ReservedGithubOidcRuntimeAuthority {
    ...
    /// again by [`GithubOidcRuntimeAuthorityIssuer`].
    #[must_use]
    pub const fn new(
        authority_id: OidcAuthorityId,
        request_bearer_key_id: OidcKeyId,
...
pub trait GithubOidcAuthorityIdGenerator: fmt::Debug + Send + Sync {
    /// Generates one fresh, unguessable authority identity for a new proposal.
    fn next_github_oidc_authority_id(&self) -> OidcAuthorityId;
}

/// Random RFC 9562 version-4 OIDC authority identity generator.
#[derive(Clone, Copy, Debug, Default)]
pub struct RandomGithubOidcAuthorityIdGenerator;

impl GithubOidcAuthorityIdGenerator for RandomGithubOidcAuthorityIdGenerator {
    fn next_github_oidc_authority_id(&self) -> OidcAuthorityId {
        OidcAuthorityId::from_uuid(Uuid::new_v4()).expect("a version-4 UUID is non-nil")
    }
}

/// Least-authority durable boundary for one entitled GitHub OIDC execution.
///
...
#[async_trait]
pub trait GithubOidcAuthorityProvisioner: fmt::Debug + Send + Sync {
...
/// Fail-closed optional issuer used while GitHub-compatible OIDC is unavailable.
///
...
/// that do not request GitHub OIDC remain unaffected. An entitled job returns a
...
#[derive(Clone, Copy, Debug, Default)]
pub struct UnavailableGithubOidcRuntimeAuthorityIssuer;

#[async_trait]
impl OptionalRuntimeAuthorityIssuer for UnavailableGithubOidcRuntimeAuthorityIssuer {
    async fn issue_optional(
        &self,
        request: RuntimeAuthorityIssueRequest<'_>,
    ) -> Result<Option<JobRuntimeAuthorities>, ControlPortError> {
        if github_oidc_is_permitted(request) {
...
pub enum GithubOidcRuntimeAuthorityIssuerConfigurationError {
    /// The OIDC issuer cannot be represented as the exact TLS runtime origin.
    #[error("the GitHub OIDC runtime-authority issuer is invalid")]
    InvalidIssuer,
}

/// Permission-gated issuer for one deterministic GitHub-compatible OIDC bearer.
pub struct GithubOidcRuntimeAuthorityIssuer {
    provisioner: Arc<dyn GithubOidcAuthorityProvisioner>,
    authority_ids: Arc<dyn GithubOidcAuthorityIdGenerator>,
...
impl GithubOidcRuntimeAuthorityIssuer {
    ...
    pub fn new(
        ...
        authority_ids: Arc<dyn GithubOidcAuthorityIdGenerator>,
        provisioner: Arc<dyn GithubOidcAuthorityProvisioner>,
    ) -> Result<Self, GithubOidcRuntimeAuthorityIssuerConfigurationError> {
        let endpoint = RuntimeAuthorityEndpoint::new(issuer.as_str())
            .map_err(|_| GithubOidcRuntimeAuthorityIssuerConfigurationError::InvalidIssuer)?;
        let name = RuntimeAuthorityName::new(GITHUB_OIDC_RUNTIME_AUTHORITY_NAMESPACE)
            .map_err(|_| GithubOidcRuntimeAuthorityIssuerConfigurationError::InvalidIssuer)?;
        Ok(Self {
            provisioner,
            authority_ids,
...
impl fmt::Debug for GithubOidcRuntimeAuthorityIssuer {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("GithubOidcRuntimeAuthorityIssuer")
...
#[async_trait]
impl OptionalRuntimeAuthorityIssuer for GithubOidcRuntimeAuthorityIssuer {
    async fn issue_optional(
        &self,
        request: RuntimeAuthorityIssueRequest<'_>,
    ) -> Result<Option<JobRuntimeAuthorities>, ControlPortError> {
        ...
        let proposed_authority_id = self.authority_ids.next_github_oidc_authority_id();
        ...
        let reserve = ReserveGithubOidcRuntimeAuthority::new(
            request,
            proposed_authority_id,
            proposed_request_bearer_key_id,
            issued_at_seconds,
        ...
        let reserved = self
            .provisioner
            .reserve_github_oidc_runtime_authority(reserve)
            .await?;
...
fn github_oidc_is_permitted(request: RuntimeAuthorityIssueRequest<'_>) -> bool {
    let job = request.job();
...
fn bearer_digest(bearer: &automata_ci_oidc_github::OidcRequestBearer) -> Sha256Digest {
