...
async fn does_not_retry_non_transient_provider_or_integrity_failures() {
    for (response, expected) in [
        ...
        (ResponseSpec::Unencrypted, BlobStoreErrorKind::Integrity),
    ] {
        let body = fixture_body();
        let descriptor = descriptor(&body);
...
async fn serve_response(
    ...
    match response {
        ResponseSpec::Complete => {
            write_object_response(&stream, descriptor, body, false, true, true).await
        }
        ResponseSpec::Truncated => {
            write_object_response(&stream, descriptor, body, true, true, true).await
        }
        ...
        ResponseSpec::InvalidMetadata => {
            write_object_response(&stream, descriptor, body, false, false, true).await
        }
        ResponseSpec::Unencrypted => {
            write_object_response(&stream, descriptor, body, false, true, false).await
        }
    }
}
