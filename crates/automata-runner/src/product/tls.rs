use automata_runner_transport::{ClientTlsConfig, TlsVersionPolicy};
use rustls::{
    RootCertStore,
...
pub(crate) fn load_client_tls(
    ...
    let policy = if sources.allow_legacy_tls12() {
        TlsVersionPolicy::Tls13WithLegacyTls12
    } else {
        TlsVersionPolicy::Tls13Only
    };
    ClientTlsConfig::new(roots, certificate_chain, private_key, policy)
