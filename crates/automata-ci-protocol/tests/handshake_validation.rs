...
fn server_selection_must_be_inside_the_runner_offer() {
    let offered = automata_ci_protocol::ProtocolRange::new(
...
#[test]
fn runner_job_ir_offer_must_be_the_exact_current_contract() {
    let future = JobIrVersion::new(JobIrVersion::current().get() + 1)
        .expect("positive future JobIR version");
    assert!(JobIrVersionRange::new(future, future).is_err());
}
