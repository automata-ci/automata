...
impl SessionResume {
    ...
    #[must_use]
    /// Returns the old session identity claimed by the runner.
    pub const fn session_id(self) -> RunnerSessionId {
        self.session_id
    }

    #[must_use]
    /// Returns the highest server command durable in the runner journal.
...
pub enum SessionDisposition {
    /// A fresh session was opened with an initial command cursor.
    Opened,
    /// The authenticated prior session and its durable cursor were resumed.
    Resumed,
}
...
impl ServerCommandHeader {
    ...
    #[must_use]
    /// Returns the message-structure schema carried by the command.
    pub const fn message_schema_version(self) -> u16 {
        self.message_schema_version
    }

    #[must_use]
    /// Returns the wire protocol negotiated for the session.
    pub const fn protocol_version(self) -> ProtocolVersion {
        self.protocol_version
    }

    #[must_use]
    /// Returns the authenticated runner session that owns the command.
    pub const fn session_id(self) -> RunnerSessionId {
        self.session_id
    }

    #[must_use]
    /// Returns the stable operation identity retained across command replays.
    pub const fn operation_id(self) -> OperationId {
        self.operation_id
    }

    #[must_use]
    /// Returns the one-based durable order within the session command stream.
...
pub enum CommandSequenceError {
    /// Zero cannot identify a command because sequences are one-based.
    #[error("server command sequences are one-based and cannot be zero")]
    Zero,
    /// The value cannot be represented by the durable signed 64-bit store.
    #[error("server command sequences must fit the durable signed 64-bit representation")]
    OutOfRange,
    /// No successor exists within the durable sequence range.
...
pub enum CommandCursorError {
    /// The supplied sequence is not the cursor's exact successor.
    #[error("expected command sequence {expected:?}, received {received:?}")]
    NonContiguous {
        /// Exact sequence required to advance the cursor.
        expected: CommandSequence,
        /// Sequence supplied by the caller.
        received: CommandSequence,
    },
    /// The cursor already acknowledges the largest durable sequence.
...
pub enum RunnerSlotOrdinalError {
    /// Zero cannot identify a runner slot because ordinals are one-based.
