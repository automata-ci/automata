...
impl ProtocolLimits {
    ...
    #[must_use]
    /// Returns the maximum encoded bytes accepted for one complete frame.
    pub const fn max_frame_bytes(self) -> usize {
        self.frame_bytes
    }

    #[must_use]
    /// Returns the default ceiling applied to each validated collection.
    pub const fn max_collection_items(self) -> usize {
        self.collection_items
    }

    #[must_use]
    /// Returns the maximum UTF-8 bytes accepted in one text value.
    pub const fn max_text_bytes(self) -> usize {
        self.text_bytes
    }

    #[must_use]
    /// Returns the maximum number of frames accepted in one log batch.
    pub const fn max_log_frames_per_batch(self) -> usize {
        self.log_frames_per_batch
    }

    #[must_use]
    /// Returns the aggregate payload-byte ceiling for one log batch.
...
pub enum ProtocolLimitsError {
    /// At least one configured ceiling is zero.
    #[error("protocol limits must be nonzero")]
    ZeroLimit,
    /// The frame ceiling exceeds [`MAX_CONFIGURABLE_FRAME_BYTES`].
    #[error("protocol frame limit exceeds the absolute 64 MiB ceiling")]
    FrameLimitTooLarge,
    /// A nested text, collection, or log budget exceeds its container budget.
