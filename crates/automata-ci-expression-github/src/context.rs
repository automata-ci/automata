...
pub enum GithubStatus {
    ...
    Cancelled,
    /// A prerequisite job did not run, without failing or being cancelled.
    ///
    /// This is distinct so job-level `success()`, `failure()`, and
    /// `cancelled()` are all false while `always()` remains true.
    Skipped,
}
