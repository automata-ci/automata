...
use automata_ci_execution::ExecutionEnvironment;
...
async fn declarations_are_hashed_in_the_workspace_and_list_is_read_only_and_deterministic() {
    ...
    let hash = state
        ...
        .expect("runner executed the sandbox hash command");
    assert_eq!(hash.working_directory().as_str(), WORKSPACE);
    assert_eq!(hash.argv().arguments()[2], "/usr/bin/sha256sum");
    assert_eq!(hash.argv().arguments()[3], "dist/app.bin");
