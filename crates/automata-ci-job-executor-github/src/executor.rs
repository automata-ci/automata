    ...
    fn materialize_github_workflow_directory(
        ...
        let (program, arguments) = match worktree.platform() {
            TargetPlatform::Posix => (
                ...
                vec![
                    "-c".to_owned(),
                    "if [ -d .ci/workflows ]; then mkdir -p .github/workflows; for source in .ci/workflows/*; do if [ ! -f \"$source\" ] || [ -L \"$source\" ]; then continue; fi; destination=\".github/workflows/${source##*/}\"; if [ -e \"$destination\" ] || [ -L \"$destination\" ]; then printf 'portable workflow path collides with native path: %s\\n' \"$destination\" >&2; exit 1; fi; cp -- \"$source\" \"$destination\"; done; fi"
                        .to_owned(),
                ],
            ),
            TargetPlatform::Windows => (
                ...
                vec![
                    ...
                    "-Command".to_owned(),
                    "if (Test-Path -LiteralPath '.ci/workflows' -PathType Container) { New-Item -ItemType Directory -Force -Path '.github/workflows' | Out-Null; Get-ChildItem -LiteralPath '.ci/workflows' -File | Where-Object { -not (($_.Attributes -band [System.IO.FileAttributes]::ReparsePoint)) } | ForEach-Object { $destination = Join-Path '.github/workflows' $_.Name; if ($null -ne (Get-Item -LiteralPath $destination -Force -ErrorAction SilentlyContinue)) { throw \"portable workflow path collides with native path: $destination\" }; Copy-Item -LiteralPath $_.FullName -Destination $destination } }"
                        .to_owned(),
                ],
            ),
        };
