...
#[test]
fn service_contract_rejects_ambiguous_names_ports_and_discovery() {
    let port = ServicePort::new(53, None, ServiceTransportProtocol::Tcp).expect("port");
    let duplicate_protocol =
        ServicePort::new(53, None, ServiceTransportProtocol::Udp).expect("port");
    let image = || {
        ImmutableImage::new(format!("docker.io/library/dns@sha256:{}", "c".repeat(64)))
            .expect("image")
    };
    assert!(
        ServiceContainerSpec::new(image(), ExecutionEnvironment::empty())
            .with_ports([port, duplicate_protocol])
            .is_err()
    );
    let service = ServiceContainerSpec::new(image(), ExecutionEnvironment::empty());
    assert!(
        ServiceContainerSpecs::new(BTreeMap::from([
            ("cache".to_owned(), service.clone()),
            ("CACHE".to_owned(), service),
        ]))
        .is_err()
    );
    let first_service = ServiceContainerSpec::new(image(), ExecutionEnvironment::empty())
        .with_ports([
            ServicePort::new(53, Some(1053), ServiceTransportProtocol::Tcp).expect("port"),
        ])
        .expect("service");
    let second_service = ServiceContainerSpec::new(image(), ExecutionEnvironment::empty())
        .with_ports([
            ServicePort::new(54, Some(1053), ServiceTransportProtocol::Tcp).expect("port"),
        ])
        .expect("service");
    assert!(
        ServiceContainerSpecs::new(BTreeMap::from([
            ("first".to_owned(), first_service),
            ("second".to_owned(), second_service),
        ]))
        .is_err()
    );
    assert!(ServicePortBinding::new(port, 0).is_err());
    assert!(ServicePort::new(53, Some(0), ServiceTransportProtocol::Tcp).is_err());
    let first = ServicePort::new(53, Some(1053), ServiceTransportProtocol::Tcp).expect("port");
    let duplicate_listener =
        ServicePort::new(54, Some(1053), ServiceTransportProtocol::Tcp).expect("port");
    assert!(
        ServiceContainerSpec::new(image(), ExecutionEnvironment::empty())
            .with_ports([first, duplicate_listener])
            .is_err()
    );
}
