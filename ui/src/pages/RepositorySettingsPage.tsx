...
const publicationResources: readonly {
  ...
  {
    field: "dashboard",
    formName: "dashboard_audience",
    label: "Run pages",
    description: "Repository, workflow, run, and job details.",
  },
  {
    field: "logs",
    formName: "log_audience",
    label: "Job logs",
    description: "Console output recorded for workflow jobs.",
  },
  {
    field: "artifacts",
    formName: "artifact_audience",
    label: "Artifacts",
    description: "Artifact names, metadata, and file downloads.",
  },
];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <section
          className="panel repository-settings"
          aria-labelledby="publication-settings-heading"
        >
          <div className="panel__heading">
            <h2 id="publication-settings-heading">Defaults for new runs</h2>
          </div>
          <div
            className="repository-settings__guidance"
            id="publication-policy-guidance"
          >
            <p>
              These defaults are recorded when a workflow run starts. Existing
              runs keep their current access.
            </p>
            <p>
              Run pages, logs, and artifacts are authorized independently. When
              a job can read Automata-managed secrets, its logs and artifacts
              remain private.
            </p>
          </div>
          {model.update === null ? (
            <div>
              <p
                className="repository-settings__notice"
                role="note"
              >
                Read-only: you can review these defaults, but they cannot be
                changed from this page.
              </p>
              <AudienceSummary policy={model.policy} />
              <div className="repository-settings__actions">
                <a className="button" href={model.repository.runsHref}>
...
function RepositorySettingsForm({
  ...
  readonly update: RepositorySettingsUpdateModel;
}) {
  const [draftPolicy, setDraftPolicy] = useState(policy);
  const [clientReady, setClientReady] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const submitted = useRef(false);
  const hasChanges = publicationResources.some(
  ...
  useEffect(() => {
    ...
    const restoreAfterHistoryNavigation = (event: PageTransitionEvent) => {
      ...
      if (submitted.current) {
        // The POST may have advanced the revision; never revive its stale form.
  ...
  const saveDisabled = clientReady && (!hasChanges || isSubmitting);
  return (
    <form
      action={update.action}
      aria-busy={isSubmitting || undefined}
      aria-describedby="publication-policy-guidance"
      method="post"
      onSubmit={submit}
    >
      <input name="csrf_token" type="hidden" value={update.csrfToken} />
      <input name="expected_revision" type="hidden" value={revision} />
      <AudienceControls onChange={selectAudience} policy={draftPolicy} />
      <div className="repository-settings__actions">
        <button
          aria-busy={isSubmitting || undefined}
          className="button button--primary repository-settings__save"
...
function AudienceSummary({
  ...
  return (
    <ul
      aria-label="Current access defaults"
      className="repository-settings__summary"
    >
      {publicationResources.map((resource) => {
        ...
        return (
          <li className="audience-summary" key={resource.field}>
            <div className="audience-summary__resource">
              <h3>{resource.label}</h3>
              <p>{resource.description}</p>
            </div>
            <div className="audience-summary__current">
              <strong>
                <span className="sr-only">Current access: </span>
                {option.label}
              </strong>
