...
> [!NOTE]
> This roadmap preserves the investigation and branch evidence from closed PR
> #30. Checked adapter work that names the consolidated feature branch is not a
> current `main` availability claim; it must be ported and revalidated in a
> focused PR. The planned [Windows local evaluation](windows-local-evaluation.md)
> milestone is a separate, non-production path.
