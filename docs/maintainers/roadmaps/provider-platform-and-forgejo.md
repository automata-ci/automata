...
| B4a GitHub delivery foundation | Bind endpoints to exact connections; normalize GitHub events through the common envelope; add the provider-neutral ingress and renewing worker | 3,500-5,500 | Push/PR/merge-group/repository-dispatch signature parity; rotation, multi-commit, instance isolation, and renewal tests |
| B4b Provider delivery invocation and control | Replace store-owned delivery/worker identities and workflow-admission evidence with common provider identities; model authenticated provider controls as fresh processing invocations derived from immutable deliveries | 6,500-8,500 | Check Run `rerequested` creates one replay-safe invocation and a fresh Check; exact control replay is idempotent; old identities and evidence definitions are absent |
| B4c GitHub delivery cutover | Atomically compose endpoint provisioning, the opaque ingress route, generic delivery processor/worker, and provider-control dispatch; delete the old GitHub ingress, queue, worker, and hard-coded route | 7,000-8,800 | End-to-end replay/admission/control parity; generic opaque route only; old ingress, worker, route, and delivery records absent |
