...
> [!NOTE]
> This document preserves the production Windows investigation from issue #16
> and the closed PR #30 branch. Statements about adapter implementation record
> evidence produced on that branch; they do not claim that the adapter is
> present on `main`. Revalidate them before carrying implementation forward.
> The smaller [Windows local evaluation](windows-local-evaluation.md) path does
> not weaken or replace these production requirements.
