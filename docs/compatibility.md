...
Automata itself is available and runs this repository's CI. The
[successful Checks on main commit `280cd4f9`](https://github.com/automata-ci/automata/commit/280cd4f9e685ac022c65a920ba24f4f019b0fd25/checks)
prove the complete GitHub-to-runner path for the features exercised by
`.ci/workflows/ci.yml`: push admission, expressions, workflow concurrency,
per-job CPU and memory limits, an exact-commit JavaScript checkout action,
shell steps, a PostgreSQL service container, rootless Linux execution, result
publication, GitHub Checks, and web run/job views.

That run does not prove every row below. The status table records each feature
independently and is maintained with its owning behavior tests and acceptance
evidence. `Component complete` cannot be inferred from decoder or compiler
acceptance alone.
...
| Workflow-level concurrency | Available | Group and `cancel-in-progress` are evaluated from admission-safe context. Standard single-pending coordination has durable preemption, idempotent replay, and admission/claim race coverage. GitHub's ordered `queue: max` policy retains at most 100 repository-scoped, case-insensitive waiters in durable FIFO order; invalid max-plus-cancel policies fail before persistence, including after expression evaluation. Reruns re-enter the same durable policy. | Job-level concurrency remains unsupported. |
...
| Exact-commit `actions/checkout` | Available | This repository's public CI resolves and runs `actions/checkout` pinned to a 40-character lowercase commit. Source archives and action bundles are immutable and content-addressed. | Private repository action resolution and mutable action references remain outside this boundary. |
...
| Service containers | Available | Digest-pinned service configuration reaches the logical plan, JobIR, executor, and rootless Podman backend. Provider lifecycle and service boundaries have component tests, and this repository's PostgreSQL CI job exercises the complete path. | Expand service-image and failure-mode acceptance. The provider requires a configured, locally verified immutable service-proxy image. |
| Scheduling and runner execution | Available | PostgreSQL coordination, leases, fencing, mTLS runner transport, host probes, and rootless Podman execution are composed and run this repository's CI. | Expand fault, scale, and heterogeneous-fleet acceptance. |
...
| Local Docker sandbox provider | Experimental | Current runner schema 8 binds the mutually exclusive `local_docker` variant to one installation UUID, a digest-pinned guest, the sealed desired-plan digest, the exact classic/config-ID or containerd/manifest-ID representation of the daemon-local imported Results proxy, and one mandatory externally provisioned Results transport. The private provider requires Docker Engine 28/API 1.48, uses only the fixed installation relay, and rechecks exact daemon, anchor, imported-image representation, plan-bound shared transit, running Results target, and peer-proxy identity. Each zero-volume job gets a deterministic internal front network shared only with its fixed-port proxy; the job has no external DNS or public egress. Custody-only destruction tolerates container-runtime/image and shared-transit damage, but exact front-network drift blocks destroy and a foreign endpoint can leave the emptied front network for operator recovery. The current lifecycle renders the sealed desired intent, creates the exact Compose-external transit/listener, and bootstraps the configured runner. | Complete local repository authority, credential, Results/cache URL and token injection, and `automata local run`. No local CLI invokes this provider for a job yet. |
| Runtime identity and result projection | Available | Runs receive immutable positive numeric aliases. Finalized job and run results publish to the Automata UI and fenced GitHub Checks. Logs, public outputs, summaries, and annotations have focused tests. | Complete retention and broader end-to-end comparison for logs, outputs, summaries, and annotations. |
...
| GitHub provider | Experimental | Configured deployments include browser and device login, encrypted provider state, authenticated push, `pull_request`, `merge_group`, and `repository_dispatch` webhook ingress, durable event replay, public/private source delivery, fenced Check Runs, scoped App credentials, and lease-bound repository authority. The canonical configured default-branch ref is manifest-revisioned and digest-bound. The `all_direct` policy evaluates each canonical direct workflow at one authenticated revision from a sorted, digest-bound inventory; path-local progress makes partial retry idempotent. Required aggregate identity is isolated to revision-testing event activities, cannot conclude skipped, and startup attests effective `merge_group` plus `merge_queues: read` installation capability. | Add the remaining reviewed event types and pass multi-workflow provider, runner, and service-image acceptance together. |
| Authentication and UI | Component complete | Tenant-scoped RBAC, management APIs, browser forms, repository publication settings, SSR run pages, Linux Secret Service-backed CLI sessions, and macOS Keychain-backed CLI sessions are composed. | Complete the production acceptance and operating evidence. |
| Standalone self-host onboarding | Planned | The private mTLS shard-management API applies GitHub App and repository desired state with revision, authorization, encryption, and replay evidence. The Automata-operated installation uses that composition. | Ship a supported standalone provisioning client and deployment bundle. Direct PostgreSQL writes are not an onboarding interface. |
| Public versioned distribution | Planned | Release, image, checksum, SBOM, and attestation preparation code exists, but publication workflows deliberately fail before mutation because the required provider-authenticated release authority is absent. | Implement and verify the release authority, then publish matching artifacts to their named registries before documenting an install version. |
| Permissions semantics | Component complete | Workflow and job permission shorthands, precedence, policy intersection, and OIDC capability gating have component coverage. | Complete GitHub permission-default and event-specific differential coverage before claiming broader semantics. |
