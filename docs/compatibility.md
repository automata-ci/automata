...
| JavaScript and local composite actions | Component complete | Repository action trees and exact Node-generation requirements are recursively prepared after runtime-context masking but before custody acknowledgement or provider work; cycles, nested containers, and unavailable profile runtimes fail with zero sandbox operations. Inputs preserve metadata order, case-insensitive overlay, empty missing values, scalar spelling, ignored `required` markers, and bounded static deprecations. Invocation-specific action/path/ref/repository context, source-ordered pre execution, occurrence-scoped state, hierarchical post registration and cleanup ordering, post-time re-evaluation, cancellation, and command files have focused coverage. Checked-out local composites remain JIT-prepared; their runtimes are checked before action code and local action `pre` entrypoints are intentionally skipped with a sanitized diagnostic. | Carry prepared runtime requirements into scheduling for literal pre-lease rejection, run the live pinned-runner input differential, then complete broader marketplace and end-to-end fixtures. |
...
| Local Docker sandbox provider | Experimental | Current runner schema 5 has a mutually exclusive `local_docker` variant bound to one installation UUID and an already-present digest-pinned guest image. The private provider uses only the fixed installation relay, requires a rootful daemon attesting default user-namespace remapping, and requires its architecture to equal the already-validated runner inventory. It rechecks exact daemon security options, anchor, image, and container identity around mutations. Every sandbox proves one nonzero host UID/GID range covering its fixed identities. Sibling containers receive no bind, Engine socket, per-job volume, or network. The administrator is attenuated UID 0 in that remapped namespace with all Linux capability sets empty; no `chown`, identity-switching, or other POSIX capability is promised. Each bounded guest operation is attempted once through a protected tmpfs client; durable replay remains host-owned, and ambiguous execution destroys the exact sandbox. | Run the ignored fixed-relay conformance fixture on every qualified daemon and architecture, then complete the separate desired-spec, Compose, credential, and `automata local run` product slices. No local CLI invokes this provider yet. |
