...
| Reusable workflows, environments, schedules, reruns, job-level concurrency, and complete permissions semantics | Unsupported end to end | Some fields are retained for diagnostics or active implementation. The workflow-rerun schema retains immutable attempt and carry-forward evidence, but no server or CLI route invokes it and the PostgreSQL repository returns `WorkflowRerunStoreError::Unsupported` before any write. | Implement, compose, and compare their runtime semantics; reruns additionally need activation consumers and graph-sealing provenance that can safely consume carried results. |
