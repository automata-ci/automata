...
| Reusable workflows, environments, schedules, reruns, job-level concurrency, and complete permissions semantics | Partial | [Authenticated workflow reruns](workflow-reruns.md) create a fresh physical attempt under the stable public run identity, reauthorize the current actor, seal exact selected/carried graph provenance, preserve nested effective results, and create a distinct Check subject through the existing fenced projection outbox. The bounded CLI can record an exact, revision-pinned protected-environment approval or rejection. Reusable workflows and the other listed surfaces remain incomplete. | Pass production provider/runner acceptance for reruns and protected environments, and implement the remaining listed semantics. |
