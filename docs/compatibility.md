...
2026-08-16. “Component complete” means the boundary has focused tests but the
...
| Local Docker sandbox provider | Experimental | Current runner schema 5 has a mutually exclusive `local_docker` variant bound to one installation UUID and an already-present digest-pinned guest image. The provider uses only the fixed installation relay, rechecks exact daemon/anchor identity before mutations, realizes zero-volume sibling containers with closed security/network/resource configuration, and provides bounded live exec/copy replay without restarting exited jobs. The ignored fixed-relay Arch fixture has passed against the reviewed Docker 29.7.2 amd64 environment. | Qualify additional supported daemon and architecture combinations, then complete the separate desired-spec, Compose, credential, and `automata local run` product slices. No local CLI invokes this provider yet. |
