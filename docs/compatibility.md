...
| Push and pull-request path filters | Experimental | Ordered positive/negative path filters, branch/tag interaction, pull-request activity defaults, the push commit run-all ceiling, and complete public pull-request file pagination have focused coverage. Public same-repository and fork PRs bind exact pre/post PR snapshots and the first 3,000 provider files. Complete, provider-run-all, retryable-unavailable, and invalid dispositions are disjoint; selection evidence is digest-bound to the authenticated event, workflow path, workflow source, and immutable admitted plan. | Private PR filtering is intentionally unavailable until a manifest-pinned `pull requests: read` selector exists; the current `contents: read` source credential is never reused. New-branch and forced/diverged push diff parity, rename semantics, commit-message skip directives, and live GitHub differential evidence remain open. A retry refetches PR files from page one rather than checkpointing partial pages. |
