...
readonly table_name="automata_git_bridge_guard"
readonly chain_name="git_bridge_input"
readonly table_identity="automata-git-bridge-firewall:v1"
readonly rule_identity="automata-git-bridge-firewall:deny-non-loopback:v1"

usage() {
    cat >&2 <<'EOF'
usage: git-bridge-firewall.sh ACTION --listen-address PRIVATE_IPV4 --port PORT
       git-bridge-firewall.sh ACTION --config CONFIG_FILE
...
sourced), must contain only AUTOMATA_GIT_BRIDGE_LISTEN_ADDRESS and
AUTOMATA_GIT_BRIDGE_LISTEN_PORT assignments, and must not traverse symlinks.
EOF
}
...
read_config() {
    ...
    while IFS= read -r line || [[ -n "${line}" ]]; do
        ...
        case "${line}" in
            ''|'#'*)
                ;;
            AUTOMATA_GIT_BRIDGE_LISTEN_ADDRESS=*)
                [[ -z "${config_address}" ]] ||
                    fail "duplicate listen address in config line ${line_number}"
                config_address="${line#*=}"
                ;;
            AUTOMATA_GIT_BRIDGE_LISTEN_PORT=*)
                [[ -z "${config_port}" ]] ||
...
table_handle() {
    ...
    if [[ "${first_line}" =~ ^table[[:space:]]+inet[[:space:]]+automata_git_bridge_guard[[:space:]]+\{[[:space:]]+#[[:space:]]handle[[:space:]]+([0-9]+)$ ]]; then
        printf '%s\n' "${BASH_REMATCH[1]}"
        return 0
    fi
    return 1
...
audit_table() {
    ...
    current="$(read_current_table)" || fail "Automata Git bridge firewall table is absent"
    [[ "${current}" == "${expected}" ]] ||
        fail "Automata Git bridge firewall table differs from the exact expected policy"
    printf 'Automata Git bridge firewall policy is exact.\n'
}
...
apply_table() {
    ...
    if current="$(read_current_table)"; then
        ...
        printf 'Automata Git bridge firewall policy is already exact; no change made.\n'
        return 0
    fi
    ...
    printf 'Automata Git bridge firewall policy applied atomically.\n'
}
...
remove_table() {
    ...
    if ! current_with_handles="$(read_current_table_with_handles)"; then
        printf 'Automata Git bridge firewall policy is absent; no change made.\n'
        return 0
    fi
    ...
    printf 'Automata Git bridge firewall policy removed.\n'
}
