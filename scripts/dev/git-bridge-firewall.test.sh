...
readonly firewall_script="${repository_root}/scripts/dev/git-bridge-firewall.sh"
readonly example_config="${repository_root}/deploy/dev/git-bridge-firewall.env.example"
readonly test_address="192.168.51.8"
readonly test_port="18088"
...
readonly test_work_root="${AUTOMATA_CANONICAL_TARGET_ROOT}/git-bridge-firewall-tests"
...
expected_rules="$({
    printf '%s\n' \
        ...
        $'\t\tip daddr 192.168.51.8 tcp dport 18088 iifname != "lo" drop comment "automata-git-bridge-firewall:deny-non-loopback:v1"' \
        $'\t}' \
        '}'
})"
...
explicit_example_rules="$(${firewall_script} render --listen-address 192.168.0.8 --port 8088)"
[[ "${config_rules}" == "${explicit_example_rules}" ]] || fail "config parsing changed"

printf '%s\n' 'AUTOMATA_RESULTS_LISTEN_ADDRESS=192.168.51.8' \
    >"${test_work_directory}/bad-config"
expect_failure "foreign config key" \
    "${firewall_script}" render --config "${test_work_directory}/bad-config"
printf '%s\n' \
    'AUTOMATA_GIT_BRIDGE_LISTEN_ADDRESS=192.168.51.8' \
    'AUTOMATA_GIT_BRIDGE_LISTEN_ADDRESS=192.168.51.9' \
    'AUTOMATA_GIT_BRIDGE_LISTEN_PORT=18088' \
    >"${test_work_directory}/duplicate-config"
expect_failure "duplicate config key" \
    "${firewall_script}" render --config "${test_work_directory}/duplicate-config"
...
expect_failure "symbolic-link config directory" \
    ...
    --config "${test_work_directory}/config-directory-link/git-bridge-firewall.env.example"
...
expect_failure "carrier-grade NAT address" \
    "${firewall_script}" render --listen-address 100.64.0.8 --port "${test_port}"
...
expect_failure "non-canonical address" \
    "${firewall_script}" render --listen-address 192.168.051.8 --port "${test_port}"
expect_failure "address injection" \
    "${firewall_script}" render --listen-address '192.168.51.8;flush ruleset' --port "${test_port}"
...
expect_failure "port injection" \
    "${firewall_script}" render --listen-address "${test_address}" --port '18088;drop'
...
if command -v sudo >/dev/null 2>&1 &&
    ...
    sudo -n unshare --net -- env \
        ...
        bash -euo pipefail -c '
            ...
            nft add rule inet automata_git_bridge_guard git_bridge_input counter
            ! "${AUTOMATA_FIREWALL_SCRIPT}" audit \
                --listen-address "${AUTOMATA_FIREWALL_TEST_ADDRESS}" \
                --port "${AUTOMATA_FIREWALL_TEST_PORT}"
            ! "${AUTOMATA_FIREWALL_SCRIPT}" apply \
            ...
            ! "${AUTOMATA_FIREWALL_SCRIPT}" remove \
                --listen-address "${AUTOMATA_FIREWALL_TEST_ADDRESS}" \
                --port "${AUTOMATA_FIREWALL_TEST_PORT}"
            nft list table inet automata_git_bridge_guard >/dev/null

            nft delete table inet automata_git_bridge_guard
            "${AUTOMATA_FIREWALL_SCRIPT}" apply \
                --listen-address "${AUTOMATA_FIREWALL_TEST_ADDRESS}" \
                --port "${AUTOMATA_FIREWALL_TEST_PORT}"
            "${AUTOMATA_FIREWALL_SCRIPT}" remove \
                --listen-address "${AUTOMATA_FIREWALL_TEST_ADDRESS}" \
                --port "${AUTOMATA_FIREWALL_TEST_PORT}"
            ! nft list table inet automata_git_bridge_guard >/dev/null 2>&1
            "${AUTOMATA_FIREWALL_SCRIPT}" remove \
                --listen-address "${AUTOMATA_FIREWALL_TEST_ADDRESS}" \
                --port "${AUTOMATA_FIREWALL_TEST_PORT}"
...
printf 'Git bridge firewall contract verified\n'
