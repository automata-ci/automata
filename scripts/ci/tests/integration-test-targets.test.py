...
AGGREGATES = {
    ...
    "automata-ci-workload-oidc": "workload_oidc",
...
# PostgreSQL's aggregate remains buildable without its optional test fixtures.
# These exact gated modules are part of that reviewed shape; every other module
# attribute still fails closed below.
POSTGRES_TEST_SUPPORT_MODULES = frozenset({
...
def module_path(name: str, attributes: list[str], source: Path) -> str | None:
    ...
    reviewed = list(
        REVIEWED_OUTER_ATTRIBUTES.get((display(source), name), ())
    )
    unreviewed = []
    for attribute in attributes:
        if attribute in reviewed:
            reviewed.remove(attribute)
        else:
            unreviewed.append(attribute)
    if reviewed:
        fail(f"reviewed attribute missing from {name} in {display(source)}")

    paths = []
    for attribute in unreviewed:
