...
class PublicationContract(unittest.TestCase):
    ...
    def attestation_results(
        self, predicate: dict, lock: dict, predicate_type: str
    ) -> list[dict]:
        return [
            {
                "verificationResult": {
                    "statement": {
                        "_type": publication.IN_TOTO_STATEMENT_TYPE,
                        "predicate": predicate,
                        "predicateType": predicate_type,
                        "subject": [
                            {
                                "digest": {
                                    "sha256": lock["image"].rsplit("sha256:", 1)[1]
                                },
                                "name": publication.IMAGE_NAME,
                            }
                        ],
                    }
                }
    ...
    def test_verified_attestations_must_match_lock_subject_and_predicates(self) -> None:
        lock_path, lock, identity = self.load_review()
        provenance_results = self.root / "provenance-results.json"
        identity_results = self.root / "identity-results.json"
        sbom_results = self.root / "sbom-results.json"
        source_ref = "refs/heads/main"
        provenance = {
            "buildDefinition": {
                "buildType": publication.GITHUB_WORKFLOW_BUILD_TYPE,
                "externalParameters": {
                    "workflow": {
                        "path": publication.WORKFLOW_PATH,
                        "ref": source_ref,
                        "repository": publication.REPOSITORY_URI,
                    }
                },
                "internalParameters": {
                    "github": {
                ...
                "resolvedDependencies": [
                    {
                        "digest": {"gitCommit": self.publisher_commit},
                        "uri": f"git+{publication.REPOSITORY_URI}@{source_ref}",
                    }
                ],
            },
            "runDetails": {
                "builder": {
                    "id": (
                        f"{publication.REPOSITORY_URI}/"
                        f"{publication.WORKFLOW_PATH}@{source_ref}"
                    )
                },
                "metadata": {
                    "invocationId": (
                        f"{publication.REPOSITORY_URI}/actions/runs/123/attempts/2"
                    )
                },
            },
        }
        provenance_results.write_text(
            json.dumps(
                self.attestation_results(
                    provenance, lock, publication.PROVENANCE_TYPE
                )
            ),
            encoding="utf-8",
        )
        identity_results.write_text(
            json.dumps(
                self.attestation_results(
                    identity, lock, publication.SOURCE_IDENTITY_TYPE
                )
            ),
            encoding="utf-8",
        )
        sbom_results.write_text(
            json.dumps(
                self.attestation_results(self.sbom, lock, publication.SBOM_TYPE)
            ),
            encoding="utf-8",
        )
        selected = self.root / "selected-identity.json"
        with contextlib.redirect_stdout(io.StringIO()):
            publication.verify_attestations(
                argparse.Namespace(
                    lock=lock_path,
                    provenance_results=provenance_results,
                    identity_results=identity_results,
                    sbom_results=sbom_results,
                    identity_output=selected,
                    github_output=None,
                )
            )
        self.assertEqual(json.loads(selected.read_bytes()), identity)

        wrong_provenance_type = self.attestation_results(
            provenance, lock, publication.SBOM_TYPE
        )
        provenance_results.write_text(
            json.dumps(wrong_provenance_type), encoding="utf-8"
        )
        with self.assertRaisesRegex(SystemExit, "exact locked-image subject"):
            publication.verify_attestations(
                argparse.Namespace(
                    lock=lock_path,
                    provenance_results=provenance_results,
                    identity_results=identity_results,
                    sbom_results=sbom_results,
                    identity_output=self.root / "not-written-provenance.json",
                    github_output=None,
                )
            )
        provenance_results.write_text(
        ...
        conflicting_identity = json.loads(json.dumps(identity))
        conflicting_identity["runtime"]["protocol_version"] = "2"
        identity_results.write_text(
            json.dumps(
                self.attestation_results(
                    identity, lock, publication.SOURCE_IDENTITY_TYPE
                ...
                + self.attestation_results(
                    conflicting_identity, lock, publication.SOURCE_IDENTITY_TYPE
                )
            ),
            encoding="utf-8",
        )
        with self.assertRaisesRegex(SystemExit, "identity conflicts"):
            publication.verify_attestations(
                argparse.Namespace(
                    lock=lock_path,
                    provenance_results=provenance_results,
                    identity_results=identity_results,
                    sbom_results=sbom_results,
                    identity_output=self.root / "not-written-conflict.json",
                    github_output=None,
                )
            )

        wrong_subject = self.attestation_results(
            identity, lock, publication.SOURCE_IDENTITY_TYPE
        )
        wrong_subject[0]["verificationResult"]["statement"]["subject"][0][
            "name"
        ] = "ghcr.io/automata-ci/different"
        identity_results.write_text(json.dumps(wrong_subject), encoding="utf-8")
        with self.assertRaisesRegex(SystemExit, "exact locked-image subject"):
            publication.verify_attestations(
                argparse.Namespace(
                    lock=lock_path,
                    ...
                    identity_results=identity_results,
                    sbom_results=sbom_results,
                    identity_output=self.root / "not-written.json",
                    github_output=None,
                )
            )
        ...
        changed_sbom = dict(self.sbom)
        changed_sbom["components"] = [{"name": "unexpected"}]
        sbom_results.write_text(
            json.dumps(
                self.attestation_results(self.sbom, lock, publication.SBOM_TYPE)
                + self.attestation_results(
                    changed_sbom, lock, publication.SBOM_TYPE
                )
            ),
            encoding="utf-8",
        )
        with self.assertRaisesRegex(SystemExit, "SBOM conflicts"):
            publication.verify_attestations(
                argparse.Namespace(
                    lock=lock_path,
                    provenance_results=provenance_results,
                    identity_results=identity_results,
                    sbom_results=sbom_results,
                    identity_output=self.root / "not-written-sbom.json",
                    github_output=None,
                )
            )
